...
# Every label this probe matches ("N words") is English, so the language has to
# be SET rather than inherited from the operator's OS
# (`startup.rs`'s `auto_detect_os_locale`) — and a private `XDG_CONFIG_HOME`
# also keeps this run off the operator's real settings/recents. A project is
# always passed, so the Welcome window never shows either way.
ENV = fixture.isolated_config(locale="en-US", label="wordcount", show_welcome=False)
