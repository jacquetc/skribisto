...
// ../use_cases/trash_selection_uc.rs.

use crate::use_cases::trash_selection_uc::{
    TrashSelectionUnitOfWorkFactoryTrait, TrashSelectionUnitOfWorkTrait,
};
...
use common::event::TrashManagementEvent::TrashSelection;
...
// Unit of work for TrashSelection

pub struct TrashSelectionUnitOfWork {
    context: DbContext,
...
impl TrashSelectionUnitOfWork {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        TrashSelectionUnitOfWork {
...
impl CommandUnitOfWork for TrashSelectionUnitOfWork {
...
//
// Exactly the same macros must be set in the use case uow trait file in ../use_cases/trash_selection_uc.rs
//
...
impl TrashSelectionUnitOfWorkTrait for TrashSelectionUnitOfWork {
    fn publish_trash_selection_event(&self, ids: Vec<EntityId>, data: Option<String>) {
        self.event_hub.send_event(Event {
            origin: Origin::TrashManagement(TrashSelection),
            ids,
            data,
        });
    }
...
pub struct TrashSelectionUnitOfWorkFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl TrashSelectionUnitOfWorkFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        TrashSelectionUnitOfWorkFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl TrashSelectionUnitOfWorkFactoryTrait for TrashSelectionUnitOfWorkFactory {
    fn create(&self) -> Box<dyn TrashSelectionUnitOfWorkTrait> {
        Box::new(TrashSelectionUnitOfWork::new(
            &self.context,
            &self.event_hub,
        ))
    }
