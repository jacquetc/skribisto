...
use crate::use_cases::trash_binder_items_uc::{
    TrashBinderItemsUnitOfWorkFactoryTrait, TrashBinderItemsUnitOfWorkTrait,
};
...
// Unit of work for TrashBinderItems

pub struct TrashBinderItemsUnitOfWork {
    context: DbContext,
...
impl TrashBinderItemsUnitOfWork {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        TrashBinderItemsUnitOfWork {
...
impl CommandUnitOfWork for TrashBinderItemsUnitOfWork {
    fn begin_transaction(&mut self) -> Result<()> {
        ...
        self.write_guard = Some(WriteTransactionGuard::acquire(&self.context, "trash_binder_items")?);
...
impl TrashBinderItemsUnitOfWorkTrait for TrashBinderItemsUnitOfWork {
    fn publish_trash_binder_items_event(&self, ids: Vec<EntityId>, data: Option<String>) {
        self.event_hub.send_event(Event {
            origin: Origin::TrashManagement(TrashBinderItems),
            ids,
            data,
        });
    }
...
pub struct TrashBinderItemsUnitOfWorkFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl TrashBinderItemsUnitOfWorkFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        TrashBinderItemsUnitOfWorkFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl TrashBinderItemsUnitOfWorkFactoryTrait for TrashBinderItemsUnitOfWorkFactory {
    fn create(&self) -> Box<dyn TrashBinderItemsUnitOfWorkTrait> {
        Box::new(TrashBinderItemsUnitOfWork::new(
            &self.context,
            &self.event_hub,
        ))
    }
