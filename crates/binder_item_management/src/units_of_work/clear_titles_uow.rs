...
// ../use_cases/clear_titles_uc.rs.

use crate::use_cases::clear_titles_uc::{
    ClearTitlesUnitOfWorkFactoryTrait, ClearTitlesUnitOfWorkTrait,
};
...
use common::event::BinderItemManagementEvent::ClearTitles;
...
// Unit of work for ClearTitles

pub struct ClearTitlesUnitOfWork {
    context: DbContext,
...
impl ClearTitlesUnitOfWork {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        ClearTitlesUnitOfWork {
...
impl CommandUnitOfWork for ClearTitlesUnitOfWork {
...
//
// Exactly the same macros must be set in the use case uow trait file in ../use_cases/clear_titles_uc.rs
//
...
impl ClearTitlesUnitOfWorkTrait for ClearTitlesUnitOfWork {
    fn publish_clear_titles_event(&self, ids: Vec<EntityId>, data: Option<String>) {
        self.event_hub.send_event(Event {
            origin: Origin::BinderItemManagement(ClearTitles),
            ids,
            data,
        });
    }
...
pub struct ClearTitlesUnitOfWorkFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl ClearTitlesUnitOfWorkFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        ClearTitlesUnitOfWorkFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl ClearTitlesUnitOfWorkFactoryTrait for ClearTitlesUnitOfWorkFactory {
    fn create(&self) -> Box<dyn ClearTitlesUnitOfWorkTrait> {
        Box::new(ClearTitlesUnitOfWork::new(&self.context, &self.event_hub))
    }
}
