...
use crate::use_cases::promote_uc::{PromoteUnitOfWorkFactoryTrait, PromoteUnitOfWorkTrait};
...
use common::event::BinderItemManagementEvent::Promote;
...
// Unit of work for Promote

pub struct PromoteUnitOfWork {
    context: DbContext,
...
impl CommandUnitOfWork for PromoteUnitOfWork {
    fn begin_transaction(&mut self) -> Result<()> {
        ...
        self.write_guard = Some(WriteTransactionGuard::acquire(&self.context, "promote_binder_item")?);
...
impl PromoteUnitOfWorkTrait for PromoteUnitOfWork {
    fn publish_promote_event(&self, ids: Vec<EntityId>, data: Option<String>) {
        self.event_hub.send_event(Event {
            origin: Origin::BinderItemManagement(Promote),
            ids,
            data,
        });
    }
...
pub struct PromoteUnitOfWorkFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl PromoteUnitOfWorkFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        PromoteUnitOfWorkFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl PromoteUnitOfWorkFactoryTrait for PromoteUnitOfWorkFactory {
    fn create(&self) -> Box<dyn PromoteUnitOfWorkTrait> {
        Box::new(PromoteUnitOfWork::new(&self.context, &self.event_hub))
    }
}
