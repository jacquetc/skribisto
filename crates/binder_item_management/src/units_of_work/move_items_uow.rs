...
use crate::use_cases::move_items_uc::{MoveItemsUnitOfWorkFactoryTrait, MoveItemsUnitOfWorkTrait};
...
use common::event::BinderItemManagementEvent::MoveItems;
...
// Unit of work for MoveItems

pub struct MoveItemsUnitOfWork {
    context: DbContext,
...
impl CommandUnitOfWork for MoveItemsUnitOfWork {
    fn begin_transaction(&mut self) -> Result<()> {
        ...
        self.write_guard = Some(WriteTransactionGuard::acquire(&self.context, "move_items")?);
...
//
// Exactly the same macros must be set in the use case uow trait file in ../use_cases/move_items_uc.rs
//
...
impl MoveItemsUnitOfWorkTrait for MoveItemsUnitOfWork {
    fn publish_move_items_event(&self, ids: Vec<EntityId>, data: Option<String>) {
        self.event_hub.send_event(Event {
            origin: Origin::BinderItemManagement(MoveItems),
            ids,
            data,
        });
    }
...
pub struct MoveItemsUnitOfWorkFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl MoveItemsUnitOfWorkFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        MoveItemsUnitOfWorkFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl MoveItemsUnitOfWorkFactoryTrait for MoveItemsUnitOfWorkFactory {
    fn create(&self) -> Box<dyn MoveItemsUnitOfWorkTrait> {
        Box::new(MoveItemsUnitOfWork::new(&self.context, &self.event_hub))
    }
}
