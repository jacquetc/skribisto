...
// Custom implementation: imports a legacy Skribisto `.skrib` (SQLite) into the
// in-memory store. The `legacy` module runs the full version-step upgrade chain
// (1.0 → 2.0, including HTML→Markdown via text-document) on a private copy, then
// maps the result to entities — mirroring the C++ Upgrader + migrateToV3, but
...
impl LoadWorkUseCase {
    ...
    pub fn execute(&mut self, dto: &LoadWorkDto) -> Result<()> {
        ...
        // Trashed entities (activated == false): indexed by TrashInfo so a trash
        // view can list them. A whole trashed binder vs a (item, container) pair.
        let mut trashed_binders: Vec<EntityId> = Vec::new();
        let mut trashed_items: Vec<(EntityId, EntityId)> = Vec::new();
        ...
        for binder in &project.binders {
            ...
            if !binder.activated {
                trashed_binders.push(created_binder.id);
            }
            ...
            for item in &binder.items {
                ...
                if !item.activated {
                    // Trashed items stay in their original binder (we don't move
                    // them to a Trash folder), so the containing binder is the
                    // true restore origin.
                    trashed_items.push((created_item.id, created_binder.id));
                }
        ...
        // Index trashed entities under System.trash_infos.
        let mut trash_info_ids: Vec<EntityId> = Vec::new();
        for binder_id in &trashed_binders {
        ...
        for (item_id, container_id) in &trashed_items {
            let info = uow.create_orphan_trash_info(&TrashInfo {
                created_at: now,
                updated_at: now,
                trashed_at: now,
                origin_binder_id: *container_id as i64,
                ..Default::default()
            })?;
            uow.set_trash_info_relationship(
                &info.id,
