...
// Custom implementation (hand-maintained): convert the open project to a single
// zipped `.skrib` at the requested path and record the new shape/path in
...
pub trait MigrateToSkribFileUnitOfWorkFactoryTrait: Send + Sync {
...
// on the impl block in ../units_of_work/migrate_to_skrib_file_uow.rs.
...
pub trait MigrateToSkribFileUnitOfWorkTrait: CommandUnitOfWork + Send + Sync {
    fn publish_migrate_to_skrib_file_event(&self, ids: Vec<EntityId>, data: Option<String>);
}

impl<'a> TreeReader for dyn MigrateToSkribFileUnitOfWorkTrait + 'a {
...
pub struct MigrateToSkribFileUseCase {
    uow_factory: Box<dyn MigrateToSkribFileUnitOfWorkFactoryTrait>,
    dto: MigrateToSkribFileDto,
}

impl MigrateToSkribFileUseCase {
    pub fn new(
        uow_factory: Box<dyn MigrateToSkribFileUnitOfWorkFactoryTrait>,
        dto: &MigrateToSkribFileDto,
    ) -> Self {
        MigrateToSkribFileUseCase {
            uow_factory,
            dto: dto.clone(),
        }
    }
...
impl LongOperation for MigrateToSkribFileUseCase {
    type Output = MigrateFileResultDto;
    ...
    fn execute(
        ...
        progress_callback(OperationProgress::new(
            0.0,
            Some("Converting to single file…".to_string()),
        ));
        ...
        let result = run_migrate(&*uow, &self.dto.file_name, now, &progress_callback, &cancel_flag);

        match result {
            Ok((work_id, output_path)) => {
                uow.commit()?;
                uow.publish_migrate_to_skrib_file_event(vec![work_id], None);
                progress_callback(OperationProgress::new(100.0, Some("completed".to_string())));
                Ok(MigrateFileResultDto { output_path })
            }
            Err(e) => {
                let _ = uow.rollback();
                Err(e)
...
fn run_migrate(
    uow: &dyn MigrateToSkribFileUnitOfWorkTrait,
    file_name: &str,
    ...
    let (target, shape, tag) =
        work_io::resolve_target(file_name, g.work_info.as_ref(), Some(SkribShape::ZipFile))?;
