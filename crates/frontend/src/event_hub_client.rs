...
// Generated by Qleany v1.9.0 from frontend_event_hub_client.tera
...
impl EventHubClient {
        ...
        F: Fn(Event) + Send + 'static,
    {
        ...
        {
            let mut subs = common::long_operation::lock_or_recover(&self.subscribers);
            ...
            loop {
                ...
                match outcome {
                    Ok(Some(event)) => {
                        ...
                        let subs = common::long_operation::lock_or_recover(&subscribers);
...
mod tests {
    ...
    fn fire_matching(client: &EventHubClient, event: Event) {
        let subs = common::long_operation::lock_or_recover(&client.subscribers);
        if let Some(callbacks) = subs.get(&event.origin) {
            for (_id, callback) in callbacks {
                callback(event.clone());
            }
    ...
    fn dropping_last_token_removes_origin_entry() {
        ...
        assert!(
            common::long_operation::lock_or_recover(&client.subscribers).contains_key(&origin_key)
        );
