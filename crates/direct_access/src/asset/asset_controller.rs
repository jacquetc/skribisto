...
use super::{
    dtos::{AssetDto, CreateAssetDto, UpdateAssetDto},
    units_of_work::{AssetReadUoWFactory, AssetWriteUoWFactory},
};
...
pub fn create_orphan(
    ...
    let uow_factory = AssetWriteUoWFactory::new(db_context, event_hub);
    let mut uc = use_cases::UndoableCreateOrphanUseCase::new(uow_factory);
    let entity_in: common::entities::Asset = entity.into();
...
pub fn create_orphan_multi(
    ...
    undo_redo_manager: &mut UndoRedoManager,
    stack_id: Option<u64>,
    entities: &[CreateAssetDto],
) -> Result<Vec<AssetDto>> {
    let uow_factory = AssetWriteUoWFactory::new(db_context, event_hub);
    let entities_in: Vec<common::entities::Asset> = entities.iter().map(|dto| dto.into()).collect();
    let mut uc = use_cases::UndoableCreateOrphanUseCase::new(uow_factory);
...
pub fn get(db_context: &DbContext, id: &EntityId) -> Result<Option<AssetDto>> {
    let uow_factory = AssetReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute(id)?.map(|e| e.into()))
}

pub fn get_all(db_context: &DbContext) -> Result<Vec<AssetDto>> {
    let uow_factory = AssetReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute_all()?.into_iter().map(|e| e.into()).collect())
}

pub fn get_multi(db_context: &DbContext, ids: &[EntityId]) -> Result<Vec<Option<AssetDto>>> {
    let uow_factory = AssetReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc
        .execute_multi(ids)?
...
mod tests {
    ...
    fn create_one(ctx: &mut TestContext) -> AssetDto {
        create_orphan(
            &ctx.db,
            &ctx.hub,
            &mut ctx.undo,
            None,
            &CreateAssetDto::default(),
        )
        .unwrap()
    }
    ...
    fn test_update() {
        let mut ctx = TestContext::new();
        let created = create_one(&mut ctx);
        let update_dto: UpdateAssetDto = created.into();
