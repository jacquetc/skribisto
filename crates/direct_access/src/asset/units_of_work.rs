...
impl CommandUnitOfWork for AssetWriteUoW {
...
impl use_cases::WriteUoW for AssetWriteUoW {
    type Entity = Asset;

    fn get(&self, id: &EntityId) -> Result<Option<Asset>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let repo = repository_factory::write::create_asset_repository(transaction)?;
        Ok(repo.get(id)?)
    }

    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Asset>>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let repo = repository_factory::write::create_asset_repository(transaction)?;
    ...
    fn create_orphan_multi(&self, entities: &[Asset]) -> Result<Vec<Asset>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let mut repo = repository_factory::write::create_asset_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
        Ok(repo.create_orphan_multi(&mut event_buffer, entities)?)
    }

    fn update_multi(&self, entities: &[Asset]) -> Result<Vec<Asset>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let mut repo = repository_factory::write::create_asset_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
    ...
    fn update_with_relationships_multi(&self, entities: &[Asset]) -> Result<Vec<Asset>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let mut repo = repository_factory::write::create_asset_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::OwnedWriteUoW for AssetWriteUoW {
    fn create_multi(
        &self,
        entities: &[Asset],
        owner_id: EntityId,
...
pub struct AssetWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl AssetWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        AssetWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl use_cases::WriteUoWFactory for AssetWriteUoWFactory {
    type Entity = Asset;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = Asset>> {
        Box::new(AssetWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::OwnedWriteUoWFactory for AssetWriteUoWFactory {
    type Entity = Asset;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = Asset>> {
        Box::new(AssetWriteUoW::new(&self.context, &self.event_hub))
    }
}
...
impl QueryUnitOfWork for AssetReadUoW {
...
impl use_cases::ReadUoW for AssetReadUoW {
    type Entity = Asset;

    fn get(&self, id: &EntityId) -> Result<Option<Asset>> {
        let transaction = self.transaction.borrow();
        let repo = repository_factory::read::create_asset_repository(
            transaction
                .as_ref()
                .ok_or_else(|| anyhow::anyhow!("No active transaction"))?,
        )?;
...
pub struct AssetReadUoWFactory {
    context: DbContext,
}

impl AssetReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        AssetReadUoWFactory {
            context: db_context.clone(),
        }
...
impl use_cases::ReadUoWFactory for AssetReadUoWFactory {
    type Entity = Asset;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = Asset>> {
        Box::new(AssetReadUoW::new(&self.context))
    }
}
