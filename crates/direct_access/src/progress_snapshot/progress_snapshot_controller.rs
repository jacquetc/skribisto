...
use super::{
    dtos::{CreateProgressSnapshotDto, ProgressSnapshotDto, UpdateProgressSnapshotDto},
    units_of_work::{ProgressSnapshotReadUoWFactory, ProgressSnapshotWriteUoWFactory},
};
...
pub fn create_orphan(
    ...
    entity: &CreateProgressSnapshotDto,
) -> Result<ProgressSnapshotDto> {
    let uow_factory = ProgressSnapshotWriteUoWFactory::new(db_context, event_hub);
    let mut uc = use_cases::CreateOrphanUseCase::new(uow_factory);
    let entity_in: common::entities::ProgressSnapshot = entity.into();
...
pub fn create_orphan_multi(
    db_context: &DbContext,
    event_hub: &Arc<EventHub>,
    entities: &[CreateProgressSnapshotDto],
) -> Result<Vec<ProgressSnapshotDto>> {
    let uow_factory = ProgressSnapshotWriteUoWFactory::new(db_context, event_hub);
    let entities_in: Vec<common::entities::ProgressSnapshot> =
...
pub fn get(db_context: &DbContext, id: &EntityId) -> Result<Option<ProgressSnapshotDto>> {
    let uow_factory = ProgressSnapshotReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute(id)?.map(|e| e.into()))
}

pub fn get_all(db_context: &DbContext) -> Result<Vec<ProgressSnapshotDto>> {
    let uow_factory = ProgressSnapshotReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
...
pub fn update(
    db_context: &DbContext,
    event_hub: &Arc<EventHub>,
    entity: &UpdateProgressSnapshotDto,
) -> Result<ProgressSnapshotDto> {
    let uow_factory = ProgressSnapshotWriteUoWFactory::new(db_context, event_hub);
    let entity_in: common::entities::ProgressSnapshot = entity.into();
...
pub fn update_multi(
    db_context: &DbContext,
    event_hub: &Arc<EventHub>,
    entities: &[UpdateProgressSnapshotDto],
) -> Result<Vec<ProgressSnapshotDto>> {
    let uow_factory = ProgressSnapshotWriteUoWFactory::new(db_context, event_hub);
    let entities_in: Vec<common::entities::ProgressSnapshot> =
        entities.iter().map(|dto| dto.into()).collect();
...
mod tests {
    ...
    fn create_one(ctx: &mut TestContext) -> ProgressSnapshotDto {
        create_orphan(&ctx.db, &ctx.hub, &CreateProgressSnapshotDto::default()).unwrap()
    }
    ...
    fn test_update() {
        let mut ctx = TestContext::new();
        let created = create_one(&mut ctx);
        let update_dto: UpdateProgressSnapshotDto = created.into();
    ...
    fn test_create_with_owner() {
        ...
        let created = create(
            &ctx.db,
            &ctx.hub,
            &CreateProgressSnapshotDto::default(),
            owner_id,
            -1,
        )
        .unwrap();
