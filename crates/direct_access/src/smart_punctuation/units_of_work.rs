...
pub struct SmartPunctuationWriteUoW {
    context: DbContext,
...
impl SmartPunctuationWriteUoW {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        SmartPunctuationWriteUoW {
...
impl CommandUnitOfWork for SmartPunctuationWriteUoW {
    fn begin_transaction(&mut self) -> Result<()> {
        ...
        self.write_guard = Some(WriteTransactionGuard::acquire(
            &self.context,
            "smart_punctuation_direct_write",
        )?);
...
impl use_cases::WriteUoW for SmartPunctuationWriteUoW {
    type Entity = SmartPunctuation;

    fn get(&self, id: &EntityId) -> Result<Option<SmartPunctuation>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let repo = repository_factory::write::create_smart_punctuation_repository(transaction)?;
        Ok(repo.get(id)?)
    }

    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<SmartPunctuation>>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let repo = repository_factory::write::create_smart_punctuation_repository(transaction)?;
        Ok(repo.get_multi(ids)?)
    }

    fn get_all(&self) -> Result<Vec<SmartPunctuation>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let repo = repository_factory::write::create_smart_punctuation_repository(transaction)?;
        Ok(repo.get_all()?)
    }

    fn create_orphan_multi(&self, entities: &[SmartPunctuation]) -> Result<Vec<SmartPunctuation>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let mut repo = repository_factory::write::create_smart_punctuation_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
        Ok(repo.create_orphan_multi(&mut event_buffer, entities)?)
    }

    fn update_multi(&self, entities: &[SmartPunctuation]) -> Result<Vec<SmartPunctuation>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let mut repo = repository_factory::write::create_smart_punctuation_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::OwnedWriteUoW for SmartPunctuationWriteUoW {
    fn create_multi(
        &self,
        entities: &[SmartPunctuation],
        owner_id: EntityId,
...
pub struct SmartPunctuationWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl SmartPunctuationWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        SmartPunctuationWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl use_cases::WriteUoWFactory for SmartPunctuationWriteUoWFactory {
    type Entity = SmartPunctuation;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = SmartPunctuation>> {
        Box::new(SmartPunctuationWriteUoW::new(
            &self.context,
            &self.event_hub,
        ))
    }
...
impl use_cases::OwnedWriteUoWFactory for SmartPunctuationWriteUoWFactory {
    type Entity = SmartPunctuation;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = SmartPunctuation>> {
        Box::new(SmartPunctuationWriteUoW::new(
            &self.context,
            &self.event_hub,
        ))
...
pub struct SmartPunctuationReadUoW {
    context: DbContext,
    transaction: RefCell<Option<Transaction>>,
}

impl SmartPunctuationReadUoW {
    pub fn new(db_context: &DbContext) -> Self {
        SmartPunctuationReadUoW {
            context: db_context.clone(),
...
impl QueryUnitOfWork for SmartPunctuationReadUoW {
...
impl use_cases::ReadUoW for SmartPunctuationReadUoW {
    type Entity = SmartPunctuation;

    fn get(&self, id: &EntityId) -> Result<Option<SmartPunctuation>> {
        let transaction = self.transaction.borrow();
        let repo = repository_factory::read::create_smart_punctuation_repository(
            transaction
                .as_ref()
                .ok_or_else(|| anyhow::anyhow!("No active transaction"))?,
        )?;
...
pub struct SmartPunctuationReadUoWFactory {
    context: DbContext,
}

impl SmartPunctuationReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        SmartPunctuationReadUoWFactory {
            context: db_context.clone(),
        }
    }
}

impl use_cases::ReadUoWFactory for SmartPunctuationReadUoWFactory {
    type Entity = SmartPunctuation;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = SmartPunctuation>> {
        Box::new(SmartPunctuationReadUoW::new(&self.context))
    }
}