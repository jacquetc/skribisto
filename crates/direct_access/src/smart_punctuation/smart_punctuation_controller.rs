...
use super::{
    dtos::{CreateSmartPunctuationDto, SmartPunctuationDto, UpdateSmartPunctuationDto},
    units_of_work::{SmartPunctuationReadUoWFactory, SmartPunctuationWriteUoWFactory},
};
...
pub fn create_orphan(
    ...
    stack_id: Option<u64>,
    entity: &CreateSmartPunctuationDto,
) -> Result<SmartPunctuationDto> {
    let uow_factory = SmartPunctuationWriteUoWFactory::new(db_context, event_hub);
    let mut uc = use_cases::UndoableCreateOrphanUseCase::new(uow_factory);
    let entity_in: common::entities::SmartPunctuation = entity.into();
...
pub fn create_orphan_multi(
    ...
    undo_redo_manager: &mut UndoRedoManager,
    stack_id: Option<u64>,
    entities: &[CreateSmartPunctuationDto],
) -> Result<Vec<SmartPunctuationDto>> {
    let uow_factory = SmartPunctuationWriteUoWFactory::new(db_context, event_hub);
    let entities_in: Vec<common::entities::SmartPunctuation> =
...
pub fn get(db_context: &DbContext, id: &EntityId) -> Result<Option<SmartPunctuationDto>> {
    let uow_factory = SmartPunctuationReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute(id)?.map(|e| e.into()))
}

pub fn get_all(db_context: &DbContext) -> Result<Vec<SmartPunctuationDto>> {
    let uow_factory = SmartPunctuationReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
...
pub fn update(
    ...
    undo_redo_manager: &mut UndoRedoManager,
    stack_id: Option<u64>,
    entity: &UpdateSmartPunctuationDto,
) -> Result<SmartPunctuationDto> {
    let uow_factory = SmartPunctuationWriteUoWFactory::new(db_context, event_hub);
    let entity_in: common::entities::SmartPunctuation = entity.into();
...
pub fn update_multi(
    ...
    undo_redo_manager: &mut UndoRedoManager,
    stack_id: Option<u64>,
    entities: &[UpdateSmartPunctuationDto],
) -> Result<Vec<SmartPunctuationDto>> {
    let uow_factory = SmartPunctuationWriteUoWFactory::new(db_context, event_hub);
    let entities_in: Vec<common::entities::SmartPunctuation> =
        entities.iter().map(|dto| dto.into()).collect();
...
mod tests {
    ...
    fn create_one(ctx: &mut TestContext) -> SmartPunctuationDto {
        create_orphan(
            &ctx.db,
            &ctx.hub,
            &mut ctx.undo,
    ...
    fn test_update() {
        let mut ctx = TestContext::new();
        let created = create_one(&mut ctx);
        let update_dto: UpdateSmartPunctuationDto = created.into();
