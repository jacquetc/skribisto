...
// Generated by Qleany v1.9.0 from entity_controller.tera

use super::{
    dtos::{CreateMilestoneDto, MilestoneDto, UpdateMilestoneDto},
    units_of_work::{MilestoneReadUoWFactory, MilestoneWriteUoWFactory},
};
use crate::MilestoneRelationshipDto;
...
pub fn create_orphan(
    ...
    stack_id: Option<u64>,
    entity: &CreateMilestoneDto,
) -> Result<MilestoneDto> {
    let uow_factory = MilestoneWriteUoWFactory::new(db_context, event_hub);
    let mut uc = use_cases::UndoableCreateOrphanUseCase::new(uow_factory);
    let entity_in: common::entities::Milestone = entity.into();
...
pub fn create_orphan_multi(
    ...
    undo_redo_manager: &mut UndoRedoManager,
    stack_id: Option<u64>,
    entities: &[CreateMilestoneDto],
) -> Result<Vec<MilestoneDto>> {
    let uow_factory = MilestoneWriteUoWFactory::new(db_context, event_hub);
    let entities_in: Vec<common::entities::Milestone> =
...
pub fn get(db_context: &DbContext, id: &EntityId) -> Result<Option<MilestoneDto>> {
    let uow_factory = MilestoneReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute(id)?.map(|e| e.into()))
}

pub fn get_all(db_context: &DbContext) -> Result<Vec<MilestoneDto>> {
    let uow_factory = MilestoneReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute_all()?.into_iter().map(|e| e.into()).collect())
}

pub fn get_multi(db_context: &DbContext, ids: &[EntityId]) -> Result<Vec<Option<MilestoneDto>>> {
    let uow_factory = MilestoneReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc
        .execute_multi(ids)?
...
pub fn get_relationship(
    ...
    let uow_factory = MilestoneReadUoWFactory::new(db_context);
    let uc = use_cases::GetRelationshipUseCase::<MilestoneRelationshipField, _>::new(uow_factory);
    uc.execute(id, field)
}
...
pub fn get_relationship_many(
    ...
    let uow_factory = MilestoneReadUoWFactory::new(db_context);
    let uc =
        use_cases::GetRelationshipManyUseCase::<MilestoneRelationshipField, _>::new(uow_factory);
...
pub fn get_relationship_count(
    ...
    let uow_factory = MilestoneReadUoWFactory::new(db_context);
    let uc =
        use_cases::GetRelationshipCountUseCase::<MilestoneRelationshipField, _>::new(uow_factory);
    uc.execute(id, field)
}
...
pub fn get_relationship_in_range(
    ...
    let uow_factory = MilestoneReadUoWFactory::new(db_context);
    let uc =
        use_cases::GetRelationshipInRangeUseCase::<MilestoneRelationshipField, _>::new(uow_factory);
...
pub fn set_relationship(
    ...
    let uow_factory = MilestoneWriteUoWFactory::new(db_context, event_hub);
    let mut uc = use_cases::UndoableSetRelationshipUseCase::<MilestoneRelationshipField, _>::new(
        uow_factory,
    );
...
pub fn move_relationship(
    ...
    let uow_factory = MilestoneWriteUoWFactory::new(db_context, event_hub);
    let mut uc = use_cases::UndoableMoveRelationshipUseCase::<MilestoneRelationshipField, _>::new(
        uow_factory,
    );
...
mod tests {
    ...
    use super::*;
    use crate::pace::dtos::CreatePaceDto;
    use crate::pace::pace_controller;
    ...
    fn create_owner_chain(ctx: &mut TestContext) -> EntityId {
        ...
        .unwrap();
        let pace = pace_controller::create(
            &ctx.db,
            &ctx.hub,
            &mut ctx.undo,
            None,
            &CreatePaceDto::default(),
            work.id,
            -1,
        )
        .unwrap();
    ...
    fn create_one(ctx: &mut TestContext) -> MilestoneDto {
        create_orphan(
            &ctx.db,
            &ctx.hub,
            &mut ctx.undo,
            None,
            &CreateMilestoneDto::default(),
        )
        .unwrap()
    }
    ...
    fn test_update() {
        let mut ctx = TestContext::new();
        let created = create_one(&mut ctx);
        let update_dto: UpdateMilestoneDto = created.into();
