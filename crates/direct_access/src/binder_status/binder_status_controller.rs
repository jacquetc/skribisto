...
use super::{
    dtos::{BinderStatusDto, CreateBinderStatusDto, UpdateBinderStatusDto},
    units_of_work::{BinderStatusReadUoWFactory, BinderStatusWriteUoWFactory},
};
...
pub fn create_orphan(
    ...
    stack_id: Option<u64>,
    entity: &CreateBinderStatusDto,
) -> Result<BinderStatusDto> {
    let uow_factory = BinderStatusWriteUoWFactory::new(db_context, event_hub);
    let mut uc = use_cases::UndoableCreateOrphanUseCase::new(uow_factory);
    let entity_in: common::entities::BinderStatus = entity.into();
...
pub fn create_orphan_multi(
    ...
    undo_redo_manager: &mut UndoRedoManager,
    stack_id: Option<u64>,
    entities: &[CreateBinderStatusDto],
) -> Result<Vec<BinderStatusDto>> {
    let uow_factory = BinderStatusWriteUoWFactory::new(db_context, event_hub);
    let entities_in: Vec<common::entities::BinderStatus> =
...
pub fn get(db_context: &DbContext, id: &EntityId) -> Result<Option<BinderStatusDto>> {
    let uow_factory = BinderStatusReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute(id)?.map(|e| e.into()))
}

pub fn get_all(db_context: &DbContext) -> Result<Vec<BinderStatusDto>> {
    let uow_factory = BinderStatusReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute_all()?.into_iter().map(|e| e.into()).collect())
}

pub fn get_multi(db_context: &DbContext, ids: &[EntityId]) -> Result<Vec<Option<BinderStatusDto>>> {
    let uow_factory = BinderStatusReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc
        .execute_multi(ids)?
...
pub fn update_multi(
    ...
    undo_redo_manager: &mut UndoRedoManager,
    stack_id: Option<u64>,
    entities: &[UpdateBinderStatusDto],
) -> Result<Vec<BinderStatusDto>> {
    let uow_factory = BinderStatusWriteUoWFactory::new(db_context, event_hub);
    let entities_in: Vec<common::entities::BinderStatus> =
        entities.iter().map(|dto| dto.into()).collect();
...
mod tests {
    ...
    fn create_one(ctx: &mut TestContext) -> BinderStatusDto {
        create_orphan(
            &ctx.db,
            &ctx.hub,
            &mut ctx.undo,
            None,
            &CreateBinderStatusDto::default(),
        )
        .unwrap()
    }
    ...
    fn test_update() {
        let mut ctx = TestContext::new();
        let created = create_one(&mut ctx);
        let update_dto: UpdateBinderStatusDto = created.into();
