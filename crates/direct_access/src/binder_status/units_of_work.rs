...
pub struct BinderStatusWriteUoW {
    context: DbContext,
...
impl BinderStatusWriteUoW {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        BinderStatusWriteUoW {
...
impl CommandUnitOfWork for BinderStatusWriteUoW {
...
impl use_cases::WriteUoW for BinderStatusWriteUoW {
    type Entity = BinderStatus;

    fn get(&self, id: &EntityId) -> Result<Option<BinderStatus>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let repo = repository_factory::write::create_binder_status_repository(transaction)?;
        Ok(repo.get(id)?)
    }

    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<BinderStatus>>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let repo = repository_factory::write::create_binder_status_repository(transaction)?;
        Ok(repo.get_multi(ids)?)
    }

    fn get_all(&self) -> Result<Vec<BinderStatus>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let repo = repository_factory::write::create_binder_status_repository(transaction)?;
        Ok(repo.get_all()?)
    }

    fn create_orphan_multi(&self, entities: &[BinderStatus]) -> Result<Vec<BinderStatus>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let mut repo = repository_factory::write::create_binder_status_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
        Ok(repo.create_orphan_multi(&mut event_buffer, entities)?)
    }

    fn update_multi(&self, entities: &[BinderStatus]) -> Result<Vec<BinderStatus>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let mut repo = repository_factory::write::create_binder_status_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::OwnedWriteUoW for BinderStatusWriteUoW {
    fn create_multi(
        &self,
        entities: &[BinderStatus],
        owner_id: EntityId,
...
pub struct BinderStatusWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl BinderStatusWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        BinderStatusWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl use_cases::WriteUoWFactory for BinderStatusWriteUoWFactory {
    type Entity = BinderStatus;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = BinderStatus>> {
        Box::new(BinderStatusWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::OwnedWriteUoWFactory for BinderStatusWriteUoWFactory {
    type Entity = BinderStatus;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = BinderStatus>> {
        Box::new(BinderStatusWriteUoW::new(&self.context, &self.event_hub))
    }
}
...
pub struct BinderStatusReadUoW {
    context: DbContext,
    transaction: RefCell<Option<Transaction>>,
}

impl BinderStatusReadUoW {
...
impl QueryUnitOfWork for BinderStatusReadUoW {
...
impl use_cases::ReadUoW for BinderStatusReadUoW {
    type Entity = BinderStatus;

    fn get(&self, id: &EntityId) -> Result<Option<BinderStatus>> {
        let transaction = self.transaction.borrow();
        let repo = repository_factory::read::create_binder_status_repository(
            transaction
                .as_ref()
                .ok_or_else(|| anyhow::anyhow!("No active transaction"))?,
        )?;
...
pub struct BinderStatusReadUoWFactory {
    context: DbContext,
}

impl BinderStatusReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        BinderStatusReadUoWFactory {
            context: db_context.clone(),
        }
    }
}

impl use_cases::ReadUoWFactory for BinderStatusReadUoWFactory {
    type Entity = BinderStatus;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = BinderStatus>> {
        Box::new(BinderStatusReadUoW::new(&self.context))
    }
}
