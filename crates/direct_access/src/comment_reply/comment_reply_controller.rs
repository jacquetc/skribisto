...
use super::{
    dtos::{CommentReplyDto, CreateCommentReplyDto, UpdateCommentReplyDto},
    units_of_work::{CommentReplyReadUoWFactory, CommentReplyWriteUoWFactory},
};
...
pub fn create_orphan(
    ...
    stack_id: Option<u64>,
    entity: &CreateCommentReplyDto,
) -> Result<CommentReplyDto> {
    let uow_factory = CommentReplyWriteUoWFactory::new(db_context, event_hub);
    let mut uc = use_cases::UndoableCreateOrphanUseCase::new(uow_factory);
    let entity_in: common::entities::CommentReply = entity.into();
...
pub fn create_orphan_multi(
    ...
    undo_redo_manager: &mut UndoRedoManager,
    stack_id: Option<u64>,
    entities: &[CreateCommentReplyDto],
) -> Result<Vec<CommentReplyDto>> {
    let uow_factory = CommentReplyWriteUoWFactory::new(db_context, event_hub);
    let entities_in: Vec<common::entities::CommentReply> =
...
pub fn get(db_context: &DbContext, id: &EntityId) -> Result<Option<CommentReplyDto>> {
    let uow_factory = CommentReplyReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute(id)?.map(|e| e.into()))
}

pub fn get_all(db_context: &DbContext) -> Result<Vec<CommentReplyDto>> {
    let uow_factory = CommentReplyReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute_all()?.into_iter().map(|e| e.into()).collect())
}

pub fn get_multi(db_context: &DbContext, ids: &[EntityId]) -> Result<Vec<Option<CommentReplyDto>>> {
    let uow_factory = CommentReplyReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc
        .execute_multi(ids)?
...
pub fn update_multi(
    ...
    undo_redo_manager: &mut UndoRedoManager,
    stack_id: Option<u64>,
    entities: &[UpdateCommentReplyDto],
) -> Result<Vec<CommentReplyDto>> {
    let uow_factory = CommentReplyWriteUoWFactory::new(db_context, event_hub);
    let entities_in: Vec<common::entities::CommentReply> =
        entities.iter().map(|dto| dto.into()).collect();
...
mod tests {
    ...
    use super::*;
    use crate::comment::comment_controller;
    use crate::comment::dtos::CreateCommentDto;
    ...
    fn create_one(ctx: &mut TestContext) -> CommentReplyDto {
        create_orphan(
            &ctx.db,
            &ctx.hub,
            &mut ctx.undo,
            None,
            &CreateCommentReplyDto::default(),
        )
        .unwrap()
    }
    ...
    fn test_update() {
        let mut ctx = TestContext::new();
        let created = create_one(&mut ctx);
        let update_dto: UpdateCommentReplyDto = created.into();
