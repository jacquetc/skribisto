...
pub struct CommentReplyWriteUoW {
    context: DbContext,
...
impl CommentReplyWriteUoW {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        CommentReplyWriteUoW {
...
impl CommandUnitOfWork for CommentReplyWriteUoW {
...
impl use_cases::WriteUoW for CommentReplyWriteUoW {
    type Entity = CommentReply;

    fn get(&self, id: &EntityId) -> Result<Option<CommentReply>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_comment_reply_repository(transaction)?;
        Ok(repo.get(id)?)
    }

    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<CommentReply>>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_comment_reply_repository(transaction)?;
        Ok(repo.get_multi(ids)?)
    }

    fn get_all(&self) -> Result<Vec<CommentReply>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_comment_reply_repository(transaction)?;
        Ok(repo.get_all()?)
    }

    fn create_orphan_multi(&self, entities: &[CommentReply]) -> Result<Vec<CommentReply>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_comment_reply_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
        Ok(repo.create_orphan_multi(&mut event_buffer, entities)?)
    }

    fn update_multi(&self, entities: &[CommentReply]) -> Result<Vec<CommentReply>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_comment_reply_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::OwnedWriteUoW for CommentReplyWriteUoW {
    fn create_multi(
        &self,
        entities: &[CommentReply],
        owner_id: EntityId,
...
pub struct CommentReplyWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl CommentReplyWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        CommentReplyWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl use_cases::WriteUoWFactory for CommentReplyWriteUoWFactory {
    type Entity = CommentReply;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = CommentReply>> {
        Box::new(CommentReplyWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::OwnedWriteUoWFactory for CommentReplyWriteUoWFactory {
    type Entity = CommentReply;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = CommentReply>> {
        Box::new(CommentReplyWriteUoW::new(&self.context, &self.event_hub))
    }
}
...
pub struct CommentReplyReadUoW {
    context: DbContext,
    transaction: RefCell<Option<Transaction>>,
}

impl CommentReplyReadUoW {
...
impl QueryUnitOfWork for CommentReplyReadUoW {
...
impl use_cases::ReadUoW for CommentReplyReadUoW {
    type Entity = CommentReply;

    fn get(&self, id: &EntityId) -> Result<Option<CommentReply>> {
        let transaction = self.transaction.borrow();
        let repo = repository_factory::read::create_comment_reply_repository(
            transaction
                .as_ref()
                .ok_or_else(|| anyhow::anyhow!("No active transaction"))?,
        )?;
...
pub struct CommentReplyReadUoWFactory {
    context: DbContext,
}

impl CommentReplyReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        CommentReplyReadUoWFactory {
            context: db_context.clone(),
        }
    }
}

impl use_cases::ReadUoWFactory for CommentReplyReadUoWFactory {
    type Entity = CommentReply;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = CommentReply>> {
        Box::new(CommentReplyReadUoW::new(&self.context))
    }
}
