...
impl CommandUnitOfWork for WorkWriteUoW {
    fn begin_transaction(&mut self) -> Result<()> {
        ...
        self.write_guard = Some(WriteTransactionGuard::acquire(&self.context, "work_direct_write")?);
...
impl use_cases::WriteUoW for WorkWriteUoW {
    type Entity = Work;

    fn get(&self, id: &EntityId) -> Result<Option<Work>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_work_repository(transaction)?;
        Ok(repo.get(id)?)
    }

    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Work>>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_work_repository(transaction)?;
        Ok(repo.get_multi(ids)?)
    }

    fn get_all(&self) -> Result<Vec<Work>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_work_repository(transaction)?;
        Ok(repo.get_all()?)
    }

    fn create_orphan_multi(&self, entities: &[Work]) -> Result<Vec<Work>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_work_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
        Ok(repo.create_orphan_multi(&mut event_buffer, entities)?)
    }

    fn update_multi(&self, entities: &[Work]) -> Result<Vec<Work>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_work_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
    ...
    fn update_with_relationships_multi(&self, entities: &[Work]) -> Result<Vec<Work>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_work_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::OwnedWriteUoW for WorkWriteUoW {
    fn create_multi(&self, entities: &[Work], owner_id: EntityId, index: i32) -> Result<Vec<Work>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_work_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::WriteRelUoW<WorkRelationshipField> for WorkWriteUoW {
    fn get_relationship(
        &self,
        id: &EntityId,
...
pub struct WorkWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl WorkWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        WorkWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
...
impl use_cases::WriteUoWFactory for WorkWriteUoWFactory {
    type Entity = Work;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = Work>> {
        Box::new(WorkWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::OwnedWriteUoWFactory for WorkWriteUoWFactory {
    type Entity = Work;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = Work>> {
        Box::new(WorkWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::WriteRelUoWFactory<WorkRelationshipField> for WorkWriteUoWFactory {
    fn create(&self) -> Box<dyn use_cases::WriteRelUoW<WorkRelationshipField>> {
        Box::new(WorkWriteUoW::new(&self.context, &self.event_hub))
    }
}
...
impl QueryUnitOfWork for WorkReadUoW {
...
impl use_cases::ReadUoW for WorkReadUoW {
    type Entity = Work;

    fn get(&self, id: &EntityId) -> Result<Option<Work>> {
        let transaction = self.transaction.borrow();
        let repo = repository_factory::read::create_work_repository(
            transaction
                .as_ref()
                .ok_or_else(|| anyhow::anyhow!("No active transaction"))?,
        )?;
...
impl use_cases::ReadRelUoW<WorkRelationshipField> for WorkReadUoW {
    fn get_relationship(
        &self,
        id: &EntityId,
        field: &WorkRelationshipField,
...
pub struct WorkReadUoWFactory {
    context: DbContext,
}

impl WorkReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        WorkReadUoWFactory {
            context: db_context.clone(),
        }
...
impl use_cases::ReadUoWFactory for WorkReadUoWFactory {
    type Entity = Work;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = Work>> {
        Box::new(WorkReadUoW::new(&self.context))
    }
}

impl use_cases::ReadRelUoWFactory<WorkRelationshipField> for WorkReadUoWFactory {
    fn create(&self) -> Box<dyn use_cases::ReadRelUoW<WorkRelationshipField>> {
        Box::new(WorkReadUoW::new(&self.context))
    }
}
