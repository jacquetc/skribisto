...
pub struct NoteTemplateWriteUoW {
    context: DbContext,
...
impl NoteTemplateWriteUoW {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        NoteTemplateWriteUoW {
...
impl CommandUnitOfWork for NoteTemplateWriteUoW {
...
impl use_cases::WriteUoW for NoteTemplateWriteUoW {
    type Entity = NoteTemplate;

    fn get(&self, id: &EntityId) -> Result<Option<NoteTemplate>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let repo = repository_factory::write::create_note_template_repository(transaction)?;
        Ok(repo.get(id)?)
    }

    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<NoteTemplate>>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let repo = repository_factory::write::create_note_template_repository(transaction)?;
        Ok(repo.get_multi(ids)?)
    }

    fn get_all(&self) -> Result<Vec<NoteTemplate>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let repo = repository_factory::write::create_note_template_repository(transaction)?;
        Ok(repo.get_all()?)
    }

    fn create_orphan_multi(&self, entities: &[NoteTemplate]) -> Result<Vec<NoteTemplate>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let mut repo = repository_factory::write::create_note_template_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
        Ok(repo.create_orphan_multi(&mut event_buffer, entities)?)
    }

    fn update_multi(&self, entities: &[NoteTemplate]) -> Result<Vec<NoteTemplate>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let mut repo = repository_factory::write::create_note_template_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::OwnedWriteUoW for NoteTemplateWriteUoW {
    fn create_multi(
        &self,
        entities: &[NoteTemplate],
        owner_id: EntityId,
...
pub struct NoteTemplateWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl NoteTemplateWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        NoteTemplateWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl use_cases::WriteUoWFactory for NoteTemplateWriteUoWFactory {
    type Entity = NoteTemplate;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = NoteTemplate>> {
        Box::new(NoteTemplateWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::OwnedWriteUoWFactory for NoteTemplateWriteUoWFactory {
    type Entity = NoteTemplate;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = NoteTemplate>> {
        Box::new(NoteTemplateWriteUoW::new(&self.context, &self.event_hub))
    }
}
...
pub struct NoteTemplateReadUoW {
    context: DbContext,
    transaction: RefCell<Option<Transaction>>,
}

impl NoteTemplateReadUoW {
...
impl QueryUnitOfWork for NoteTemplateReadUoW {
...
impl use_cases::ReadUoW for NoteTemplateReadUoW {
    type Entity = NoteTemplate;

    fn get(&self, id: &EntityId) -> Result<Option<NoteTemplate>> {
        let transaction = self.transaction.borrow();
        let repo = repository_factory::read::create_note_template_repository(
            transaction
                .as_ref()
                .ok_or_else(|| anyhow::anyhow!("No active transaction"))?,
        )?;
...
pub struct NoteTemplateReadUoWFactory {
    context: DbContext,
}

impl NoteTemplateReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        NoteTemplateReadUoWFactory {
            context: db_context.clone(),
        }
    }
}

impl use_cases::ReadUoWFactory for NoteTemplateReadUoWFactory {
    type Entity = NoteTemplate;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = NoteTemplate>> {
        Box::new(NoteTemplateReadUoW::new(&self.context))
    }
}
