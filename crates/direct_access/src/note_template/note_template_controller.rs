...
use super::{
    dtos::{CreateNoteTemplateDto, NoteTemplateDto, UpdateNoteTemplateDto},
    units_of_work::{NoteTemplateReadUoWFactory, NoteTemplateWriteUoWFactory},
};
...
/// carry the value they read.
///
/// A template's uid is what names its `.djot` blob on disk
/// (`skrib_format::slug::note_template_relpath`); without this, every template
/// file was renamed on every reopen, because the `EntityId` it used to be keyed
/// by is re-minted by `load_work`.
fn with_identity(mut e: common::entities::NoteTemplate) -> common::entities::NoteTemplate {
    if e.uid.is_nil() {
...
pub fn create_orphan(
    ...
    stack_id: Option<u64>,
    entity: &CreateNoteTemplateDto,
) -> Result<NoteTemplateDto> {
    let uow_factory = NoteTemplateWriteUoWFactory::new(db_context, event_hub);
    let mut uc = use_cases::UndoableCreateOrphanUseCase::new(uow_factory);
    let entity_in: common::entities::NoteTemplate = with_identity(entity.into());
...
pub fn create_orphan_multi(
    ...
    undo_redo_manager: &mut UndoRedoManager,
    stack_id: Option<u64>,
    entities: &[CreateNoteTemplateDto],
) -> Result<Vec<NoteTemplateDto>> {
    let uow_factory = NoteTemplateWriteUoWFactory::new(db_context, event_hub);
    let entities_in: Vec<common::entities::NoteTemplate> = entities
        .iter()
...
pub fn get(db_context: &DbContext, id: &EntityId) -> Result<Option<NoteTemplateDto>> {
    let uow_factory = NoteTemplateReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute(id)?.map(|e| e.into()))
}

pub fn get_all(db_context: &DbContext) -> Result<Vec<NoteTemplateDto>> {
    let uow_factory = NoteTemplateReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute_all()?.into_iter().map(|e| e.into()).collect())
}

pub fn get_multi(db_context: &DbContext, ids: &[EntityId]) -> Result<Vec<Option<NoteTemplateDto>>> {
    let uow_factory = NoteTemplateReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc
        .execute_multi(ids)?
...
pub fn update(
    ...
    undo_redo_manager: &mut UndoRedoManager,
    stack_id: Option<u64>,
    entity: &UpdateNoteTemplateDto,
) -> Result<NoteTemplateDto> {
    let uow_factory = NoteTemplateWriteUoWFactory::new(db_context, event_hub);
    let entity_in: common::entities::NoteTemplate = entity.into();
...
pub fn update_multi(
    ...
    undo_redo_manager: &mut UndoRedoManager,
    stack_id: Option<u64>,
    entities: &[UpdateNoteTemplateDto],
) -> Result<Vec<NoteTemplateDto>> {
    let uow_factory = NoteTemplateWriteUoWFactory::new(db_context, event_hub);
    let entities_in: Vec<common::entities::NoteTemplate> =
...
mod tests {
    ...
    fn create_one(ctx: &mut TestContext) -> NoteTemplateDto {
        create_orphan(
            &ctx.db,
            &ctx.hub,
            &mut ctx.undo,
            None,
            &CreateNoteTemplateDto::default(),
        )
        .unwrap()
    }
    ...
    fn test_update() {
        let mut ctx = TestContext::new();
        let created = create_one(&mut ctx);
        let update_dto: UpdateNoteTemplateDto = created.into();
