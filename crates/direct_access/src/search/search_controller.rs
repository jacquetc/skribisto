...
use super::{
    dtos::{CreateSearchDto, SearchDto, UpdateSearchDto},
    units_of_work::{SearchReadUoWFactory, SearchWriteUoWFactory},
};
use crate::SearchRelationshipDto;
...
pub fn create_orphan(
    ...
    entity: &CreateSearchDto,
) -> Result<SearchDto> {
    let uow_factory = SearchWriteUoWFactory::new(db_context, event_hub);
    let mut uc = use_cases::CreateOrphanUseCase::new(uow_factory);
    let entity_in: common::entities::Search = entity.into();
...
pub fn create_orphan_multi(
    db_context: &DbContext,
    event_hub: &Arc<EventHub>,
    entities: &[CreateSearchDto],
) -> Result<Vec<SearchDto>> {
    let uow_factory = SearchWriteUoWFactory::new(db_context, event_hub);
    let entities_in: Vec<common::entities::Search> =
...
pub fn get(db_context: &DbContext, id: &EntityId) -> Result<Option<SearchDto>> {
    let uow_factory = SearchReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute(id)?.map(|e| e.into()))
}

pub fn get_all(db_context: &DbContext) -> Result<Vec<SearchDto>> {
    let uow_factory = SearchReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute_all()?.into_iter().map(|e| e.into()).collect())
}

pub fn get_multi(db_context: &DbContext, ids: &[EntityId]) -> Result<Vec<Option<SearchDto>>> {
    let uow_factory = SearchReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc
        .execute_multi(ids)?
...
pub fn get_relationship(
    ...
    let uow_factory = SearchReadUoWFactory::new(db_context);
    let uc = use_cases::GetRelationshipUseCase::<SearchRelationshipField, _>::new(uow_factory);
    uc.execute(id, field)
}
...
pub fn get_relationship_many(
    ...
    let uow_factory = SearchReadUoWFactory::new(db_context);
    let uc = use_cases::GetRelationshipManyUseCase::<SearchRelationshipField, _>::new(uow_factory);
...
pub fn get_relationship_count(
    ...
    let uow_factory = SearchReadUoWFactory::new(db_context);
    let uc = use_cases::GetRelationshipCountUseCase::<SearchRelationshipField, _>::new(uow_factory);
    uc.execute(id, field)
}
...
pub fn get_relationship_in_range(
    ...
    let uow_factory = SearchReadUoWFactory::new(db_context);
    let uc =
        use_cases::GetRelationshipInRangeUseCase::<SearchRelationshipField, _>::new(uow_factory);
...
pub fn set_relationship(
    db_context: &DbContext,
    event_hub: &Arc<EventHub>,
    dto: &SearchRelationshipDto,
) -> Result<()> {
    let uow_factory = SearchWriteUoWFactory::new(db_context, event_hub);
    let mut uc = use_cases::SetRelationshipUseCase::<SearchRelationshipField, _>::new(uow_factory);
...
pub fn move_relationship(
    ...
    let uow_factory = SearchWriteUoWFactory::new(db_context, event_hub);
    let mut uc = use_cases::MoveRelationshipUseCase::<SearchRelationshipField, _>::new(uow_factory);
...
mod tests {
    ...
    fn create_one(ctx: &mut TestContext) -> SearchDto {
        create_orphan(&ctx.db, &ctx.hub, &CreateSearchDto::default()).unwrap()
    }
    ...
    fn test_update() {
        let mut ctx = TestContext::new();
        let created = create_one(&mut ctx);
        let update_dto: UpdateSearchDto = created.into();
    ...
    fn test_create_with_owner() {
        ...
        let created = create(&ctx.db, &ctx.hub, &CreateSearchDto::default(), owner_id, -1).unwrap();
        assert!(created.id > 0);
        let fetched = get(&ctx.db, &created.id).unwrap();
        assert!(fetched.is_some());
    }
    ...
    fn test_get_relationship_default() {
        ...
        let created = create_one(&mut ctx);
        let rel_ids =
            get_relationship(&ctx.db, &created.id, &SearchRelationshipField::Results).unwrap();
    ...
    fn test_get_relationship_count_default() {
        let mut ctx = TestContext::new();
        let created = create_one(&mut ctx);
        let count = get_relationship_count(&ctx.db, &created.id, &SearchRelationshipField::Results)
            .unwrap();
        assert_eq!(count, 0);
    }
}
