...
use super::{
    dtos::{BinderItemDto, CreateBinderItemDto, UpdateBinderItemDto},
    units_of_work::{BinderItemReadUoWFactory, BinderItemWriteUoWFactory},
};
use crate::BinderItemRelationshipDto;
...
pub fn get(db_context: &DbContext, id: &EntityId) -> Result<Option<BinderItemDto>> {
    let uow_factory = BinderItemReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute(id)?.map(|e| e.into()))
}

pub fn get_all(db_context: &DbContext) -> Result<Vec<BinderItemDto>> {
    let uow_factory = BinderItemReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute_all()?.into_iter().map(|e| e.into()).collect())
}

pub fn get_multi(db_context: &DbContext, ids: &[EntityId]) -> Result<Vec<Option<BinderItemDto>>> {
    let uow_factory = BinderItemReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc
        .execute_multi(ids)?
...
pub fn get_relationship(
    ...
    let uow_factory = BinderItemReadUoWFactory::new(db_context);
    let uc = use_cases::GetRelationshipUseCase::<BinderItemRelationshipField, _>::new(uow_factory);
    uc.execute(id, field)
}
...
pub fn get_relationship_many(
    ...
    let uow_factory = BinderItemReadUoWFactory::new(db_context);
    let uc =
        use_cases::GetRelationshipManyUseCase::<BinderItemRelationshipField, _>::new(uow_factory);
...
pub fn get_relationship_count(
    ...
    let uow_factory = BinderItemReadUoWFactory::new(db_context);
    let uc =
        use_cases::GetRelationshipCountUseCase::<BinderItemRelationshipField, _>::new(uow_factory);
    uc.execute(id, field)
}
...
pub fn get_relationship_in_range(
    ...
    let uow_factory = BinderItemReadUoWFactory::new(db_context);
    let uc = use_cases::GetRelationshipInRangeUseCase::<BinderItemRelationshipField, _>::new(
        uow_factory,
    );
...
pub fn set_relationship(
    ...
    let uow_factory = BinderItemWriteUoWFactory::new(db_context, event_hub);
    let mut uc = use_cases::UndoableSetRelationshipUseCase::<BinderItemRelationshipField, _>::new(
        uow_factory,
    );
...
pub fn move_relationship(
    ...
    let uow_factory = BinderItemWriteUoWFactory::new(db_context, event_hub);
    let mut uc = use_cases::UndoableMoveRelationshipUseCase::<BinderItemRelationshipField, _>::new(
        uow_factory,
    );
...
mod tests {
    ...
    use super::*;
    use crate::binder::binder_controller;
    use crate::binder::dtos::CreateBinderDto;
    ...
    fn create_owner_chain(ctx: &mut TestContext) -> EntityId {
        ...
        .unwrap();
        let binder = binder_controller::create(
            &ctx.db,
            &ctx.hub,
            &mut ctx.undo,
            None,
    ...
    fn create_one(ctx: &mut TestContext) -> BinderItemDto {
        create_orphan(
            &ctx.db,
            &ctx.hub,
            &mut ctx.undo,
    ...
    fn test_update() {
        let mut ctx = TestContext::new();
        let created = create_one(&mut ctx);
        let update_dto: UpdateBinderItemDto = created.into();
    ...
    fn test_get_relationship_default() {
        ...
        let created = create_one(&mut ctx);
        let rel_ids =
            get_relationship(&ctx.db, &created.id, &BinderItemRelationshipField::Contents).unwrap();
    ...
    fn test_get_relationship_count_default() {
        ...
        let created = create_one(&mut ctx);
        let count =
            get_relationship_count(&ctx.db, &created.id, &BinderItemRelationshipField::Contents)
                .unwrap();
        assert_eq!(count, 0);
    }
}
