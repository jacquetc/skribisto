...
// Generated by Qleany v1.10.0 from entity_units_of_work.tera
...
impl CommandUnitOfWork for BinderTagWriteUoW {
...
impl use_cases::WriteUoW for BinderTagWriteUoW {
    type Entity = BinderTag;

    fn get(&self, id: &EntityId) -> Result<Option<BinderTag>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let repo = repository_factory::write::create_binder_tag_repository(transaction)?;
        Ok(repo.get(id)?)
    }

    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<BinderTag>>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let repo = repository_factory::write::create_binder_tag_repository(transaction)?;
        Ok(repo.get_multi(ids)?)
    }

    fn get_all(&self) -> Result<Vec<BinderTag>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let repo = repository_factory::write::create_binder_tag_repository(transaction)?;
        Ok(repo.get_all()?)
    }

    fn create_orphan_multi(&self, entities: &[BinderTag]) -> Result<Vec<BinderTag>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let mut repo = repository_factory::write::create_binder_tag_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
        Ok(repo.create_orphan_multi(&mut event_buffer, entities)?)
    }

    fn update_multi(&self, entities: &[BinderTag]) -> Result<Vec<BinderTag>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let mut repo = repository_factory::write::create_binder_tag_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
    ...
    fn update_with_relationships_multi(&self, entities: &[BinderTag]) -> Result<Vec<BinderTag>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let mut repo = repository_factory::write::create_binder_tag_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::OwnedWriteUoW for BinderTagWriteUoW {
    fn create_multi(
        &self,
        entities: &[BinderTag],
        owner_id: EntityId,
        index: i32,
    ) -> Result<Vec<BinderTag>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let mut repo = repository_factory::write::create_binder_tag_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::WriteRelUoW<BinderTagRelationshipField> for BinderTagWriteUoW {
    fn get_relationship(
        &self,
        id: &EntityId,
...
pub struct BinderTagWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl BinderTagWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        BinderTagWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl use_cases::WriteUoWFactory for BinderTagWriteUoWFactory {
    type Entity = BinderTag;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = BinderTag>> {
        Box::new(BinderTagWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::OwnedWriteUoWFactory for BinderTagWriteUoWFactory {
    type Entity = BinderTag;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = BinderTag>> {
        Box::new(BinderTagWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::WriteRelUoWFactory<BinderTagRelationshipField> for BinderTagWriteUoWFactory {
    fn create(&self) -> Box<dyn use_cases::WriteRelUoW<BinderTagRelationshipField>> {
        Box::new(BinderTagWriteUoW::new(&self.context, &self.event_hub))
    }
}
...
pub struct BinderTagReadUoW {
    context: DbContext,
    transaction: RefCell<Option<Transaction>>,
}

impl BinderTagReadUoW {
...
impl QueryUnitOfWork for BinderTagReadUoW {
...
impl use_cases::ReadUoW for BinderTagReadUoW {
    type Entity = BinderTag;

    fn get(&self, id: &EntityId) -> Result<Option<BinderTag>> {
        let transaction = self.transaction.borrow();
        let repo = repository_factory::read::create_binder_tag_repository(
            transaction
                .as_ref()
                .ok_or_else(|| anyhow::anyhow!("No active transaction"))?,
        )?;
...
impl use_cases::ReadRelUoW<BinderTagRelationshipField> for BinderTagReadUoW {
    fn get_relationship(
        &self,
        id: &EntityId,
        field: &BinderTagRelationshipField,
...
pub struct BinderTagReadUoWFactory {
    context: DbContext,
}

impl BinderTagReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        BinderTagReadUoWFactory {
            context: db_context.clone(),
        }
    }
}

impl use_cases::ReadUoWFactory for BinderTagReadUoWFactory {
    type Entity = BinderTag;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = BinderTag>> {
        Box::new(BinderTagReadUoW::new(&self.context))
    }
}

impl use_cases::ReadRelUoWFactory<BinderTagRelationshipField> for BinderTagReadUoWFactory {
    fn create(&self) -> Box<dyn use_cases::ReadRelUoW<BinderTagRelationshipField>> {
        Box::new(BinderTagReadUoW::new(&self.context))
    }
}
