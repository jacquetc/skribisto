...
use super::{
    dtos::{CreateHolidayDto, HolidayDto, UpdateHolidayDto},
    units_of_work::{HolidayReadUoWFactory, HolidayWriteUoWFactory},
};
...
pub fn create_orphan(
    ...
    stack_id: Option<u64>,
    entity: &CreateHolidayDto,
) -> Result<HolidayDto> {
    let uow_factory = HolidayWriteUoWFactory::new(db_context, event_hub);
    let mut uc = use_cases::UndoableCreateOrphanUseCase::new(uow_factory);
    let entity_in: common::entities::Holiday = entity.into();
...
pub fn create_orphan_multi(
    ...
    undo_redo_manager: &mut UndoRedoManager,
    stack_id: Option<u64>,
    entities: &[CreateHolidayDto],
) -> Result<Vec<HolidayDto>> {
    let uow_factory = HolidayWriteUoWFactory::new(db_context, event_hub);
    let entities_in: Vec<common::entities::Holiday> =
...
pub fn get(db_context: &DbContext, id: &EntityId) -> Result<Option<HolidayDto>> {
    let uow_factory = HolidayReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute(id)?.map(|e| e.into()))
}

pub fn get_all(db_context: &DbContext) -> Result<Vec<HolidayDto>> {
    let uow_factory = HolidayReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute_all()?.into_iter().map(|e| e.into()).collect())
}

pub fn get_multi(db_context: &DbContext, ids: &[EntityId]) -> Result<Vec<Option<HolidayDto>>> {
    let uow_factory = HolidayReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc
        .execute_multi(ids)?
...
mod tests {
    ...
    fn create_one(ctx: &mut TestContext) -> HolidayDto {
        create_orphan(
            &ctx.db,
            &ctx.hub,
            &mut ctx.undo,
            None,
            &CreateHolidayDto::default(),
        )
        .unwrap()
    }
    ...
    fn test_update() {
        let mut ctx = TestContext::new();
        let created = create_one(&mut ctx);
        let update_dto: UpdateHolidayDto = created.into();
