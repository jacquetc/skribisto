...
use super::{
    dtos::{CreatePaceDto, PaceDto, UpdatePaceDto},
    units_of_work::{PaceReadUoWFactory, PaceWriteUoWFactory},
};
use crate::PaceRelationshipDto;
...
pub fn create_orphan(
    ...
    stack_id: Option<u64>,
    entity: &CreatePaceDto,
) -> Result<PaceDto> {
    let uow_factory = PaceWriteUoWFactory::new(db_context, event_hub);
    let mut uc = use_cases::UndoableCreateOrphanUseCase::new(uow_factory);
    let entity_in: common::entities::Pace = entity.into();
...
pub fn create_orphan_multi(
    ...
    undo_redo_manager: &mut UndoRedoManager,
    stack_id: Option<u64>,
    entities: &[CreatePaceDto],
) -> Result<Vec<PaceDto>> {
    let uow_factory = PaceWriteUoWFactory::new(db_context, event_hub);
    let entities_in: Vec<common::entities::Pace> = entities.iter().map(|dto| dto.into()).collect();
    let mut uc = use_cases::UndoableCreateOrphanUseCase::new(uow_factory);
...
pub fn get(db_context: &DbContext, id: &EntityId) -> Result<Option<PaceDto>> {
    let uow_factory = PaceReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute(id)?.map(|e| e.into()))
}

pub fn get_all(db_context: &DbContext) -> Result<Vec<PaceDto>> {
    let uow_factory = PaceReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute_all()?.into_iter().map(|e| e.into()).collect())
}

pub fn get_multi(db_context: &DbContext, ids: &[EntityId]) -> Result<Vec<Option<PaceDto>>> {
    let uow_factory = PaceReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc
        .execute_multi(ids)?
...
pub fn get_relationship(
    ...
    let uow_factory = PaceReadUoWFactory::new(db_context);
    let uc = use_cases::GetRelationshipUseCase::<PaceRelationshipField, _>::new(uow_factory);
    uc.execute(id, field)
}
...
pub fn get_relationship_many(
    ...
    let uow_factory = PaceReadUoWFactory::new(db_context);
    let uc = use_cases::GetRelationshipManyUseCase::<PaceRelationshipField, _>::new(uow_factory);
...
pub fn get_relationship_count(
    ...
    let uow_factory = PaceReadUoWFactory::new(db_context);
    let uc = use_cases::GetRelationshipCountUseCase::<PaceRelationshipField, _>::new(uow_factory);
    uc.execute(id, field)
}
...
pub fn get_relationship_in_range(
    ...
    let uow_factory = PaceReadUoWFactory::new(db_context);
    let uc = use_cases::GetRelationshipInRangeUseCase::<PaceRelationshipField, _>::new(uow_factory);
...
pub fn set_relationship(
    ...
    let uow_factory = PaceWriteUoWFactory::new(db_context, event_hub);
    let mut uc =
        use_cases::UndoableSetRelationshipUseCase::<PaceRelationshipField, _>::new(uow_factory);
...
pub fn move_relationship(
    ...
    let uow_factory = PaceWriteUoWFactory::new(db_context, event_hub);
    let mut uc =
        use_cases::UndoableMoveRelationshipUseCase::<PaceRelationshipField, _>::new(uow_factory);
...
mod tests {
    ...
    fn create_one(ctx: &mut TestContext) -> PaceDto {
        create_orphan(
            &ctx.db,
            &ctx.hub,
            &mut ctx.undo,
    ...
    fn test_update() {
        let mut ctx = TestContext::new();
        let created = create_one(&mut ctx);
        let update_dto: UpdatePaceDto = created.into();
    ...
    fn test_get_relationship_default() {
        ...
        let created = create_one(&mut ctx);
        let rel_ids =
            get_relationship(&ctx.db, &created.id, &PaceRelationshipField::BookItem).unwrap();
    ...
    fn test_get_relationship_count_default() {
        ...
        let created = create_one(&mut ctx);
        let count =
            get_relationship_count(&ctx.db, &created.id, &PaceRelationshipField::BookItem).unwrap();
        assert_eq!(count, 0);
    }
}
