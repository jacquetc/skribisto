...
impl CommandUnitOfWork for PaceWriteUoW {
    fn begin_transaction(&mut self) -> Result<()> {
        ...
        self.write_guard = Some(WriteTransactionGuard::acquire(&self.context, "pace_direct_write")?);
...
impl use_cases::WriteUoW for PaceWriteUoW {
    type Entity = Pace;

    fn get(&self, id: &EntityId) -> Result<Option<Pace>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_pace_repository(transaction)?;
        Ok(repo.get(id)?)
    }

    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Pace>>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_pace_repository(transaction)?;
        Ok(repo.get_multi(ids)?)
    }

    fn get_all(&self) -> Result<Vec<Pace>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_pace_repository(transaction)?;
        Ok(repo.get_all()?)
    }

    fn create_orphan_multi(&self, entities: &[Pace]) -> Result<Vec<Pace>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_pace_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
        Ok(repo.create_orphan_multi(&mut event_buffer, entities)?)
    }

    fn update_multi(&self, entities: &[Pace]) -> Result<Vec<Pace>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_pace_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
    ...
    fn update_with_relationships_multi(&self, entities: &[Pace]) -> Result<Vec<Pace>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_pace_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::OwnedWriteUoW for PaceWriteUoW {
    fn create_multi(&self, entities: &[Pace], owner_id: EntityId, index: i32) -> Result<Vec<Pace>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_pace_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::WriteRelUoW<PaceRelationshipField> for PaceWriteUoW {
    fn get_relationship(
        &self,
        id: &EntityId,
...
pub struct PaceWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl PaceWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        PaceWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
...
impl use_cases::WriteUoWFactory for PaceWriteUoWFactory {
    type Entity = Pace;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = Pace>> {
        Box::new(PaceWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::OwnedWriteUoWFactory for PaceWriteUoWFactory {
    type Entity = Pace;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = Pace>> {
        Box::new(PaceWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::WriteRelUoWFactory<PaceRelationshipField> for PaceWriteUoWFactory {
    fn create(&self) -> Box<dyn use_cases::WriteRelUoW<PaceRelationshipField>> {
        Box::new(PaceWriteUoW::new(&self.context, &self.event_hub))
    }
}
...
impl QueryUnitOfWork for PaceReadUoW {
...
impl use_cases::ReadUoW for PaceReadUoW {
    type Entity = Pace;

    fn get(&self, id: &EntityId) -> Result<Option<Pace>> {
        let transaction = self.transaction.borrow();
        let repo = repository_factory::read::create_pace_repository(
            transaction
                .as_ref()
                .ok_or_else(|| anyhow::anyhow!("No active transaction"))?,
        )?;
...
impl use_cases::ReadRelUoW<PaceRelationshipField> for PaceReadUoW {
    fn get_relationship(
        &self,
        id: &EntityId,
        field: &PaceRelationshipField,
...
pub struct PaceReadUoWFactory {
    context: DbContext,
}

impl PaceReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        PaceReadUoWFactory {
            context: db_context.clone(),
        }
...
impl use_cases::ReadUoWFactory for PaceReadUoWFactory {
    type Entity = Pace;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = Pace>> {
        Box::new(PaceReadUoW::new(&self.context))
    }
}

impl use_cases::ReadRelUoWFactory<PaceRelationshipField> for PaceReadUoWFactory {
    fn create(&self) -> Box<dyn use_cases::ReadRelUoW<PaceRelationshipField>> {
        Box::new(PaceReadUoW::new(&self.context))
    }
}
