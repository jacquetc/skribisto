...
impl CommandUnitOfWork for FootnoteWriteUoW {
...
impl use_cases::WriteUoW for FootnoteWriteUoW {
    type Entity = Footnote;

    fn get(&self, id: &EntityId) -> Result<Option<Footnote>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let repo = repository_factory::write::create_footnote_repository(transaction)?;
        Ok(repo.get(id)?)
    }

    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Footnote>>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let repo = repository_factory::write::create_footnote_repository(transaction)?;
        Ok(repo.get_multi(ids)?)
    }

    fn get_all(&self) -> Result<Vec<Footnote>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let repo = repository_factory::write::create_footnote_repository(transaction)?;
        Ok(repo.get_all()?)
    }

    fn create_orphan_multi(&self, entities: &[Footnote]) -> Result<Vec<Footnote>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let mut repo = repository_factory::write::create_footnote_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
        Ok(repo.create_orphan_multi(&mut event_buffer, entities)?)
    }

    fn update_multi(&self, entities: &[Footnote]) -> Result<Vec<Footnote>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let mut repo = repository_factory::write::create_footnote_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
    ...
    fn update_with_relationships_multi(&self, entities: &[Footnote]) -> Result<Vec<Footnote>> {
        let transaction = self.transaction.as_ref().ok_or_else(no_transaction)?;
        let mut repo = repository_factory::write::create_footnote_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::OwnedWriteUoW for FootnoteWriteUoW {
    fn create_multi(
        &self,
        entities: &[Footnote],
        owner_id: EntityId,
...
impl use_cases::WriteRelUoW<FootnoteRelationshipField> for FootnoteWriteUoW {
    fn get_relationship(
        &self,
        id: &EntityId,
...
pub struct FootnoteWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl FootnoteWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        FootnoteWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl use_cases::WriteUoWFactory for FootnoteWriteUoWFactory {
    type Entity = Footnote;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = Footnote>> {
        Box::new(FootnoteWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::OwnedWriteUoWFactory for FootnoteWriteUoWFactory {
    type Entity = Footnote;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = Footnote>> {
        Box::new(FootnoteWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::WriteRelUoWFactory<FootnoteRelationshipField> for FootnoteWriteUoWFactory {
    fn create(&self) -> Box<dyn use_cases::WriteRelUoW<FootnoteRelationshipField>> {
        Box::new(FootnoteWriteUoW::new(&self.context, &self.event_hub))
    }
}
...
pub struct FootnoteReadUoW {
    context: DbContext,
    transaction: RefCell<Option<Transaction>>,
}

impl FootnoteReadUoW {
...
impl QueryUnitOfWork for FootnoteReadUoW {
...
impl use_cases::ReadUoW for FootnoteReadUoW {
    type Entity = Footnote;

    fn get(&self, id: &EntityId) -> Result<Option<Footnote>> {
        let transaction = self.transaction.borrow();
        let repo = repository_factory::read::create_footnote_repository(
            transaction
                .as_ref()
                .ok_or_else(|| anyhow::anyhow!("No active transaction"))?,
        )?;
...
impl use_cases::ReadRelUoW<FootnoteRelationshipField> for FootnoteReadUoW {
    fn get_relationship(
        &self,
        id: &EntityId,
        field: &FootnoteRelationshipField,
...
pub struct FootnoteReadUoWFactory {
    context: DbContext,
}

impl FootnoteReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        FootnoteReadUoWFactory {
            context: db_context.clone(),
        }
    }
}

impl use_cases::ReadUoWFactory for FootnoteReadUoWFactory {
    type Entity = Footnote;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = Footnote>> {
        Box::new(FootnoteReadUoW::new(&self.context))
    }
}

impl use_cases::ReadRelUoWFactory<FootnoteRelationshipField> for FootnoteReadUoWFactory {
    fn create(&self) -> Box<dyn use_cases::ReadRelUoW<FootnoteRelationshipField>> {
        Box::new(FootnoteReadUoW::new(&self.context))
    }
}
