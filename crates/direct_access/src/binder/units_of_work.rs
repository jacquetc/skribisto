...
impl CommandUnitOfWork for BinderWriteUoW {
    fn begin_transaction(&mut self) -> Result<()> {
        ...
        self.write_guard = Some(WriteTransactionGuard::acquire(&self.context, "binder_direct_write")?);
...
impl use_cases::WriteUoW for BinderWriteUoW {
    type Entity = Binder;

    fn get(&self, id: &EntityId) -> Result<Option<Binder>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_binder_repository(transaction)?;
        Ok(repo.get(id)?)
    }

    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Binder>>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_binder_repository(transaction)?;
        Ok(repo.get_multi(ids)?)
    }

    fn get_all(&self) -> Result<Vec<Binder>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_binder_repository(transaction)?;
        Ok(repo.get_all()?)
    }

    fn create_orphan_multi(&self, entities: &[Binder]) -> Result<Vec<Binder>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_binder_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
        Ok(repo.create_orphan_multi(&mut event_buffer, entities)?)
    }

    fn update_multi(&self, entities: &[Binder]) -> Result<Vec<Binder>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_binder_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
    ...
    fn update_with_relationships_multi(&self, entities: &[Binder]) -> Result<Vec<Binder>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_binder_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::OwnedWriteUoW for BinderWriteUoW {
    fn create_multi(
        &self,
        entities: &[Binder],
        owner_id: EntityId,
...
impl use_cases::WriteRelUoW<BinderRelationshipField> for BinderWriteUoW {
    fn get_relationship(
        &self,
        id: &EntityId,
...
pub struct BinderWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl BinderWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        BinderWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl use_cases::WriteUoWFactory for BinderWriteUoWFactory {
    type Entity = Binder;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = Binder>> {
        Box::new(BinderWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::OwnedWriteUoWFactory for BinderWriteUoWFactory {
    type Entity = Binder;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = Binder>> {
        Box::new(BinderWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::WriteRelUoWFactory<BinderRelationshipField> for BinderWriteUoWFactory {
    fn create(&self) -> Box<dyn use_cases::WriteRelUoW<BinderRelationshipField>> {
        Box::new(BinderWriteUoW::new(&self.context, &self.event_hub))
    }
}
...
impl QueryUnitOfWork for BinderReadUoW {
...
impl use_cases::ReadUoW for BinderReadUoW {
    type Entity = Binder;

    fn get(&self, id: &EntityId) -> Result<Option<Binder>> {
        let transaction = self.transaction.borrow();
        let repo = repository_factory::read::create_binder_repository(
            transaction
                .as_ref()
                .ok_or_else(|| anyhow::anyhow!("No active transaction"))?,
        )?;
...
impl use_cases::ReadRelUoW<BinderRelationshipField> for BinderReadUoW {
    fn get_relationship(
        &self,
        id: &EntityId,
        field: &BinderRelationshipField,
...
pub struct BinderReadUoWFactory {
    context: DbContext,
}

impl BinderReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        BinderReadUoWFactory {
            context: db_context.clone(),
        }
    }
}

impl use_cases::ReadUoWFactory for BinderReadUoWFactory {
    type Entity = Binder;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = Binder>> {
        Box::new(BinderReadUoW::new(&self.context))
    }
}

impl use_cases::ReadRelUoWFactory<BinderRelationshipField> for BinderReadUoWFactory {
    fn create(&self) -> Box<dyn use_cases::ReadRelUoW<BinderRelationshipField>> {
        Box::new(BinderReadUoW::new(&self.context))
    }
}
