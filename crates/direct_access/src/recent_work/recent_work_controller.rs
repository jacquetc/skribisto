...
use super::{
    dtos::{CreateRecentWorkDto, RecentWorkDto, UpdateRecentWorkDto},
    units_of_work::{RecentWorkReadUoWFactory, RecentWorkWriteUoWFactory},
};
...
pub fn create_orphan(
    ...
    entity: &CreateRecentWorkDto,
) -> Result<RecentWorkDto> {
    let uow_factory = RecentWorkWriteUoWFactory::new(db_context, event_hub);
    let mut uc = use_cases::CreateOrphanUseCase::new(uow_factory);
    let entity_in: common::entities::RecentWork = entity.into();
...
pub fn create_orphan_multi(
    db_context: &DbContext,
    event_hub: &Arc<EventHub>,
    entities: &[CreateRecentWorkDto],
) -> Result<Vec<RecentWorkDto>> {
    let uow_factory = RecentWorkWriteUoWFactory::new(db_context, event_hub);
    let entities_in: Vec<common::entities::RecentWork> =
...
pub fn get(db_context: &DbContext, id: &EntityId) -> Result<Option<RecentWorkDto>> {
    let uow_factory = RecentWorkReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute(id)?.map(|e| e.into()))
}

pub fn get_all(db_context: &DbContext) -> Result<Vec<RecentWorkDto>> {
    let uow_factory = RecentWorkReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute_all()?.into_iter().map(|e| e.into()).collect())
}

pub fn get_multi(db_context: &DbContext, ids: &[EntityId]) -> Result<Vec<Option<RecentWorkDto>>> {
    let uow_factory = RecentWorkReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc
        .execute_multi(ids)?
...
mod tests {
    ...
    fn create_one(ctx: &mut TestContext) -> RecentWorkDto {
        create_orphan(&ctx.db, &ctx.hub, &CreateRecentWorkDto::default()).unwrap()
    }
    ...
    fn test_update() {
        let mut ctx = TestContext::new();
        let created = create_one(&mut ctx);
        let update_dto: UpdateRecentWorkDto = created.into();
