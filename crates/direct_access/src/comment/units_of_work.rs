...
impl CommandUnitOfWork for CommentWriteUoW {
...
impl use_cases::WriteUoW for CommentWriteUoW {
    type Entity = Comment;

    fn get(&self, id: &EntityId) -> Result<Option<Comment>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_comment_repository(transaction)?;
        Ok(repo.get(id)?)
    }

    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Comment>>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_comment_repository(transaction)?;
        Ok(repo.get_multi(ids)?)
    }

    fn get_all(&self) -> Result<Vec<Comment>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_comment_repository(transaction)?;
        Ok(repo.get_all()?)
    }

    fn create_orphan_multi(&self, entities: &[Comment]) -> Result<Vec<Comment>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_comment_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
        Ok(repo.create_orphan_multi(&mut event_buffer, entities)?)
    }

    fn update_multi(&self, entities: &[Comment]) -> Result<Vec<Comment>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_comment_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
    ...
    fn update_with_relationships_multi(&self, entities: &[Comment]) -> Result<Vec<Comment>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_comment_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::OwnedWriteUoW for CommentWriteUoW {
    fn create_multi(
        &self,
        entities: &[Comment],
        owner_id: EntityId,
...
impl use_cases::WriteRelUoW<CommentRelationshipField> for CommentWriteUoW {
    fn get_relationship(
        &self,
        id: &EntityId,
...
pub struct CommentWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl CommentWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        CommentWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl use_cases::WriteUoWFactory for CommentWriteUoWFactory {
    type Entity = Comment;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = Comment>> {
        Box::new(CommentWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::OwnedWriteUoWFactory for CommentWriteUoWFactory {
    type Entity = Comment;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = Comment>> {
        Box::new(CommentWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::WriteRelUoWFactory<CommentRelationshipField> for CommentWriteUoWFactory {
    fn create(&self) -> Box<dyn use_cases::WriteRelUoW<CommentRelationshipField>> {
        Box::new(CommentWriteUoW::new(&self.context, &self.event_hub))
    }
}
...
impl QueryUnitOfWork for CommentReadUoW {
...
impl use_cases::ReadUoW for CommentReadUoW {
    type Entity = Comment;

    fn get(&self, id: &EntityId) -> Result<Option<Comment>> {
        let transaction = self.transaction.borrow();
        let repo = repository_factory::read::create_comment_repository(
            transaction
                .as_ref()
                .ok_or_else(|| anyhow::anyhow!("No active transaction"))?,
        )?;
...
impl use_cases::ReadRelUoW<CommentRelationshipField> for CommentReadUoW {
    fn get_relationship(
        &self,
        id: &EntityId,
        field: &CommentRelationshipField,
...
pub struct CommentReadUoWFactory {
    context: DbContext,
}

impl CommentReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        CommentReadUoWFactory {
            context: db_context.clone(),
        }
    }
}

impl use_cases::ReadUoWFactory for CommentReadUoWFactory {
    type Entity = Comment;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = Comment>> {
        Box::new(CommentReadUoW::new(&self.context))
    }
}

impl use_cases::ReadRelUoWFactory<CommentRelationshipField> for CommentReadUoWFactory {
    fn create(&self) -> Box<dyn use_cases::ReadRelUoW<CommentRelationshipField>> {
        Box::new(CommentReadUoW::new(&self.context))
    }
}
