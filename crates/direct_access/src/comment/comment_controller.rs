...
use super::{
    dtos::{CommentDto, CreateCommentDto, UpdateCommentDto},
    units_of_work::{CommentReadUoWFactory, CommentWriteUoWFactory},
};
use crate::CommentRelationshipDto;
...
fn with_identity(mut e: common::entities::Comment) -> common::entities::Comment {
    if e.uid.is_nil() {
...
pub fn create_orphan(
    ...
    let entity_in: common::entities::Comment = with_identity(entity.into());
...
pub fn get(db_context: &DbContext, id: &EntityId) -> Result<Option<CommentDto>> {
    let uow_factory = CommentReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute(id)?.map(|e| e.into()))
}

pub fn get_all(db_context: &DbContext) -> Result<Vec<CommentDto>> {
    let uow_factory = CommentReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute_all()?.into_iter().map(|e| e.into()).collect())
}

pub fn get_multi(db_context: &DbContext, ids: &[EntityId]) -> Result<Vec<Option<CommentDto>>> {
    let uow_factory = CommentReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc
        .execute_multi(ids)?
...
pub fn update(
    ...
    undo_redo_manager: &mut UndoRedoManager,
    stack_id: Option<u64>,
    entity: &UpdateCommentDto,
) -> Result<CommentDto> {
    let uow_factory = CommentWriteUoWFactory::new(db_context, event_hub);
    let entity_in: common::entities::Comment = entity.into();
...
pub fn get_relationship(
    ...
    let uow_factory = CommentReadUoWFactory::new(db_context);
    let uc = use_cases::GetRelationshipUseCase::<CommentRelationshipField, _>::new(uow_factory);
    uc.execute(id, field)
}
...
pub fn get_relationship_many(
    ...
    let uow_factory = CommentReadUoWFactory::new(db_context);
    let uc = use_cases::GetRelationshipManyUseCase::<CommentRelationshipField, _>::new(uow_factory);
...
pub fn get_relationship_count(
    ...
    let uow_factory = CommentReadUoWFactory::new(db_context);
    let uc =
        use_cases::GetRelationshipCountUseCase::<CommentRelationshipField, _>::new(uow_factory);
    uc.execute(id, field)
}
...
pub fn get_relationship_in_range(
    ...
    let uow_factory = CommentReadUoWFactory::new(db_context);
    let uc =
        use_cases::GetRelationshipInRangeUseCase::<CommentRelationshipField, _>::new(uow_factory);
...
pub fn set_relationship(
    ...
    let uow_factory = CommentWriteUoWFactory::new(db_context, event_hub);
    let mut uc =
        use_cases::UndoableSetRelationshipUseCase::<CommentRelationshipField, _>::new(uow_factory);
...
pub fn move_relationship(
    ...
    let uow_factory = CommentWriteUoWFactory::new(db_context, event_hub);
    let mut uc =
        use_cases::UndoableMoveRelationshipUseCase::<CommentRelationshipField, _>::new(uow_factory);
...
mod tests {
    ...
    fn create_one(ctx: &mut TestContext) -> CommentDto {
        create_orphan(
            &ctx.db,
            &ctx.hub,
            &mut ctx.undo,
            None,
            &CreateCommentDto::default(),
        )
        .unwrap()
    }
    ...
    fn test_update() {
        let mut ctx = TestContext::new();
        let created = create_one(&mut ctx);
        let update_dto: UpdateCommentDto = created.into();
    ...
    fn test_get_relationship_default() {
        ...
        let created = create_one(&mut ctx);
        let rel_ids =
            get_relationship(&ctx.db, &created.id, &CommentRelationshipField::Content).unwrap();
    ...
    fn test_get_relationship_count_default() {
        ...
        let created = create_one(&mut ctx);
        let count =
            get_relationship_count(&ctx.db, &created.id, &CommentRelationshipField::Content)
                .unwrap();
        assert_eq!(count, 0);
    }
}
