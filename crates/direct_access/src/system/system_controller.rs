...
mod tests {
    ...
    fn test_get_relationship_default() {
        ...
        let created = create_one(&mut ctx);
        let rel_ids =
            get_relationship(&ctx.db, &created.id, &SystemRelationshipField::RecentWorks).unwrap();
    ...
    fn test_get_relationship_count_default() {
        ...
        let created = create_one(&mut ctx);
        let count =
            get_relationship_count(&ctx.db, &created.id, &SystemRelationshipField::RecentWorks)
                .unwrap();
        assert_eq!(count, 0);
    }
}
