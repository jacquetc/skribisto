...
mod tests {
    ...
    fn removing_a_works_only_window_runs_teardown_as_last_and_drops_the_session() {
        ...
        reg.register_window(BastydeWindowId::new(1), 1, None, stack_teardown, window_teardown);
    ...
    fn removing_one_of_two_windows_on_a_work_is_not_last_and_keeps_the_session() {
        ...
        reg.register_window(BastydeWindowId::new(1), 1, None, first_stack, inert_window_teardown());
        reg.register_window(BastydeWindowId::new(2), 1, None, second_stack, inert_window_teardown());
    ...
    fn removing_a_window_never_touches_a_sibling_works_session() {
        ...
        reg.register(1, a);
        reg.register(2, b);
        reg.register_window(BastydeWindowId::new(1), 1, None, Rc::new(|_| {}), inert_window_teardown());
        reg.register_window(BastydeWindowId::new(2), 2, None, Rc::new(|_| {}), inert_window_teardown());
    ...
    fn removing_a_window_twice_is_a_safe_no_op_the_second_time() {
        ...
        reg.register(1, fixture_session());
        let (window_teardown, window_calls) = tracked_window_teardown();
        reg.register_window(BastydeWindowId::new(1), 1, None, Rc::new(|_| {}), window_teardown);
    ...
    fn switching_a_windows_only_work_in_place_tears_down_the_old_session_and_stack() {
        ...
        reg.register_window(BastydeWindowId::new(1), 2, None, new_stack, inert_window_teardown());
    ...
    fn switching_in_place_away_from_a_work_with_a_sibling_window_is_not_last() {
        ...
        reg.register_window(BastydeWindowId::new(1), 1, None, window_a_stack, inert_window_teardown());
        reg.register_window(BastydeWindowId::new(2), 1, None, Rc::new(|_| {}), inert_window_teardown());

        // Window 1 switches to Work 2 in place; window 2 still shows Work 1.
        reg.register(2, fixture_session());
        reg.register_window(BastydeWindowId::new(1), 2, None, Rc::new(|_| {}), inert_window_teardown());
    ...
    fn reloading_the_same_work_in_place_does_not_prematurely_tear_it_down() {
        ...
        reg.register_window(BastydeWindowId::new(1), 1, None, second_stack, inert_window_teardown());
    ...
    fn ordinals_are_never_reused_once_a_lower_numbered_sibling_closes() {
        ...
        let third =
            reg.register_window(BastydeWindowId::new(3), 1, None, Rc::new(|_| {}), inert_window_teardown());
        assert_eq!(
            third, 3,
    ...
    fn a_reserved_ordinal_is_used_verbatim_and_consumes_no_second_number() {
        ...
        reg.attach(1).expect("the new window attaches to the open Work");
        let bound = reg.register_window(
            BastydeWindowId::new(2), 1, Some(reserved), Rc::new(|_| {}), inert_window_teardown());
    ...
    fn a_reserved_ordinal_never_overrides_a_window_already_on_that_work() {
        ...
        let again =
            reg.register_window(BastydeWindowId::new(1), 1, Some(9), Rc::new(|_| {}), inert_window_teardown());
        assert_eq!(again, first, "an established binding outranks a reservation");
    }

    #[test]
    fn reserving_for_an_unknown_work_is_a_safe_one() {
        let reg = WorkRegistry::new();
        assert_eq!(reg.reserve_window_ordinal(404), 1, "no window on it yet, so the next one is the first");
    }
    ...
    fn a_reservation_never_collides_with_a_live_window_whose_counter_lags() {
        ...
        reg.register_window(BastydeWindowId::new(1), 1, Some(1), Rc::new(|_| {}), inert_window_teardown());
    ...
    fn windows_for_lists_every_window_on_a_work_and_no_others() {
        ...
        reg.register_window(BastydeWindowId::new(1), 1, None, Rc::new(|_| {}), inert_window_teardown());
        reg.register_window(BastydeWindowId::new(2), 1, None, Rc::new(|_| {}), inert_window_teardown());
        reg.register_window(BastydeWindowId::new(3), 2, None, Rc::new(|_| {}), inert_window_teardown());
        ...
        assert_eq!(reg.windows_for(2), vec![BastydeWindowId::new(3)], "Work 2's window must not be swept in");
        assert!(reg.windows_for(404).is_empty(), "an unknown Work has no windows");
    }
    ...
    fn windows_for_is_ordered_by_ordinal_not_by_registration() {
        ...
        reg.register_window(BastydeWindowId::new(30), 1, Some(3), Rc::new(|_| {}), inert_window_teardown());
        reg.register_window(BastydeWindowId::new(20), 1, Some(2), Rc::new(|_| {}), inert_window_teardown());
        reg.register_window(BastydeWindowId::new(10), 1, Some(1), Rc::new(|_| {}), inert_window_teardown());
    ...
    fn open_work_ids_lists_a_multiply_attached_work_exactly_once() {
        ...
        assert_eq!(reg.window_count_for(1), 2, "…even though two windows hold it");
    }
}
