...
impl ExportViewModel {
    ...
    /// The one panel format for an index; a bound `SegmentedControl` drives `format_index`.
    pub fn format_at(index: usize) -> ExportFormat {
        PANEL_FORMATS[index.min(PANEL_FORMATS.len() - 1)].clone()
    }
    ...
    fn selected_format(&self) -> ExportFormat {
        Self::format_at(self.format_index.get())
    }
