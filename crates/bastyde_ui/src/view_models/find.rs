...
use bastyde::prelude::*;
use bastyde::text_document::{FindMatch, FindOptions, HighlightFormat, TextDocument};
use bastyde::widgets::rich_text::{EditorHandle, FindSession};
...
impl FindViewModel {
    ...
    fn replace_options(&self) -> bastyde::text_document::ReplaceOptions {
        bastyde::text_document::ReplaceOptions::new(self.options())
    }
    ...
    fn compute_replacement(&self, matched: &str) -> String {
        ...
        if self.preserve_case.get() {
            bastyde::text_document::matching::preserve_case(
                matched,
                &replacement,
                bastyde::text_document::matching::FoldLocale::Root,
            )
        } else {
            replacement
        }
