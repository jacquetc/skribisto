...
use bastyde::prelude::{Signal, WindowState};
...
impl DistractionFreeSurfaceViewModel {
    ...
    /// Wire the live single behind the strip's item name. Called from the
    /// surface's `build`, the one place with a `BuildContext`.
    pub fn wire(&self, ctx: &mut bastyde::prelude::BuildContext) {
        self.item.wire(ctx);
    }
