...
//!
//! **Left, the timeline.** A `bastyde_charts::BarChart` over the recorded
...
//! [`crate::view_models::timeline_axis::MAX_BARS`] the axis groups moments into
...
use bastyde::data::{ChartDatum, ChartModel, ChartSelection, SelectionMode, SeriesId};
use bastyde::prelude::*;
...
use bastyde_charts::{AxisConfig, BarChart};
...
use crate::view_models::versions::ProjectHandle;
use crate::view_models::{Axis, ChangeKind, RowChange, TimelineViewModel};
...
impl TimelinePanel {
    fn body(&self, axis: &Axis) -> Box<dyn Widget> {
        ...
        Box::new(
            HStack::new()
                ...
                .child(bastyde::widgets::Divider::vertical())
                .child(
                    Expand::horizontal()
                        .flex(1.0)
    ...
    ///
    /// `bastyde_charts::BarChart` rather than a row of rectangles, which is what
    ...
    fn change_list(&self) -> Box<dyn Widget> {
        ...
        let model = bastyde::data::ListModel::from_vec(rows);
...
mod tests {
    ...
    fn moments(n: usize, days: i64) -> Vec<crate::view_models::timeline::Moment> {
        ...
        (0..n)
            .map(|i| {
                ...
                crate::view_models::timeline::Moment {
                    at,
