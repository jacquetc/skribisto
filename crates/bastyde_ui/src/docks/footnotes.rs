...
use crate::view_models::{FootnoteFilter, FootnotesViewModel};
...
mod tests {
    ...
    fn the_dock_builds_and_lays_out() {
        ...
        tree.layout(bastyde::prelude::SizeProposal::exact(300.0, 700.0));
    ...
    fn the_filter_bar_fits_the_trailing_dock() {
        for width in [260.0_f32, 300.0] {
            ...
            tree.layout(bastyde::prelude::SizeProposal::exact(width, 200.0));
    ...
    fn a_row_builds_at_dock_width() {
        ...
        tree.layout(bastyde::prelude::SizeProposal::exact(300.0, 200.0));
        let b = tree.bounds(id);
        assert!(
            b.width > 0.0 && b.height > 0.0,
