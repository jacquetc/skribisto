...
struct DiffPane {
    ...
    handle: RefCell<Option<bastyde::widgets::rich_text::EditorHandle>>,
}
...
fn filter_row(vm: VersionsViewModel) -> impl Widget + use<> {
    ...
    let mut row = HStack::new().spacing(6.0).child(bati!(
        IconButton::new(crate::icons::versions::recent()) {
            size: IconButtonSize::Compact
            tooltip: tr!(versions_last_30_days())
            on_activate_fn: move |_| recent_vm.set_last_days(crate::docks::timeline::RECENT_DAYS)
        }
    ));
    if vm.is_filtered() {
...
fn list(view: &TimelineView, vm: VersionsViewModel) -> impl Widget + use<> {
    let model = bastyde::data::ListModel::from_vec(visible_rows(view, &vm));
    let pin_vm = vm.clone();
...
mod tests {
    use super::*;
    use crate::view_models::versions::{ProjectHandle, VersionScope};
