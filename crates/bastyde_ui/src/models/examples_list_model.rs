...
//! this has no real/mock split: the data (one entry today, *Starforgers*) is the
//! same in both builds. The `.skrib` bytes are **embedded** in the binary so an
//! example opens from any working directory and in a shipped build; the Welcome
//! view-model extracts them to a writable temp copy before loading (the repo
//! original is read-only). The public shape mirrors `RecentWorkListModel` so the
//! Welcome panel treats recents and examples uniformly.
...
/// Reactive-shaped list model over the static bundled examples.
#[derive(Clone)]
