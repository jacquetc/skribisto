...
//!
//! The public surface is a `bastyde::data::ListModel<TextReplacementRuleRow>` a
...
//! reads via `get_all_text_replacement_rule` (there is one Work per process) and
//! stays live on `TextReplacementRule` events + project switches, with writes
//! going through the generated `text_replacement_rule_commands`; the mock one
//! holds a fabricated set and mutates it in place, since `--features mocks` has
//! no real `Work` to own a created rule.
//!
...
mod imp {
    ...
    use frontend::commands::text_replacement_rule_commands;
    ...
    impl TextReplacementRuleListModel {
        ...
        /// mutation — from this pane or an undo/redo — re-reads the set, and a
        /// project switch replaces it wholesale.
    ...
    /// Read every `TextReplacementRule` (one Work per process) into sorted
    /// rows.
    fn load_rows(ctx: &AppContext) -> Vec<TextReplacementRuleRow> {
        let mut rows: Vec<TextReplacementRuleRow> =
            text_replacement_rule_commands::get_all_text_replacement_rule(ctx)
                .unwrap_or_default()
                .into_iter()
