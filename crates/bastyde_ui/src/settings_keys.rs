...
pub static SETTINGS: &[SettingSpec] = &[
    ...
    SettingSpec {
        key: bastyde::settings::TEXT_SCALE_KEY.key,
        ty: "float (1.0 = 100 %)",
        default: || val((bastyde::settings::TEXT_SCALE_KEY.default)()),
        check: check::<f32>,
        doc: "Interface text scale. Bastyde's own key, read before the first window opens.",
    },
...
mod tests {
    ...
    /// bastyde's own `accessibility.text_scale` (a `SettingsKey<f32>` in the framework).
    #[test]
    fn every_registered_key_still_exists() {
        ...
        let exempt = [
            ...
            bastyde::settings::TEXT_SCALE_KEY.key,
        ];
