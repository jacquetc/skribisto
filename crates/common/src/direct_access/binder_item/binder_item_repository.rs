...
impl<'a> BinderItemRepository<'a> {
    ...
    pub fn restore(
        ...
        let binder_item_ids: Vec<_> = store.binder_items.read().unwrap().keys().copied().collect();
        emit(
            DirectAccessEntity::BinderItem(EntityEvent::Created),
            binder_item_ids.clone(),
        );
        emit(
        ...
        {
            let child_ids: Vec<_> = store.contents.read().unwrap().keys().copied().collect();
            emit(
