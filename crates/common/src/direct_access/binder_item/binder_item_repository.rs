...
// Generated by Qleany v1.10.0 from common_entity_repository.tera
...
use crate::direct_access::binder::BinderRelationshipField;
use crate::error::RepositoryError;
use serde::{Deserialize, Serialize};

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub enum BinderItemRelationshipField {
    Books,
    Contents,
    PointOfView,
    References,
...
impl Display for BinderItemRelationshipField {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "{:?}", self)
    }
}

pub trait BinderItemTable {
    fn create(&mut self, entity: &BinderItem) -> Result<BinderItem, RepositoryError>;
    fn create_multi(&mut self, entities: &[BinderItem])
    ...
    fn update_multi(&mut self, entities: &[BinderItem])
    -> Result<Vec<BinderItem>, RepositoryError>;
...
pub struct BinderItemRepository<'a> {
    table: Box<dyn BinderItemTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> BinderItemRepository<'a> {
    pub fn new(table: Box<dyn BinderItemTable + 'a>, transaction: &'a Transaction) -> Self {
        BinderItemRepository { table, transaction }
    }
    ...
    pub fn create_orphan(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::BinderItem(EntityEvent::Created)),
            ids: vec![new.id],
            data: None,
        });
        Ok(new)
    ...
    pub fn get(&self, id: &EntityId) -> Result<Option<BinderItem>, RepositoryError> {
        self.table.get(id)
    }
    pub fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<BinderItem>>, RepositoryError> {
        self.table.get_multi(ids)
    }
    pub fn get_all(&self) -> Result<Vec<BinderItem>, RepositoryError> {
        self.table.get_all()
    }
    ...
    pub fn update(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::BinderItem(EntityEvent::Updated)),
            ids: vec![updated.id],
            data: None,
        });
        Ok(updated)
    ...
    pub fn remove(
        ...
        let contents = entity.contents.clone();
        ...
        repository_factory::write::create_content_repository(self.transaction)?
            .remove_multi(event_buffer, &contents)?;
        // Before removal, find which owner(s) reference this entity
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_binder_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&BinderRelationshipField::BinderItems, &[*id])?
                .into_iter()
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::BinderItem(EntityEvent::Removed)),
            ids: vec![*id],
            data: None,
        });
    ...
    pub fn remove_multi(
        ...
        let mut contents_ids: Vec<EntityId> = entities
            .iter()
            .flat_map(|entity| entity.as_ref().map(|entity| entity.contents.clone()))
            .flatten()
            .collect();
        // remove duplicates
        contents_ids.sort();
        ...
        repository_factory::write::create_content_repository(self.transaction)?
            .remove_multi(event_buffer, &contents_ids)?;
        // Before removal, find which owner(s) reference these entities
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_binder_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&BinderRelationshipField::BinderItems, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn get_relationships_from_owner(
        ...
        let repo = repository_factory::write::create_binder_repository(self.transaction)?;
        repo.get_relationship(owner_id, &BinderRelationshipField::BinderItems)
    }
    ...
    pub fn set_relationships_in_owner(
        ...
        let mut repo = repository_factory::write::create_binder_repository(self.transaction)?;
    ...
    pub fn restore(
        ...
        self.reconcile_backref_binder_binder_items(event_buffer, snap_store, &snap.root_ids)?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("binder_item", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.binder_items);
            for id in &ids {
                match (snap.binder_items.contains_key(id), live.contains_key(id)) {
        ...
        //    all locks before recursing so self-referential ownership cannot deadlock).
        {
            ...
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(list) = snap.jn_content_from_binder_item_contents.get(id) {
        ...
        {
            let mut live = write_or_recover(&store.binder_items);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.binder_items.get(id) {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_binder_item_from_binder_item_books);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_binder_item_from_binder_item_books.get(id) {
                    Some(v) => {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_content_from_binder_item_contents);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_content_from_binder_item_contents.get(id) {
                    Some(v) => {
                        live_jn.insert(*id, v.clone());
                    }
                    None => {
        ...
        {
            let mut live_jn =
                write_or_recover(&store.jn_binder_item_from_binder_item_point_of_view);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_binder_item_from_binder_item_point_of_view.get(id) {
                    Some(v) => {
                        live_jn.insert(*id, v.clone());
                    }
                    None => {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_binder_tag_from_binder_item_tags);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_binder_tag_from_binder_item_tags.get(id) {
                    Some(v) => {
                        live_jn.insert(*id, v.clone());
                    }
                    None => {
        ...
        self.reconcile_backref_binder_item_books(event_buffer, snap, &ids)?;
        self.reconcile_backref_binder_tag_creates_in(event_buffer, snap, &ids)?;
        self.reconcile_backref_binder_item_point_of_view(event_buffer, snap, &ids)?;
    ...
    /// `binder.binder_items`.
    fn reconcile_backref_binder_binder_items(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        for (left, rights) in snap.jn_binder_item_from_binder_binder_items.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
        {
            let live_jn = read_or_recover(&store.jn_binder_item_from_binder_binder_items);
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_binder_item_from_binder_binder_items)
                    .insert(left, reconciled);
                event_buffer.push(Event {
                    origin: Origin::DirectAccess(DirectAccessEntity::Binder(EntityEvent::Updated)),
                    ids: vec![left],
                    data: None,
                });
            }
    ...
    fn reconcile_backref_pace_book_item(
        ...
        {
            let live_jn = read_or_recover(&store.jn_binder_item_from_pace_book_item);
            for (left, rights) in live_jn.iter() {
                if rights.iter().any(|rid| scope.contains(rid)) {
                    left_keys.insert(*left);
                }
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_binder_item_from_pace_book_item)
                    .insert(left, reconciled);
                event_buffer.push(Event {
    ...
    /// Surgically reconcile one external referrer junction (out-of-scope key -> in-scope values):
    /// `binder_item.books`.
    fn reconcile_backref_binder_item_books(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        // External left keys whose ordered list references any scope id, in snapshot or live.
        let mut left_keys: std::collections::HashSet<EntityId> = std::collections::HashSet::new();
        for (left, rights) in snap.jn_binder_item_from_binder_item_books.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
        {
            let live_jn = read_or_recover(&store.jn_binder_item_from_binder_item_books);
            for (left, rights) in live_jn.iter() {
                if rights.iter().any(|rid| scope.contains(rid)) {
                    left_keys.insert(*left);
                }
        ...
        for left in left_keys {
            let snap_list: Vec<EntityId> = snap
                .jn_binder_item_from_binder_item_books
                .get(&left)
                .cloned()
                .unwrap_or_default();
            let new_list = {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_binder_item_from_binder_item_books)
                    .insert(left, reconciled);
                event_buffer.push(Event {
    ...
    /// Surgically reconcile one external referrer junction (out-of-scope key -> in-scope values):
    /// `binder_tag.creates_in`.
    fn reconcile_backref_binder_tag_creates_in(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        // External left keys whose ordered list references any scope id, in snapshot or live.
        let mut left_keys: std::collections::HashSet<EntityId> = std::collections::HashSet::new();
        for (left, rights) in snap.jn_binder_item_from_binder_tag_creates_in.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
        {
            let live_jn = read_or_recover(&store.jn_binder_item_from_binder_tag_creates_in);
            for (left, rights) in live_jn.iter() {
                if rights.iter().any(|rid| scope.contains(rid)) {
                    left_keys.insert(*left);
                }
        ...
        for left in left_keys {
            let snap_list: Vec<EntityId> = snap
                .jn_binder_item_from_binder_tag_creates_in
                .get(&left)
                .cloned()
                .unwrap_or_default();
            let new_list = {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_binder_item_from_binder_tag_creates_in)
                    .insert(left, reconciled);
                event_buffer.push(Event {
                    origin: Origin::DirectAccess(DirectAccessEntity::BinderTag(
                        EntityEvent::Updated,
                    )),
                    ids: vec![left],
                    data: None,
    ...
    /// Surgically reconcile one external referrer junction (out-of-scope key -> in-scope values):
    /// `binder_item.point_of_view`.
    fn reconcile_backref_binder_item_point_of_view(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        // External left keys whose ordered list references any scope id, in snapshot or live.
        let mut left_keys: std::collections::HashSet<EntityId> = std::collections::HashSet::new();
        for (left, rights) in snap.jn_binder_item_from_binder_item_point_of_view.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
        {
            let live_jn = read_or_recover(&store.jn_binder_item_from_binder_item_point_of_view);
            for (left, rights) in live_jn.iter() {
                if rights.iter().any(|rid| scope.contains(rid)) {
                    left_keys.insert(*left);
                }
        ...
        for left in left_keys {
            let snap_list: Vec<EntityId> = snap
                .jn_binder_item_from_binder_item_point_of_view
                .get(&left)
                .cloned()
                .unwrap_or_default();
            let new_list = {
    ...
    fn reconcile_backref_binder_item_references(
        ...
        // External left keys whose ordered list references any scope id, in snapshot or live.
        let mut left_keys: std::collections::HashSet<EntityId> = std::collections::HashSet::new();
        for (left, rights) in snap.jn_binder_item_from_binder_item_references.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
        {
            let live_jn = read_or_recover(&store.jn_binder_item_from_binder_item_references);
            for (left, rights) in live_jn.iter() {
                if rights.iter().any(|rid| scope.contains(rid)) {
                    left_keys.insert(*left);
                }
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_binder_item_from_binder_item_references)
                    .insert(left, reconciled);
                event_buffer.push(Event {
                    origin: Origin::DirectAccess(DirectAccessEntity::BinderItem(
                        EntityEvent::Updated,
    ...
    fn reconcile_backref_milestone_target_item(
        ...
        {
            let live_jn = read_or_recover(&store.jn_binder_item_from_milestone_target_item);
            for (left, rights) in live_jn.iter() {
                if rights.iter().any(|rid| scope.contains(rid)) {
                    left_keys.insert(*left);
                }
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_binder_item_from_milestone_target_item)
                    .insert(left, reconciled);
                event_buffer.push(Event {
    ...
    /// Surgically reconcile one external referrer junction (out-of-scope key -> in-scope values):
    /// `trash_info.trashed_binder_item`.
    fn reconcile_backref_trash_info_trashed_binder_item(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        {
            let live_jn =
                read_or_recover(&store.jn_binder_item_from_trash_info_trashed_binder_item);
            for (left, rights) in live_jn.iter() {
                if rights.iter().any(|rid| scope.contains(rid)) {
                    left_keys.insert(*left);
                }
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_binder_item_from_trash_info_trashed_binder_item)
                    .insert(left, reconciled);
                event_buffer.push(Event {
                    origin: Origin::DirectAccess(DirectAccessEntity::TrashInfo(
                        EntityEvent::Updated,
                    )),
                    ids: vec![left],
                    data: None,
...
pub struct BinderItemRepositoryRO<'a> {
    table: Box<dyn BinderItemTableRO + 'a>,
}
impl<'a> BinderItemRepositoryRO<'a> {
    pub fn new(table: Box<dyn BinderItemTableRO + 'a>) -> Self {
        BinderItemRepositoryRO { table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<BinderItem>, RepositoryError> {
        self.table.get(id)
    }
