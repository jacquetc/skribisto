...
pub enum TextReplacementRuleRelationshipField {}

impl Display for TextReplacementRuleRelationshipField {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "{:?}", self)
    }
}

pub trait TextReplacementRuleTable {
    fn create(
        &mut self,
        entity: &TextReplacementRule,
    ) -> Result<TextReplacementRule, RepositoryError>;
    fn create_multi(
        &mut self,
        entities: &[TextReplacementRule],
    ) -> Result<Vec<TextReplacementRule>, RepositoryError>;
    fn get(&self, id: &EntityId) -> Result<Option<TextReplacementRule>, RepositoryError>;
    fn get_multi(
        &self,
        ids: &[EntityId],
    ) -> Result<Vec<Option<TextReplacementRule>>, RepositoryError>;
    fn get_all(&self) -> Result<Vec<TextReplacementRule>, RepositoryError>;
    fn update(
        &mut self,
        entity: &TextReplacementRule,
    ) -> Result<TextReplacementRule, RepositoryError>;
...
pub trait TextReplacementRuleTableRO {
    fn get(&self, id: &EntityId) -> Result<Option<TextReplacementRule>, RepositoryError>;
    fn get_multi(
        &self,
        ids: &[EntityId],
...
pub struct TextReplacementRuleRepository<'a> {
    table: Box<dyn TextReplacementRuleTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> TextReplacementRuleRepository<'a> {
    pub fn new(
        table: Box<dyn TextReplacementRuleTable + 'a>,
        transaction: &'a Transaction,
    ) -> Self {
        TextReplacementRuleRepository { table, transaction }
    }
    ...
    pub fn create_orphan(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::TextReplacementRule(
                EntityEvent::Created,
            )),
            ids: vec![new.id],
            data: None,
    ...
    pub fn get(&self, id: &EntityId) -> Result<Option<TextReplacementRule>, RepositoryError> {
        self.table.get(id)
    }
    pub fn get_multi(
        &self,
    ...
    pub fn get_all(&self) -> Result<Vec<TextReplacementRule>, RepositoryError> {
        self.table.get_all()
    }
    ...
    pub fn get_relationships_from_owner(
        ...
        repo.get_relationship(owner_id, &WorkRelationshipField::TextReplacementRules)
    }
    ...
    pub fn restore(
        ...
        self.reconcile_backref_work_text_replacement_rules(
            event_buffer,
            snap_store,
            &snap.root_ids,
        )?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("text_replacement_rule", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.text_replacement_rules);
            for id in &ids {
                match (
                    snap.text_replacement_rules.contains_key(id),
        ...
        {
            let mut live = write_or_recover(&store.text_replacement_rules);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.text_replacement_rules.get(id) {
    ...
    fn reconcile_backref_work_text_replacement_rules(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        {
            let live_jn =
                read_or_recover(&store.jn_text_replacement_rule_from_work_text_replacement_rules);
...
pub struct TextReplacementRuleRepositoryRO<'a> {
    table: Box<dyn TextReplacementRuleTableRO + 'a>,
}
impl<'a> TextReplacementRuleRepositoryRO<'a> {
    pub fn new(table: Box<dyn TextReplacementRuleTableRO + 'a>) -> Self {
        TextReplacementRuleRepositoryRO { table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<TextReplacementRule>, RepositoryError> {
        self.table.get(id)
    }
