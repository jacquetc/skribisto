...
impl<'a> WorkRepository<'a> {
    ...
    pub(crate) fn restore_subtree(
        ...
        }
        {
            ...
            {
                let live_jn = read_or_recover(&store.jn_asset_from_work_assets);
                for id in &ids {
                    if let Some(list) = live_jn.get(id) {
                        child_ids.extend(list.iter().copied());
                    }
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_asset_from_work_assets);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_asset_from_work_assets.get(id) {
                    Some(v) => {
