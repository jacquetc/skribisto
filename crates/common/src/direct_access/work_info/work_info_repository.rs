...
impl<'a> WorkInfoRepository<'a> {
    ...
    pub(crate) fn restore_subtree(
        ...
        {
            let live = read_or_recover(&store.work_infos);
            for id in &ids {
        ...
        //    all locks before recursing so self-referential ownership cannot deadlock).
        {
            ...
            {
                let live_jn = read_or_recover(&store.jn_search_from_work_info_search);
                for id in &ids {
        ...
        {
            let mut live = write_or_recover(&store.work_infos);
            for id in to_create.iter().chain(to_update.iter()) {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_search_from_work_info_search);
            for id in to_create.iter().chain(to_update.iter()) {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_work_from_work_info_work);
            for id in to_create.iter().chain(to_update.iter()) {
    ...
    fn reconcile_backref_system_work_infos(
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_work_info_from_system_work_infos)
