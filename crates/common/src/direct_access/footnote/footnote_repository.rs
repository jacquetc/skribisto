...
pub enum FootnoteRelationshipField {
    Content,
}

impl Display for FootnoteRelationshipField {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "{:?}", self)
    }
}

pub trait FootnoteTable {
    ...
    fn update_with_relationships(&mut self, entity: &Footnote)
...
pub struct FootnoteRepository<'a> {
    table: Box<dyn FootnoteTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> FootnoteRepository<'a> {
    pub fn new(table: Box<dyn FootnoteTable + 'a>, transaction: &'a Transaction) -> Self {
        FootnoteRepository { table, transaction }
    }
    ...
    pub fn create_orphan(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Footnote(EntityEvent::Created)),
            ids: vec![new.id],
            data: None,
        });
        Ok(new)
    ...
    pub fn get(&self, id: &EntityId) -> Result<Option<Footnote>, RepositoryError> {
        self.table.get(id)
    }
    pub fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Footnote>>, RepositoryError> {
        self.table.get_multi(ids)
    }
    pub fn get_all(&self) -> Result<Vec<Footnote>, RepositoryError> {
        self.table.get_all()
    }
    ...
    pub fn update(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Footnote(EntityEvent::Updated)),
            ids: vec![updated.id],
            data: None,
        });
        Ok(updated)
    ...
    pub fn remove(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            ...
            owner_repo
                .get_relationships_from_right_ids(&WorkRelationshipField::Footnotes, &[*id])?
                .into_iter()
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Footnote(EntityEvent::Removed)),
            ids: vec![*id],
            data: None,
        });
    ...
    pub fn remove_multi(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_work_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&WorkRelationshipField::Footnotes, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn get_relationships_from_owner(
        ...
        repo.get_relationship(owner_id, &WorkRelationshipField::Footnotes)
    }
    ...
    pub fn restore(
        ...
        self.reconcile_backref_work_footnotes(event_buffer, snap_store, &snap.root_ids)?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("footnote", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.footnotes);
            for id in &ids {
                match (snap.footnotes.contains_key(id), live.contains_key(id)) {
        ...
        {
            let mut live = write_or_recover(&store.footnotes);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.footnotes.get(id) {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_content_from_footnote_content);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_content_from_footnote_content.get(id) {
                    Some(v) => {
    ...
    /// `work.footnotes`.
    fn reconcile_backref_work_footnotes(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        for (left, rights) in snap.jn_footnote_from_work_footnotes.iter() {
        ...
        for left in left_keys {
            let snap_list: Vec<EntityId> = snap
                .jn_footnote_from_work_footnotes
                .get(&left)
                .cloned()
                .unwrap_or_default();
            let new_list = {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_footnote_from_work_footnotes).insert(left, reconciled);
                event_buffer.push(Event {
...
pub struct FootnoteRepositoryRO<'a> {
    table: Box<dyn FootnoteTableRO + 'a>,
}
impl<'a> FootnoteRepositoryRO<'a> {
    pub fn new(table: Box<dyn FootnoteTableRO + 'a>) -> Self {
        FootnoteRepositoryRO { table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<Footnote>, RepositoryError> {
        self.table.get(id)
    }
