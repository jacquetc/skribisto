...
impl<'a> SystemRepository<'a> {
    ...
    pub(crate) fn restore_subtree(
        ...
        }
        {
            ...
            {
                let live_jn = read_or_recover(&store.jn_work_info_from_system_work_infos);
                for id in &ids {
                    if let Some(list) = live_jn.get(id) {
                        child_ids.extend(list.iter().copied());
                    }
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_recent_work_from_system_recent_works);
            for id in to_create.iter().chain(to_update.iter()) {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_work_info_from_system_work_infos);
            for id in to_create.iter().chain(to_update.iter()) {
