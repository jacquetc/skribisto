...
pub enum MilestoneRelationshipField {
    TargetItem,
}

impl Display for MilestoneRelationshipField {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "{:?}", self)
    }
}

pub trait MilestoneTable {
...
pub struct MilestoneRepository<'a> {
    table: Box<dyn MilestoneTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> MilestoneRepository<'a> {
    pub fn new(table: Box<dyn MilestoneTable + 'a>, transaction: &'a Transaction) -> Self {
        MilestoneRepository { table, transaction }
    }
    ...
    pub fn create_orphan(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Milestone(EntityEvent::Created)),
            ids: vec![new.id],
            data: None,
        });
        Ok(new)
    ...
    pub fn get(&self, id: &EntityId) -> Result<Option<Milestone>, RepositoryError> {
        self.table.get(id)
    }
    pub fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Milestone>>, RepositoryError> {
        self.table.get_multi(ids)
    }
    pub fn get_all(&self) -> Result<Vec<Milestone>, RepositoryError> {
        self.table.get_all()
    }
    ...
    pub fn update(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Milestone(EntityEvent::Updated)),
            ids: vec![updated.id],
            data: None,
        });
        Ok(updated)
    ...
    pub fn remove(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            ...
            owner_repo
                .get_relationships_from_right_ids(&PaceRelationshipField::Milestones, &[*id])?
                .into_iter()
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Milestone(EntityEvent::Removed)),
            ids: vec![*id],
            data: None,
        });
    ...
    pub fn remove_multi(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_pace_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&PaceRelationshipField::Milestones, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn get_relationships_from_owner(
        ...
        repo.get_relationship(owner_id, &PaceRelationshipField::Milestones)
    }
    ...
    pub fn restore(
        ...
        self.reconcile_backref_pace_milestones(event_buffer, snap_store, &snap.root_ids)?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("milestone", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.milestones);
            for id in &ids {
                match (snap.milestones.contains_key(id), live.contains_key(id)) {
        ...
        {
            let mut live = write_or_recover(&store.milestones);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.milestones.get(id) {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_binder_item_from_milestone_target_item);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_binder_item_from_milestone_target_item.get(id) {
                    Some(v) => {
    ...
    /// `pace.milestones`.
    fn reconcile_backref_pace_milestones(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        for (left, rights) in snap.jn_milestone_from_pace_milestones.iter() {
        ...
        for left in left_keys {
            let snap_list: Vec<EntityId> = snap
                .jn_milestone_from_pace_milestones
                .get(&left)
                .cloned()
                .unwrap_or_default();
            let new_list = {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_milestone_from_pace_milestones).insert(left, reconciled);
                event_buffer.push(Event {
...
pub struct MilestoneRepositoryRO<'a> {
    table: Box<dyn MilestoneTableRO + 'a>,
}
impl<'a> MilestoneRepositoryRO<'a> {
    pub fn new(table: Box<dyn MilestoneTableRO + 'a>) -> Self {
        MilestoneRepositoryRO { table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<Milestone>, RepositoryError> {
        self.table.get(id)
    }
