...
pub enum BinderStatusRelationshipField {}

impl Display for BinderStatusRelationshipField {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "{:?}", self)
    }
}

pub trait BinderStatusTable {
    fn create(&mut self, entity: &BinderStatus) -> Result<BinderStatus, RepositoryError>;
    fn create_multi(
        &mut self,
        entities: &[BinderStatus],
    ) -> Result<Vec<BinderStatus>, RepositoryError>;
    fn get(&self, id: &EntityId) -> Result<Option<BinderStatus>, RepositoryError>;
    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<BinderStatus>>, RepositoryError>;
    fn get_all(&self) -> Result<Vec<BinderStatus>, RepositoryError>;
    fn update(&mut self, entity: &BinderStatus) -> Result<BinderStatus, RepositoryError>;
    fn update_multi(
        &mut self,
        entities: &[BinderStatus],
    ) -> Result<Vec<BinderStatus>, RepositoryError>;
    fn update_with_relationships(
        &mut self,
        entity: &BinderStatus,
    ) -> Result<BinderStatus, RepositoryError>;
...
pub trait BinderStatusTableRO {
    fn get(&self, id: &EntityId) -> Result<Option<BinderStatus>, RepositoryError>;
    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<BinderStatus>>, RepositoryError>;
    fn get_all(&self) -> Result<Vec<BinderStatus>, RepositoryError>;
}

pub struct BinderStatusRepository<'a> {
    table: Box<dyn BinderStatusTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> BinderStatusRepository<'a> {
    pub fn new(table: Box<dyn BinderStatusTable + 'a>, transaction: &'a Transaction) -> Self {
        BinderStatusRepository { table, transaction }
    }
    ...
    pub fn create_orphan(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::BinderStatus(EntityEvent::Created)),
            ids: vec![new.id],
            data: None,
        });
        Ok(new)
    ...
    pub fn get(&self, id: &EntityId) -> Result<Option<BinderStatus>, RepositoryError> {
        self.table.get(id)
    }
    pub fn get_multi(
        &self,
        ids: &[EntityId],
    ) -> Result<Vec<Option<BinderStatus>>, RepositoryError> {
        self.table.get_multi(ids)
    }
    pub fn get_all(&self) -> Result<Vec<BinderStatus>, RepositoryError> {
        self.table.get_all()
    }
    ...
    pub fn update(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::BinderStatus(EntityEvent::Updated)),
            ids: vec![updated.id],
            data: None,
        });
        Ok(updated)
    ...
    pub fn remove(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            ...
            owner_repo
                .get_relationships_from_right_ids(&WorkRelationshipField::Statuses, &[*id])?
                .into_iter()
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::BinderStatus(EntityEvent::Removed)),
            ids: vec![*id],
            data: None,
        });
    ...
    pub fn remove_multi(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_work_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&WorkRelationshipField::Statuses, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn get_relationships_from_owner(
        ...
        repo.get_relationship(owner_id, &WorkRelationshipField::Statuses)
    }
    ...
    pub fn restore(
        ...
        self.reconcile_backref_work_statuses(event_buffer, snap_store, &snap.root_ids)?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("binder_status", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.binder_statuss);
            for id in &ids {
                match (snap.binder_statuss.contains_key(id), live.contains_key(id)) {
        ...
        {
            let mut live = write_or_recover(&store.binder_statuss);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.binder_statuss.get(id) {
        ...
        self.reconcile_backref_binder_item_status(event_buffer, snap, &ids)?;

        // 5. Events: precise per-id (Created/Updated/Removed), not a whole-store storm.
        if !to_create.is_empty() {
            event_buffer.push(Event {
                origin: Origin::DirectAccess(DirectAccessEntity::BinderStatus(
                    EntityEvent::Created,
                )),
                ids: to_create.clone(),
                data: None,
    ...
    /// `binder_item.status`.
    fn reconcile_backref_binder_item_status(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        for (left, rights) in snap.jn_binder_status_from_binder_item_status.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
        {
            let live_jn = read_or_recover(&store.jn_binder_status_from_binder_item_status);
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_binder_status_from_binder_item_status)
    ...
    /// Surgically reconcile one external referrer junction (out-of-scope key -> in-scope values):
    /// `work.statuses`.
    fn reconcile_backref_work_statuses(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        // External left keys whose ordered list references any scope id, in snapshot or live.
        let mut left_keys: std::collections::HashSet<EntityId> = std::collections::HashSet::new();
        for (left, rights) in snap.jn_binder_status_from_work_statuses.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_binder_status_from_work_statuses)
                    .insert(left, reconciled);
                event_buffer.push(Event {
...
pub struct BinderStatusRepositoryRO<'a> {
    table: Box<dyn BinderStatusTableRO + 'a>,
}
impl<'a> BinderStatusRepositoryRO<'a> {
    pub fn new(table: Box<dyn BinderStatusTableRO + 'a>) -> Self {
        BinderStatusRepositoryRO { table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<BinderStatus>, RepositoryError> {
        self.table.get(id)
    }
