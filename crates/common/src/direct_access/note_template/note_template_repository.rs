...
pub enum NoteTemplateRelationshipField {}

impl Display for NoteTemplateRelationshipField {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "{:?}", self)
    }
}

pub trait NoteTemplateTable {
    fn create(&mut self, entity: &NoteTemplate) -> Result<NoteTemplate, RepositoryError>;
    fn create_multi(
        &mut self,
        entities: &[NoteTemplate],
    ) -> Result<Vec<NoteTemplate>, RepositoryError>;
    fn get(&self, id: &EntityId) -> Result<Option<NoteTemplate>, RepositoryError>;
    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<NoteTemplate>>, RepositoryError>;
    fn get_all(&self) -> Result<Vec<NoteTemplate>, RepositoryError>;
    fn update(&mut self, entity: &NoteTemplate) -> Result<NoteTemplate, RepositoryError>;
    fn update_multi(
        &mut self,
        entities: &[NoteTemplate],
    ) -> Result<Vec<NoteTemplate>, RepositoryError>;
    fn update_with_relationships(
        &mut self,
        entity: &NoteTemplate,
    ) -> Result<NoteTemplate, RepositoryError>;
...
pub trait NoteTemplateTableRO {
    fn get(&self, id: &EntityId) -> Result<Option<NoteTemplate>, RepositoryError>;
    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<NoteTemplate>>, RepositoryError>;
    fn get_all(&self) -> Result<Vec<NoteTemplate>, RepositoryError>;
}

pub struct NoteTemplateRepository<'a> {
    table: Box<dyn NoteTemplateTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> NoteTemplateRepository<'a> {
    pub fn new(table: Box<dyn NoteTemplateTable + 'a>, transaction: &'a Transaction) -> Self {
        NoteTemplateRepository { table, transaction }
    }
    ...
    pub fn create_orphan(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::NoteTemplate(EntityEvent::Created)),
            ids: vec![new.id],
            data: None,
        });
        Ok(new)
    ...
    pub fn get(&self, id: &EntityId) -> Result<Option<NoteTemplate>, RepositoryError> {
        self.table.get(id)
    }
    pub fn get_multi(
        &self,
        ids: &[EntityId],
    ) -> Result<Vec<Option<NoteTemplate>>, RepositoryError> {
        self.table.get_multi(ids)
    }
    pub fn get_all(&self) -> Result<Vec<NoteTemplate>, RepositoryError> {
        self.table.get_all()
    }
    ...
    pub fn update(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::NoteTemplate(EntityEvent::Updated)),
            ids: vec![updated.id],
            data: None,
        });
        Ok(updated)
    ...
    pub fn remove(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            ...
            owner_repo
                .get_relationships_from_right_ids(&WorkRelationshipField::NoteTemplates, &[*id])?
                .into_iter()
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::NoteTemplate(EntityEvent::Removed)),
            ids: vec![*id],
            data: None,
        });
    ...
    pub fn remove_multi(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_work_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&WorkRelationshipField::NoteTemplates, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn get_relationships_from_owner(
        ...
        repo.get_relationship(owner_id, &WorkRelationshipField::NoteTemplates)
    }
    ...
    pub fn restore(
        ...
        self.reconcile_backref_work_note_templates(event_buffer, snap_store, &snap.root_ids)?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("note_template", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.note_templates);
            for id in &ids {
                match (snap.note_templates.contains_key(id), live.contains_key(id)) {
        ...
        {
            let mut live = write_or_recover(&store.note_templates);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.note_templates.get(id) {
        ...
        self.reconcile_backref_binder_tag_note_template(event_buffer, snap, &ids)?;

        // 5. Events: precise per-id (Created/Updated/Removed), not a whole-store storm.
        if !to_create.is_empty() {
            event_buffer.push(Event {
                origin: Origin::DirectAccess(DirectAccessEntity::NoteTemplate(
                    EntityEvent::Created,
                )),
                ids: to_create.clone(),
                data: None,
    ...
    /// `binder_tag.note_template`.
    fn reconcile_backref_binder_tag_note_template(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        for (left, rights) in snap.jn_note_template_from_binder_tag_note_template.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
        {
            let live_jn = read_or_recover(&store.jn_note_template_from_binder_tag_note_template);
        ...
        for left in left_keys {
            ...
            let new_list = {
                let live_jn =
                    read_or_recover(&store.jn_note_template_from_binder_tag_note_template);
    ...
    /// Surgically reconcile one external referrer junction (out-of-scope key -> in-scope values):
    /// `work.note_templates`.
    fn reconcile_backref_work_note_templates(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        // External left keys whose ordered list references any scope id, in snapshot or live.
        let mut left_keys: std::collections::HashSet<EntityId> = std::collections::HashSet::new();
        for (left, rights) in snap.jn_note_template_from_work_note_templates.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_note_template_from_work_note_templates)
                    .insert(left, reconciled);
                event_buffer.push(Event {
...
pub struct NoteTemplateRepositoryRO<'a> {
    table: Box<dyn NoteTemplateTableRO + 'a>,
}
impl<'a> NoteTemplateRepositoryRO<'a> {
    pub fn new(table: Box<dyn NoteTemplateTableRO + 'a>) -> Self {
        NoteTemplateRepositoryRO { table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<NoteTemplate>, RepositoryError> {
        self.table.get(id)
    }
