...
pub enum ProgressSnapshotRelationshipField {}

impl Display for ProgressSnapshotRelationshipField {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "{:?}", self)
    }
}

pub trait ProgressSnapshotTable {
    fn create(&mut self, entity: &ProgressSnapshot) -> Result<ProgressSnapshot, RepositoryError>;
    fn create_multi(
        &mut self,
        entities: &[ProgressSnapshot],
    ) -> Result<Vec<ProgressSnapshot>, RepositoryError>;
    fn get(&self, id: &EntityId) -> Result<Option<ProgressSnapshot>, RepositoryError>;
    fn get_multi(&self, ids: &[EntityId])
    -> Result<Vec<Option<ProgressSnapshot>>, RepositoryError>;
    fn get_all(&self) -> Result<Vec<ProgressSnapshot>, RepositoryError>;
    fn update(&mut self, entity: &ProgressSnapshot) -> Result<ProgressSnapshot, RepositoryError>;
    fn update_multi(
        &mut self,
        entities: &[ProgressSnapshot],
    ) -> Result<Vec<ProgressSnapshot>, RepositoryError>;
...
pub trait ProgressSnapshotTableRO {
    fn get(&self, id: &EntityId) -> Result<Option<ProgressSnapshot>, RepositoryError>;
    fn get_multi(&self, ids: &[EntityId])
    -> Result<Vec<Option<ProgressSnapshot>>, RepositoryError>;
    fn get_all(&self) -> Result<Vec<ProgressSnapshot>, RepositoryError>;
...
pub struct ProgressSnapshotRepository<'a> {
    table: Box<dyn ProgressSnapshotTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> ProgressSnapshotRepository<'a> {
    pub fn new(table: Box<dyn ProgressSnapshotTable + 'a>, transaction: &'a Transaction) -> Self {
        ProgressSnapshotRepository { table, transaction }
    }
    ...
    pub fn create_orphan(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::ProgressSnapshot(
                EntityEvent::Created,
            )),
            ids: vec![new.id],
            data: None,
    ...
    pub fn get(&self, id: &EntityId) -> Result<Option<ProgressSnapshot>, RepositoryError> {
        self.table.get(id)
    }
    pub fn get_multi(
        &self,
    ...
    pub fn get_all(&self) -> Result<Vec<ProgressSnapshot>, RepositoryError> {
        self.table.get_all()
    }
    ...
    pub fn get_relationships_from_owner(
        ...
        repo.get_relationship(owner_id, &WorkInfoRelationshipField::ProgressSnapshots)
    }
    ...
    pub fn restore(
        ...
        self.reconcile_backref_work_info_progress_snapshots(
            event_buffer,
            snap_store,
            &snap.root_ids,
        )?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("progress_snapshot", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.progress_snapshots);
            for id in &ids {
                match (
                    snap.progress_snapshots.contains_key(id),
        ...
        {
            let mut live = write_or_recover(&store.progress_snapshots);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.progress_snapshots.get(id) {
    ...
    /// `work_info.progress_snapshots`.
    fn reconcile_backref_work_info_progress_snapshots(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        {
            let live_jn =
                read_or_recover(&store.jn_progress_snapshot_from_work_info_progress_snapshots);
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_progress_snapshot_from_work_info_progress_snapshots)
...
pub struct ProgressSnapshotRepositoryRO<'a> {
    table: Box<dyn ProgressSnapshotTableRO + 'a>,
}
impl<'a> ProgressSnapshotRepositoryRO<'a> {
    pub fn new(table: Box<dyn ProgressSnapshotTableRO + 'a>) -> Self {
        ProgressSnapshotRepositoryRO { table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<ProgressSnapshot>, RepositoryError> {
        self.table.get(id)
    }
