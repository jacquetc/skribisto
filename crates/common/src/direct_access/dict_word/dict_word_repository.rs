...
pub enum DictWordRelationshipField {}

impl Display for DictWordRelationshipField {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "{:?}", self)
    }
}

pub trait DictWordTable {
    fn create(&mut self, entity: &DictWord) -> Result<DictWord, RepositoryError>;
    fn create_multi(&mut self, entities: &[DictWord]) -> Result<Vec<DictWord>, RepositoryError>;
    fn get(&self, id: &EntityId) -> Result<Option<DictWord>, RepositoryError>;
    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<DictWord>>, RepositoryError>;
    fn get_all(&self) -> Result<Vec<DictWord>, RepositoryError>;
    fn update(&mut self, entity: &DictWord) -> Result<DictWord, RepositoryError>;
    fn update_multi(&mut self, entities: &[DictWord]) -> Result<Vec<DictWord>, RepositoryError>;
    fn update_with_relationships(&mut self, entity: &DictWord)
    -> Result<DictWord, RepositoryError>;
    fn update_with_relationships_multi(
        &mut self,
        entities: &[DictWord],
    ) -> Result<Vec<DictWord>, RepositoryError>;
    fn remove(&mut self, id: &EntityId) -> Result<(), RepositoryError>;
    fn remove_multi(&mut self, ids: &[EntityId]) -> Result<(), RepositoryError>;
}

pub trait DictWordTableRO {
    fn get(&self, id: &EntityId) -> Result<Option<DictWord>, RepositoryError>;
    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<DictWord>>, RepositoryError>;
    fn get_all(&self) -> Result<Vec<DictWord>, RepositoryError>;
}

pub struct DictWordRepository<'a> {
    table: Box<dyn DictWordTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> DictWordRepository<'a> {
    pub fn new(table: Box<dyn DictWordTable + 'a>, transaction: &'a Transaction) -> Self {
        DictWordRepository { table, transaction }
    }
    ...
    pub fn create_orphan(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::DictWord(EntityEvent::Created)),
            ids: vec![new.id],
            data: None,
        });
        Ok(new)
    ...
    pub fn get(&self, id: &EntityId) -> Result<Option<DictWord>, RepositoryError> {
        self.table.get(id)
    }
    pub fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<DictWord>>, RepositoryError> {
        self.table.get_multi(ids)
    }
    pub fn get_all(&self) -> Result<Vec<DictWord>, RepositoryError> {
        self.table.get_all()
    }
    ...
    pub fn update(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::DictWord(EntityEvent::Updated)),
            ids: vec![updated.id],
            data: None,
        });
        Ok(updated)
    ...
    pub fn remove(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            ...
            owner_repo
                .get_relationships_from_right_ids(&WorkRelationshipField::DictWords, &[*id])?
                .into_iter()
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::DictWord(EntityEvent::Removed)),
            ids: vec![*id],
            data: None,
        });
    ...
    pub fn remove_multi(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_work_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&WorkRelationshipField::DictWords, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn get_relationships_from_owner(
        ...
        repo.get_relationship(owner_id, &WorkRelationshipField::DictWords)
    }
    ...
    pub fn restore(
        ...
        self.reconcile_backref_work_dict_words(event_buffer, snap_store, &snap.root_ids)?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("dict_word", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.dict_words);
            for id in &ids {
                match (snap.dict_words.contains_key(id), live.contains_key(id)) {
        ...
        {
            let mut live = write_or_recover(&store.dict_words);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.dict_words.get(id) {
    ...
    /// `work.dict_words`.
    fn reconcile_backref_work_dict_words(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        for (left, rights) in snap.jn_dict_word_from_work_dict_words.iter() {
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_dict_word_from_work_dict_words).insert(left, reconciled);
                event_buffer.push(Event {
...
pub struct DictWordRepositoryRO<'a> {
    table: Box<dyn DictWordTableRO + 'a>,
}
impl<'a> DictWordRepositoryRO<'a> {
    pub fn new(table: Box<dyn DictWordTableRO + 'a>) -> Self {
        DictWordRepositoryRO { table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<DictWord>, RepositoryError> {
        self.table.get(id)
    }
