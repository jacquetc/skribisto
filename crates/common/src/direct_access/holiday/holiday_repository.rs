...
use crate::direct_access::pace::PaceRelationshipField;
use crate::error::RepositoryError;
use serde::{Deserialize, Serialize};

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub enum HolidayRelationshipField {}

impl Display for HolidayRelationshipField {
...
pub trait HolidayTable {
    fn create(&mut self, entity: &Holiday) -> Result<Holiday, RepositoryError>;
    fn create_multi(&mut self, entities: &[Holiday]) -> Result<Vec<Holiday>, RepositoryError>;
    fn get(&self, id: &EntityId) -> Result<Option<Holiday>, RepositoryError>;
    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Holiday>>, RepositoryError>;
    fn get_all(&self) -> Result<Vec<Holiday>, RepositoryError>;
    fn update(&mut self, entity: &Holiday) -> Result<Holiday, RepositoryError>;
    fn update_multi(&mut self, entities: &[Holiday]) -> Result<Vec<Holiday>, RepositoryError>;
    fn update_with_relationships(&mut self, entity: &Holiday) -> Result<Holiday, RepositoryError>;
...
pub trait HolidayTableRO {
    fn get(&self, id: &EntityId) -> Result<Option<Holiday>, RepositoryError>;
    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Holiday>>, RepositoryError>;
    fn get_all(&self) -> Result<Vec<Holiday>, RepositoryError>;
}

pub struct HolidayRepository<'a> {
    table: Box<dyn HolidayTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> HolidayRepository<'a> {
    pub fn new(table: Box<dyn HolidayTable + 'a>, transaction: &'a Transaction) -> Self {
        HolidayRepository { table, transaction }
    }
    ...
    pub fn create_orphan(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Holiday(EntityEvent::Created)),
            ids: vec![new.id],
            data: None,
        });
        Ok(new)
    ...
    pub fn get(&self, id: &EntityId) -> Result<Option<Holiday>, RepositoryError> {
        self.table.get(id)
    }
    pub fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Holiday>>, RepositoryError> {
        self.table.get_multi(ids)
    }
    pub fn get_all(&self) -> Result<Vec<Holiday>, RepositoryError> {
        self.table.get_all()
    }
    ...
    pub fn update(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Holiday(EntityEvent::Updated)),
            ids: vec![updated.id],
            data: None,
        });
        Ok(updated)
    ...
    pub fn remove(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_pace_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&PaceRelationshipField::Holidays, &[*id])?
                .into_iter()
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Holiday(EntityEvent::Removed)),
            ids: vec![*id],
            data: None,
        });
    ...
    pub fn remove_multi(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_pace_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&PaceRelationshipField::Holidays, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn get_relationships_from_owner(
        ...
        let repo = repository_factory::write::create_pace_repository(self.transaction)?;
        repo.get_relationship(owner_id, &PaceRelationshipField::Holidays)
    }
    ...
    pub fn set_relationships_in_owner(
        ...
        let mut repo = repository_factory::write::create_pace_repository(self.transaction)?;
    ...
    pub fn restore(
        ...
        self.reconcile_backref_pace_holidays(event_buffer, snap_store, &snap.root_ids)?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("holiday", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.holidays);
            for id in &ids {
                match (snap.holidays.contains_key(id), live.contains_key(id)) {
        ...
        {
            let mut live = write_or_recover(&store.holidays);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.holidays.get(id) {
    ...
    /// `pace.holidays`.
    fn reconcile_backref_pace_holidays(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        for (left, rights) in snap.jn_holiday_from_pace_holidays.iter() {
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_holiday_from_pace_holidays).insert(left, reconciled);
                event_buffer.push(Event {
...
pub struct HolidayRepositoryRO<'a> {
    table: Box<dyn HolidayTableRO + 'a>,
}
impl<'a> HolidayRepositoryRO<'a> {
    pub fn new(table: Box<dyn HolidayTableRO + 'a>) -> Self {
        HolidayRepositoryRO { table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<Holiday>, RepositoryError> {
        self.table.get(id)
    }
