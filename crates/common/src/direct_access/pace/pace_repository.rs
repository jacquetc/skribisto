...
pub enum PaceRelationshipField {
    BookItem,
    Holidays,
    Milestones,
}

impl Display for PaceRelationshipField {
...
pub struct PaceRepository<'a> {
    table: Box<dyn PaceTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> PaceRepository<'a> {
    pub fn new(table: Box<dyn PaceTable + 'a>, transaction: &'a Transaction) -> Self {
        PaceRepository { table, transaction }
    }
    ...
    pub fn create_orphan(
        ...
        entity: &Pace,
    ) -> Result<Pace, RepositoryError> {
        let new = self.table.create(entity)?;
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Pace(EntityEvent::Created)),
            ids: vec![new.id],
            data: None,
        });
        Ok(new)
    ...
    pub fn get(&self, id: &EntityId) -> Result<Option<Pace>, RepositoryError> {
        self.table.get(id)
    }
    pub fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Pace>>, RepositoryError> {
        self.table.get_multi(ids)
    }
    pub fn get_all(&self) -> Result<Vec<Pace>, RepositoryError> {
        self.table.get_all()
    }
    ...
    pub fn remove(
        ...
        let holidays = entity.holidays.clone();
        let milestones = entity.milestones.clone();

        // remove all strong relationships, initiating a cascade remove

        repository_factory::write::create_holiday_repository(self.transaction)?
            .remove_multi(event_buffer, &holidays)?;
        repository_factory::write::create_milestone_repository(self.transaction)?
            .remove_multi(event_buffer, &milestones)?;
        ...
        let affected_owner_ids: Vec<EntityId> = {
            ...
            owner_repo
                .get_relationships_from_right_ids(&WorkRelationshipField::Paces, &[*id])?
                .into_iter()
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Pace(EntityEvent::Removed)),
            ids: vec![*id],
            data: None,
        });
    ...
    pub fn remove_multi(
        ...
        let mut holidays_ids: Vec<EntityId> = entities
            .iter()
            .flat_map(|entity| entity.as_ref().map(|entity| entity.holidays.clone()))
            .flatten()
            .collect();
        // remove duplicates
        holidays_ids.sort();
        holidays_ids.dedup();
        let mut milestones_ids: Vec<EntityId> = entities
            .iter()
            .flat_map(|entity| entity.as_ref().map(|entity| entity.milestones.clone()))
            .flatten()
            .collect();
        // remove duplicates
        milestones_ids.sort();
        milestones_ids.dedup();

        // remove all strong relationships, initiating a cascade remove

        repository_factory::write::create_holiday_repository(self.transaction)?
            .remove_multi(event_buffer, &holidays_ids)?;
        repository_factory::write::create_milestone_repository(self.transaction)?
            .remove_multi(event_buffer, &milestones_ids)?;
        // Before removal, find which owner(s) reference these entities
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_work_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&WorkRelationshipField::Paces, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn set_relationship_multi(
        ...
        if !all_right_ids.is_empty() {
            match field {
                PaceRelationshipField::BookItem => {
                    let child_repo =
    ...
    pub fn get_relationships_from_owner(
        ...
        repo.get_relationship(owner_id, &WorkRelationshipField::Paces)
    }
    ...
    pub fn set_relationships_in_owner(
        ...
        repo.set_relationship(event_buffer, owner_id, &WorkRelationshipField::Paces, ids)
    }
    ...
    pub fn restore(
        ...
        self.reconcile_backref_work_paces(event_buffer, snap_store, &snap.root_ids)?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("pace", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.paces);
            for id in &ids {
                match (snap.paces.contains_key(id), live.contains_key(id)) {
        ...
        //    all locks before recursing so self-referential ownership cannot deadlock).
        {
            ...
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(list) = snap.jn_holiday_from_pace_holidays.get(id) {
                    child_ids.extend(list.iter().copied());
                }
            }
            {
                let live_jn = read_or_recover(&store.jn_holiday_from_pace_holidays);
                for id in &ids {
        ...
        }
        {
            let mut child_ids: Vec<EntityId> = Vec::new();
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(list) = snap.jn_milestone_from_pace_milestones.get(id) {
                    child_ids.extend(list.iter().copied());
                }
            }
            {
                let live_jn = read_or_recover(&store.jn_milestone_from_pace_milestones);
                for id in &ids {
                    if let Some(list) = live_jn.get(id) {
                        child_ids.extend(list.iter().copied());
                    }
        ...
        {
            let mut live = write_or_recover(&store.paces);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.paces.get(id) {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_binder_item_from_pace_book_item);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_binder_item_from_pace_book_item.get(id) {
                    Some(v) => {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_holiday_from_pace_holidays);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_holiday_from_pace_holidays.get(id) {
                    Some(v) => {
                        live_jn.insert(*id, v.clone());
                    }
                    None => {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_milestone_from_pace_milestones);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_milestone_from_pace_milestones.get(id) {
                    Some(v) => {
                        live_jn.insert(*id, v.clone());
                    }
                    None => {
    ...
    /// `work.paces`.
    fn reconcile_backref_work_paces(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        for (left, rights) in snap.jn_pace_from_work_paces.iter() {
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_pace_from_work_paces).insert(left, reconciled);
                event_buffer.push(Event {
...
pub struct PaceRepositoryRO<'a> {
    table: Box<dyn PaceTableRO + 'a>,
}
impl<'a> PaceRepositoryRO<'a> {
    pub fn new(table: Box<dyn PaceTableRO + 'a>) -> Self {
        PaceRepositoryRO { table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<Pace>, RepositoryError> {
        self.table.get(id)
    }
