...
impl<'a> PaceRepository<'a> {
    ...
    pub(crate) fn restore_subtree(
        ...
        {
            let live = store.paces.read().unwrap();
            for id in &ids {
        ...
        //    all locks before recursing so self-referential ownership cannot deadlock).
        {
            ...
            {
                let live_jn = store.jn_holiday_from_pace_holidays.read().unwrap();
                for id in &ids {
        ...
        }
        {
            ...
            {
                let live_jn = store.jn_milestone_from_pace_milestones.read().unwrap();
                for id in &ids {
                    if let Some(list) = live_jn.get(id) {
                        child_ids.extend(list.iter().copied());
                    }
        ...
        {
            let mut live = store.paces.write().unwrap();
            for id in to_create.iter().chain(to_update.iter()) {
        ...
        {
            let mut live_jn = store.jn_binder_item_from_pace_book_item.write().unwrap();
            for id in to_create.iter().chain(to_update.iter()) {
        ...
        {
            let mut live_jn = store.jn_holiday_from_pace_holidays.write().unwrap();
            for id in to_create.iter().chain(to_update.iter()) {
        ...
        {
            let mut live_jn = store.jn_milestone_from_pace_milestones.write().unwrap();
            for id in to_create.iter().chain(to_update.iter()) {
