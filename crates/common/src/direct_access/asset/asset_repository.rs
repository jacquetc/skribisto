...
pub enum AssetRelationshipField {}

impl Display for AssetRelationshipField {
...
pub trait AssetTable {
    fn create(&mut self, entity: &Asset) -> Result<Asset, RepositoryError>;
    fn create_multi(&mut self, entities: &[Asset]) -> Result<Vec<Asset>, RepositoryError>;
    fn get(&self, id: &EntityId) -> Result<Option<Asset>, RepositoryError>;
    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Asset>>, RepositoryError>;
    fn get_all(&self) -> Result<Vec<Asset>, RepositoryError>;
    fn update(&mut self, entity: &Asset) -> Result<Asset, RepositoryError>;
    fn update_multi(&mut self, entities: &[Asset]) -> Result<Vec<Asset>, RepositoryError>;
    fn update_with_relationships(&mut self, entity: &Asset) -> Result<Asset, RepositoryError>;
...
pub struct AssetRepository<'a> {
    table: Box<dyn AssetTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> AssetRepository<'a> {
    pub fn new(table: Box<dyn AssetTable + 'a>, transaction: &'a Transaction) -> Self {
        AssetRepository { table, transaction }
    }
    ...
    pub fn create_orphan(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Asset(EntityEvent::Created)),
            ids: vec![new.id],
            data: None,
        });
        Ok(new)
    ...
    pub fn get(&self, id: &EntityId) -> Result<Option<Asset>, RepositoryError> {
        self.table.get(id)
    }
    pub fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Asset>>, RepositoryError> {
        self.table.get_multi(ids)
    }
    pub fn get_all(&self) -> Result<Vec<Asset>, RepositoryError> {
        self.table.get_all()
    }
    ...
    pub fn update(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Asset(EntityEvent::Updated)),
            ids: vec![updated.id],
            data: None,
        });
        Ok(updated)
    ...
    pub fn remove(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            ...
            owner_repo
                .get_relationships_from_right_ids(&WorkRelationshipField::Assets, &[*id])?
                .into_iter()
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Asset(EntityEvent::Removed)),
            ids: vec![*id],
            data: None,
        });
    ...
    pub fn remove_multi(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_work_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&WorkRelationshipField::Assets, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn get_relationships_from_owner(
        ...
        repo.get_relationship(owner_id, &WorkRelationshipField::Assets)
    }
    ...
    pub fn set_relationships_in_owner(
        ...
        repo.set_relationship(event_buffer, owner_id, &WorkRelationshipField::Assets, ids)
    }
    ...
    pub fn restore(
        ...
        self.reconcile_backref_work_assets(event_buffer, snap_store, &snap.root_ids)?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("asset", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.assets);
            for id in &ids {
                match (snap.assets.contains_key(id), live.contains_key(id)) {
        ...
        {
            let mut live = write_or_recover(&store.assets);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.assets.get(id) {
    ...
    /// `work.assets`.
    fn reconcile_backref_work_assets(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        for (left, rights) in snap.jn_asset_from_work_assets.iter() {
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_asset_from_work_assets).insert(left, reconciled);
                event_buffer.push(Event {
...
pub struct AssetRepositoryRO<'a> {
    table: Box<dyn AssetTableRO + 'a>,
}
impl<'a> AssetRepositoryRO<'a> {
    pub fn new(table: Box<dyn AssetTableRO + 'a>) -> Self {
        AssetRepositoryRO { table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<Asset>, RepositoryError> {
        self.table.get(id)
    }
