...
impl<'a> BinderTagRepository<'a> {
    ...
    pub fn restore(
        ...
        self.reconcile_backref_work_tags(event_buffer, snap_store, &snap.root_ids)?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("binder_tag", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.binder_tags);
            for id in &ids {
                match (snap.binder_tags.contains_key(id), live.contains_key(id)) {
        ...
        {
            let mut live = write_or_recover(&store.binder_tags);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.binder_tags.get(id) {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_binder_item_from_binder_tag_creates_in);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_binder_item_from_binder_tag_creates_in.get(id) {
                    Some(v) => {
        ...
        {
            let mut live_jn =
                write_or_recover(&store.jn_note_template_from_binder_tag_note_template);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_note_template_from_binder_tag_note_template.get(id) {
                    Some(v) => {
                        live_jn.insert(*id, v.clone());
                    }
                    None => {
        ...
        self.reconcile_backref_binder_item_tags(event_buffer, snap, &ids)?;
    ...
    /// `binder_item.tags`.
    fn reconcile_backref_binder_item_tags(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        for (left, rights) in snap.jn_binder_tag_from_binder_item_tags.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
        {
            let live_jn = read_or_recover(&store.jn_binder_tag_from_binder_item_tags);
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_binder_tag_from_binder_item_tags)
    ...
    /// Surgically reconcile one external referrer junction (out-of-scope key -> in-scope values):
    /// `work.tags`.
    fn reconcile_backref_work_tags(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        // External left keys whose ordered list references any scope id, in snapshot or live.
        let mut left_keys: std::collections::HashSet<EntityId> = std::collections::HashSet::new();
        for (left, rights) in snap.jn_binder_tag_from_work_tags.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_binder_tag_from_work_tags).insert(left, reconciled);
                event_buffer.push(Event {
