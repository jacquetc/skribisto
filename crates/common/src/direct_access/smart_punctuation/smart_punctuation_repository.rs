...
pub enum SmartPunctuationRelationshipField {}

impl Display for SmartPunctuationRelationshipField {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "{:?}", self)
    }
}

pub trait SmartPunctuationTable {
    fn create(&mut self, entity: &SmartPunctuation) -> Result<SmartPunctuation, RepositoryError>;
    fn create_multi(
        &mut self,
        entities: &[SmartPunctuation],
    ) -> Result<Vec<SmartPunctuation>, RepositoryError>;
    fn get(&self, id: &EntityId) -> Result<Option<SmartPunctuation>, RepositoryError>;
    fn get_multi(&self, ids: &[EntityId])
    -> Result<Vec<Option<SmartPunctuation>>, RepositoryError>;
    fn get_all(&self) -> Result<Vec<SmartPunctuation>, RepositoryError>;
    fn update(&mut self, entity: &SmartPunctuation) -> Result<SmartPunctuation, RepositoryError>;
    fn update_multi(
        &mut self,
        entities: &[SmartPunctuation],
    ) -> Result<Vec<SmartPunctuation>, RepositoryError>;
    ...
    ) -> Result<SmartPunctuation, RepositoryError>;
...
pub trait SmartPunctuationTableRO {
    fn get(&self, id: &EntityId) -> Result<Option<SmartPunctuation>, RepositoryError>;
    fn get_multi(&self, ids: &[EntityId])
    -> Result<Vec<Option<SmartPunctuation>>, RepositoryError>;
    fn get_all(&self) -> Result<Vec<SmartPunctuation>, RepositoryError>;
...
pub struct SmartPunctuationRepository<'a> {
    table: Box<dyn SmartPunctuationTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> SmartPunctuationRepository<'a> {
    pub fn new(table: Box<dyn SmartPunctuationTable + 'a>, transaction: &'a Transaction) -> Self {
        SmartPunctuationRepository { table, transaction }
    }
    ...
    pub fn create_orphan(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::SmartPunctuation(
                EntityEvent::Created,
            )),
            ids: vec![new.id],
            data: None,
    ...
    pub fn get(&self, id: &EntityId) -> Result<Option<SmartPunctuation>, RepositoryError> {
        self.table.get(id)
    }
    pub fn get_multi(
        &self,
    ...
    pub fn get_all(&self) -> Result<Vec<SmartPunctuation>, RepositoryError> {
        self.table.get_all()
    }
    ...
    pub fn remove(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            ...
            owner_repo
                .get_relationships_from_right_ids(&WorkRelationshipField::SmartPunctuation, &[*id])?
                .into_iter()
    ...
    pub fn remove_multi(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_work_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&WorkRelationshipField::SmartPunctuation, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn get_relationships_from_owner(
        ...
        repo.get_relationship(owner_id, &WorkRelationshipField::SmartPunctuation)
    }
    ...
    pub fn restore(
        ...
        self.reconcile_backref_work_smart_punctuation(event_buffer, snap_store, &snap.root_ids)?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("smart_punctuation", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
    ...
    /// `work.smart_punctuation`.
    fn reconcile_backref_work_smart_punctuation(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        for (left, rights) in snap.jn_smart_punctuation_from_work_smart_punctuation.iter() {
...
pub struct SmartPunctuationRepositoryRO<'a> {
    table: Box<dyn SmartPunctuationTableRO + 'a>,
}
impl<'a> SmartPunctuationRepositoryRO<'a> {
    pub fn new(table: Box<dyn SmartPunctuationTableRO + 'a>) -> Self {
        SmartPunctuationRepositoryRO { table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<SmartPunctuation>, RepositoryError> {
        self.table.get(id)
    }
