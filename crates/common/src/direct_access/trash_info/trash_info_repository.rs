...
impl<'a> TrashInfoRepository<'a> {
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("trash_info", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.trash_infos);
            for id in &ids {
                match (snap.trash_infos.contains_key(id), live.contains_key(id)) {
        ...
        {
            let mut live = write_or_recover(&store.trash_infos);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.trash_infos.get(id) {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_binder_from_trash_info_trashed_binder);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_binder_from_trash_info_trashed_binder.get(id) {
                    Some(v) => {
    ...
    fn reconcile_backref_work_trash_infos(
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_trash_info_from_work_trash_infos)
