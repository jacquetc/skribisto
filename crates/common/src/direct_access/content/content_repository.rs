...
impl<'a> ContentRepository<'a> {
    ...
    pub fn restore(
        ...
        self.reconcile_backref_binder_item_contents(event_buffer, snap_store, &snap.root_ids)?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("content", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        self.reconcile_backref_comment_content(event_buffer, snap, &ids)?;
    ...
    /// `comment.content`.
    fn reconcile_backref_comment_content(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        for (left, rights) in snap.jn_content_from_comment_content.iter() {
    ...
    /// Surgically reconcile one external referrer junction (out-of-scope key -> in-scope values):
    /// `binder_item.contents`.
    fn reconcile_backref_binder_item_contents(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        // External left keys whose ordered list references any scope id, in snapshot or live.
        let mut left_keys: std::collections::HashSet<EntityId> = std::collections::HashSet::new();
        for (left, rights) in snap.jn_content_from_binder_item_contents.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
