...
use crate::direct_access::search::SearchRelationshipField;
use crate::error::RepositoryError;
use serde::{Deserialize, Serialize};

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub enum SearchResultRelationshipField {}

impl Display for SearchResultRelationshipField {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "{:?}", self)
    }
}

pub trait SearchResultTable {
    fn create(&mut self, entity: &SearchResult) -> Result<SearchResult, RepositoryError>;
    fn create_multi(
        &mut self,
        entities: &[SearchResult],
    ) -> Result<Vec<SearchResult>, RepositoryError>;
    fn get(&self, id: &EntityId) -> Result<Option<SearchResult>, RepositoryError>;
    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<SearchResult>>, RepositoryError>;
    fn get_all(&self) -> Result<Vec<SearchResult>, RepositoryError>;
    fn update(&mut self, entity: &SearchResult) -> Result<SearchResult, RepositoryError>;
    fn update_multi(
        &mut self,
        entities: &[SearchResult],
    ) -> Result<Vec<SearchResult>, RepositoryError>;
...
pub trait SearchResultTableRO {
    fn get(&self, id: &EntityId) -> Result<Option<SearchResult>, RepositoryError>;
    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<SearchResult>>, RepositoryError>;
    fn get_all(&self) -> Result<Vec<SearchResult>, RepositoryError>;
}

pub struct SearchResultRepository<'a> {
    table: Box<dyn SearchResultTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> SearchResultRepository<'a> {
    pub fn new(table: Box<dyn SearchResultTable + 'a>, transaction: &'a Transaction) -> Self {
        SearchResultRepository { table, transaction }
    }
    ...
    pub fn create_orphan(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::SearchResult(EntityEvent::Created)),
            ids: vec![new.id],
            data: None,
        });
        Ok(new)
    ...
    pub fn get(&self, id: &EntityId) -> Result<Option<SearchResult>, RepositoryError> {
        self.table.get(id)
    }
    pub fn get_multi(
        &self,
        ids: &[EntityId],
    ) -> Result<Vec<Option<SearchResult>>, RepositoryError> {
        self.table.get_multi(ids)
    }
    pub fn get_all(&self) -> Result<Vec<SearchResult>, RepositoryError> {
        self.table.get_all()
    }
    ...
    pub fn update(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::SearchResult(EntityEvent::Updated)),
            ids: vec![updated.id],
            data: None,
        });
        Ok(updated)
    ...
    pub fn remove(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_search_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&SearchRelationshipField::Results, &[*id])?
                .into_iter()
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::SearchResult(EntityEvent::Removed)),
            ids: vec![*id],
            data: None,
        });
    ...
    pub fn remove_multi(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_search_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&SearchRelationshipField::Results, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn get_relationships_from_owner(
        ...
        let repo = repository_factory::write::create_search_repository(self.transaction)?;
        repo.get_relationship(owner_id, &SearchRelationshipField::Results)
    }
    ...
    pub fn set_relationships_in_owner(
        ...
        let mut repo = repository_factory::write::create_search_repository(self.transaction)?;
    ...
    pub fn restore(
        ...
        self.reconcile_backref_search_results(event_buffer, snap_store, &snap.root_ids)?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("search_result", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.search_results);
            for id in &ids {
                match (snap.search_results.contains_key(id), live.contains_key(id)) {
        ...
        {
            let mut live = write_or_recover(&store.search_results);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.search_results.get(id) {
        ...
        if !to_create.is_empty() {
            event_buffer.push(Event {
                origin: Origin::DirectAccess(DirectAccessEntity::SearchResult(
                    EntityEvent::Created,
                )),
                ids: to_create.clone(),
                data: None,
    ...
    /// `search.results`.
    fn reconcile_backref_search_results(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        for (left, rights) in snap.jn_search_result_from_search_results.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
        {
            let live_jn = read_or_recover(&store.jn_search_result_from_search_results);
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_search_result_from_search_results)
...
pub struct SearchResultRepositoryRO<'a> {
    table: Box<dyn SearchResultTableRO + 'a>,
}
impl<'a> SearchResultRepositoryRO<'a> {
    pub fn new(table: Box<dyn SearchResultTableRO + 'a>) -> Self {
        SearchResultRepositoryRO { table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<SearchResult>, RepositoryError> {
        self.table.get(id)
    }
