...
impl<'a> SearchRepository<'a> {
    ...
    pub(crate) fn restore_subtree(
        ...
        {
            let live = read_or_recover(&store.searchs);
            for id in &ids {
        ...
        {
            let mut live = write_or_recover(&store.searchs);
            for id in to_create.iter().chain(to_update.iter()) {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_search_result_from_search_results);
            for id in to_create.iter().chain(to_update.iter()) {
    ...
    fn reconcile_backref_work_info_search(
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_search_from_work_info_search).insert(left, reconciled);
                event_buffer.push(Event {
