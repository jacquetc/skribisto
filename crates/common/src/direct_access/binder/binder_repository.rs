...
impl<'a> BinderRepository<'a> {
    ...
    pub fn restore(
        ...
        let binder_ids: Vec<_> = store.binders.read().unwrap().keys().copied().collect();
        emit(
            DirectAccessEntity::Binder(EntityEvent::Created),
            binder_ids.clone(),
        );
        emit(DirectAccessEntity::Binder(EntityEvent::Updated), binder_ids);

        // Emit Created events for strong children

        {
            let child_ids: Vec<_> = store.binder_items.read().unwrap().keys().copied().collect();
            emit(
