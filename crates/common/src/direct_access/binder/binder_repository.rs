...
pub enum BinderRelationshipField {
    BinderItems,
}

impl Display for BinderRelationshipField {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "{:?}", self)
    }
}

pub trait BinderTable {
...
pub struct BinderRepository<'a> {
    table: Box<dyn BinderTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> BinderRepository<'a> {
    pub fn new(table: Box<dyn BinderTable + 'a>, transaction: &'a Transaction) -> Self {
        BinderRepository { table, transaction }
    }
    ...
    pub fn create_orphan(
        ...
        entity: &Binder,
    ) -> Result<Binder, RepositoryError> {
        let new = self.table.create(entity)?;
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Binder(EntityEvent::Created)),
            ids: vec![new.id],
            data: None,
        });
        Ok(new)
    ...
    pub fn get(&self, id: &EntityId) -> Result<Option<Binder>, RepositoryError> {
        self.table.get(id)
    }
    pub fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Binder>>, RepositoryError> {
        self.table.get_multi(ids)
    }
    pub fn get_all(&self) -> Result<Vec<Binder>, RepositoryError> {
        self.table.get_all()
    }
    ...
    pub fn remove(
        ...
        let binder_items = entity.binder_items.clone();
        ...
        repository_factory::write::create_binder_item_repository(self.transaction)?
            .remove_multi(event_buffer, &binder_items)?;
        ...
        let affected_owner_ids: Vec<EntityId> = {
            ...
            owner_repo
                .get_relationships_from_right_ids(&WorkRelationshipField::Binders, &[*id])?
                .into_iter()
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Binder(EntityEvent::Removed)),
            ids: vec![*id],
            data: None,
        });
    ...
    pub fn remove_multi(
        ...
        let mut binder_items_ids: Vec<EntityId> = entities
            .iter()
            .flat_map(|entity| entity.as_ref().map(|entity| entity.binder_items.clone()))
            .flatten()
            .collect();
        // remove duplicates
        binder_items_ids.sort();
        binder_items_ids.dedup();

        // remove all strong relationships, initiating a cascade remove

        repository_factory::write::create_binder_item_repository(self.transaction)?
            .remove_multi(event_buffer, &binder_items_ids)?;
        // Before removal, find which owner(s) reference these entities
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_work_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&WorkRelationshipField::Binders, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn get_relationships_from_owner(
        ...
        repo.get_relationship(owner_id, &WorkRelationshipField::Binders)
    }
    ...
    pub fn set_relationships_in_owner(
        ...
        repo.set_relationship(event_buffer, owner_id, &WorkRelationshipField::Binders, ids)
    }
    ...
    pub fn restore(
        ...
        self.reconcile_backref_work_binders(event_buffer, snap_store, &snap.root_ids)?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("binder", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.binders);
            for id in &ids {
                match (snap.binders.contains_key(id), live.contains_key(id)) {
        ...
        //    all locks before recursing so self-referential ownership cannot deadlock).
        {
            ...
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(list) = snap.jn_binder_item_from_binder_binder_items.get(id) {
        ...
        {
            let mut live = write_or_recover(&store.binders);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.binders.get(id) {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_binder_item_from_binder_binder_items);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_binder_item_from_binder_binder_items.get(id) {
                    Some(v) => {
        ...
        self.reconcile_backref_trash_info_trashed_binder(event_buffer, snap, &ids)?;
    ...
    /// `work.binders`.
    fn reconcile_backref_work_binders(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        for (left, rights) in snap.jn_binder_from_work_binders.iter() {
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_binder_from_work_binders).insert(left, reconciled);
                event_buffer.push(Event {
    ...
    /// Surgically reconcile one external referrer junction (out-of-scope key -> in-scope values):
    /// `trash_info.trashed_binder`.
    fn reconcile_backref_trash_info_trashed_binder(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        // External left keys whose ordered list references any scope id, in snapshot or live.
        let mut left_keys: std::collections::HashSet<EntityId> = std::collections::HashSet::new();
        for (left, rights) in snap.jn_binder_from_trash_info_trashed_binder.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
        {
            let live_jn = read_or_recover(&store.jn_binder_from_trash_info_trashed_binder);
            for (left, rights) in live_jn.iter() {
                if rights.iter().any(|rid| scope.contains(rid)) {
                    left_keys.insert(*left);
                }
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_binder_from_trash_info_trashed_binder)
...
pub struct BinderRepositoryRO<'a> {
    table: Box<dyn BinderTableRO + 'a>,
}
impl<'a> BinderRepositoryRO<'a> {
    pub fn new(table: Box<dyn BinderTableRO + 'a>) -> Self {
        BinderRepositoryRO { table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<Binder>, RepositoryError> {
        self.table.get(id)
    }
