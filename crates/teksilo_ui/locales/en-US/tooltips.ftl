...
goal-exportable = This item is left out of the export.
goal-exportable-more = Its own length still shows, because the writing is still there, but it counts toward no total, exactly as it will appear in no exported book. Leaving a container out does not leave the pieces inside it out: each item answers for itself, which is what the "Apply to children" button beside the switch is for.
...
concept-label-more = Not a [tag](:concept-tag): a label belongs to one item alone, plain text with no color or catalog behind it, something like "1st plot point" or "needs a name". Set it from the item's context menu, or edit it inline in the Overview's Label column; it shows as a small subtitle under the title, in the outline and in the stream alike.
...
concept-chapter-mode = Whether a new chapter is a folder of scenes or a single flat item.
...
concept-backup-more = Taken on a schedule you choose: on open, on close, on an interval, or any mix, with old copies pruned by age or by count. Opening one loads it in its own window: you can look and even edit, but only Save As or Restore keeps anything. Different from a [version](:concept-version), which tracks one item's own past.
concept-version = One item's own past: every change to its text or synopsis.
concept-version-more = Built from the project's own save history and its backups together, one entry per change rather than one per file. Pick an entry to see what changed and restore only that item's text to it; the rest of the project is untouched, and a safety copy is taken first. Different from a [backup](:concept-backup), which is a copy of everything.
concept-trash = A trashed item is hidden, not moved: it stays exactly where it was.
concept-trash-more = Trashing only flips one flag: the item keeps its place in the binder, marked inactive rather than removed. Restore it and it reappears in that exact spot, with no re-filing to do. Empty Trash removes trashed items for good, and each of these is one undoable step, like any other project edit.
...
concept-status-more = Not a [tag](:concept-tag): an item has exactly one status, the stages are ordered, and the project owns the list, so you can rename, reorder or add to it whenever you like. An asterisk beside a chapter or book means the parts below it do not all agree with it. Set it from the Inspector, a card, a stream item, or here.
