...
goal-exportable = Cet élément est hors export.
goal-exportable-more = Sa longueur propre reste affichée, car le texte est toujours là, mais il ne compte dans aucun total, exactement comme il n’apparaîtra dans aucun livre exporté. Exclure un conteneur n’exclut pas ce qu’il contient : chaque élément répond pour lui-même, et c’est à cela que sert le bouton « Appliquer aux enfants » à côté de l’interrupteur.
...
concept-label-more = Ce n’est pas une [étiquette](:concept-tag) : elle n’appartient qu’à cet élément, un texte libre sans couleur ni catalogue derrière lui, quelque chose comme « 1er rebondissement » ou « à nommer ». Définissez-la depuis le menu contextuel de l’élément, ou modifiez-la directement dans la colonne Libellé de la vue d’ensemble ; elle apparaît en petit sous le titre, dans l’arborescence comme dans le flux.
...
concept-chapter-mode = Si un nouveau chapitre est un dossier de scènes ou un seul élément à plat.
...
concept-status = Où en est un élément dans votre propre flux de travail : une étape à la fois, qui avance.
concept-status-more = Ce n'est pas une [étiquette](:concept-tag) : un élément a exactement un statut, les étapes sont ordonnées, et la liste appartient au projet, donc vous pouvez la renommer, la réordonner ou la compléter quand vous voulez. Un astérisque à côté d'un chapitre ou d'un livre signale que les parties en dessous ne sont pas toutes d'accord avec lui. Se règle depuis l'inspecteur, une fiche, un élément de flux, ou ici.
