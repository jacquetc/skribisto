...
use teksilo::data::TreeCheckedModel;
use teksilo::data::{CheckState, NodeId, TreeModel};
use teksilo::prelude::*;
use teksilo::widgets::{StandardTreeItem, TextWidget, TreeView};
...
impl Widget for ChooseTreeWidget {
    fn build(&mut self, ctx: &mut BuildContext) -> Vec<WidgetId> {
        let Some(model) = self.model.clone() else {
            let id = ctx.add(
                teksilo::widgets::Center::new().child(
