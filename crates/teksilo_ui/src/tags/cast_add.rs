...
pub fn cast_add_button(
    ...
    PopoverButton::new(
        teksilo::widgets::Button::new(tr!(cast_add()))
            .variant(teksilo::widgets::ButtonVariant::Plain),
    )
...
///   build — teksilo drops effects on rebuild (see Inspector's move subscription).
#[derive(Clone)]
