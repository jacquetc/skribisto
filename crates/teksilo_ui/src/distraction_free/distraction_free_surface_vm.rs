...
//! [`crate::shared::FocusViewModel`] and `GoToViewModel`, for the same
...
pub struct SurfaceDeps {
    ...
    /// settings rows.
    pub settings: crate::settings::SettingsViewModel,
}
...
impl DistractionFreeSurfaceViewModel {
    ...
    pub fn hand_back(&self, item_id: u64, state: crate::shared::ViewState) {
    ...
    fn sync_band_color(&self) {
        ...
        let next = crate::shared::CaretHighlightSettings::document_color(theme.caret_band_color());
    ...
    pub fn caret_band(&self) -> Option<crate::shared::CaretHighlightSettings> {
        let deps = self.deps.borrow();
        let d = deps.as_ref()?;
        Some(crate::shared::CaretHighlightSettings::new(
