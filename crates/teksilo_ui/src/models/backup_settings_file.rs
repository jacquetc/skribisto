...
//! read-modify-write unconditionally, see `teksilo_settings::file`'s module
...
//! into the app's shared `teksilo::settings::SettingsRegistry` (see
...
/// left to configure, exactly like `teksilo-settings`' own `WindowStateService`.
...
/// directly against `teksilo-settings::file`: `SettingsFile::load` and `load_strict` both call
...
/// currently offers, and this crate does not modify `teksilo` to add one (out of scope, and
...
/// `teksilo-settings::lock`'s module docs) — and wedge on a lock nobody holds any more.
///
...
mod tests {
    ...
    fn registering_with_the_settings_registry_reloads_on_dispatch() {
        ...
        use teksilo::settings::SettingsRegistry;
