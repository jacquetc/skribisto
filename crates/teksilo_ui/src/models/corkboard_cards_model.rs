...
//!
//! Built the same "framework way" the Outline tree is (`teksilo::data`): the
//! model owns a [`ListModel<CorkboardCard>`](teksilo::data::ListModel) synced to the Qleany backend via the
//! framework's keyed [`reconcile_by_key`](teksilo::data::ListModel::reconcile_by_key)
//! (insert / remove / move / in-place update in one diff — no hand-rolled
//! reconcile), and exposes a [`SortFilterListModel`](teksilo::data::SortFilterListModel) projection that gives the
//! corkboard a **live search filter and column sort for free**. It implements
//! [`ListDataSource`](teksilo::data::ListDataSource) itself, so a `GridView::from_source` renders it and a
...
mod filter_tests {
    use super::*;
    use teksilo::data::{ListDataSource, ListModel, SortFilterListModel};
...
mod imp {
    ...
    impl ListDataSource for CorkboardCardsModel {
        ...
        fn observe_changes(
            &self,
            f: impl Fn(&teksilo::data::DataChange) + 'static,
        ) -> ObserverHandle {
