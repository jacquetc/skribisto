// SPDX-License-Identifier: GPL-3.0-only
// SPDX-FileCopyrightText: 2026 Cyril Jacquet

//! The status bar's indicators. Each is a thin reactive view over a pure view-model
//! function — the save glyph over `crate::save::save_status`, the word count over
//! [`word_count_status`], the writing-session timer over
//! `crate::writing_session`.

pub(crate) mod focus_strip;
pub(crate) mod game_indicator;
pub(crate) mod go_to_button;
pub(crate) mod notification_bell;
pub(crate) mod save_indicator;
pub(crate) mod session_status_item;
pub(crate) mod word_count_indicator;
mod word_count_status;

pub use word_count_status::{CountDisplay, GoalDisplay, count_display, goal_display};
