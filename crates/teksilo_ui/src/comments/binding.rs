...
/// an unrelated one.
fn deepen(c: teksilo::prelude::Color) -> teksilo::prelude::Color {
    c.darken(0.18)
}
...
fn to_doc_color(c: teksilo::prelude::Color) -> teksilo::text_document::Color {
    let q = |v: f32| (v.clamp(0.0, 1.0) * 255.0).round() as u8;
    teksilo::text_document::Color::rgb(q(c.r()), q(c.g()), q(c.b()))
}
...
impl CommentBinding {
    ...
    pub fn annotation_spans(&self) -> Vec<teksilo::widgets::rich_text::TextAnnotationSpan> {
        let rows = self.vm.model().rows_for_content(self.content_id);
        self.session
            .anchors()
            .into_iter()
            ...
            .filter_map(|a| {
                ...
                Some(teksilo::widgets::rich_text::TextAnnotationSpan {
                    start: a.start,
                    end: a.end,
