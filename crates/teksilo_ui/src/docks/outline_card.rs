...
impl OutlineCard {
    ...
    fn load_deferred(&self) {
        ...
        {
            let plain = teksilo::text_document::djot_to_plain_text(
                &synopsis.data,
                &teksilo::text_document::DjotImportOptions::default(),
            );
...
impl Widget for DeferredLine {
    fn build(&mut self, ctx: &mut BuildContext) -> Vec<WidgetId> {
        ...
        self.value.bind_to(
            ...
            teksilo::core::binding::BindingLevel::Rebuild,
        );
...
mod tests {
    ...
    fn built(card: OutlineCard) -> WidgetTree {
        ...
        tree.add(card);
        tree.layout(teksilo::canvas::SizeProposal::exact(500.0, 600.0));
        tree
    }
