...
//!
//! `teksilo`'s `NotificationCenterButton::for_audience` bakes its scope in at
...
use teksilo::prelude::*;
use teksilo::widgets::styles::IconButtonRecipe;
use teksilo::widgets::{
...
mod tests {
    ...
    use teksilo::widgets::{NotificationEntry, ToastPriority, ToastRoute};

    /// A minimal archived entry routed to `route` — mirrors teksilo's own
