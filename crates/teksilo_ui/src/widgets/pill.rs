...
///
/// Mirrors `teksilo_widgets::tooltip::rich::DWELL_PROMOTION`, which is `pub(crate)` and so
...
pub fn attach_labelled_composite_tooltip(
    ...
    let tooltip = CompositeTooltipWidget::new()
        .content_boxed(body)
        // A genuine maximum: composite tooltips hug their content (fixed in teksilo), so
...
impl Widget for Pill {
    fn build(&mut self, ctx: &mut BuildContext) -> Vec<WidgetId> {
        let mut content = teksilo::widgets::HStack::new().spacing(4.0);
            ...
            (self.on_remove.clone(), self.remove_label.clone())
        {
            ...
            let x_glyph = (teksilo::widgets::BuiltInIcons::defaults().clear)()
                .icon_size(12.0)
                .color(
                    self.text_color
                        .clone()
