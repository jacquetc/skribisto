...
//! to [`crate::shared::binder_ops`] (a shared layer below the view-models, which reads the
//! backend directly) or fires as an [`AppIntent`] on the bus for `App` to route.
//!
...
use teksilo::prelude::*; // Signal, BuildContext, EventContext, tr!
use teksilo::widgets::{MessageBox, MessageBoxButtons};
use uuid::Uuid;
...
impl OverviewViewModel {
    ...
    /// item's text. Both guards live in [`crate::shared::binder_ops`], so the two views
    /// cannot drift.
