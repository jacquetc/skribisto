...
//! `work_id()` — a [`crate::shared::long_op::CapturedWork`] bundled with the op id in
//! a [`crate::shared::long_op::TrackedOp`], captured once in [`SaveAsViewModel::start`]
...
use teksilo::prelude::*; // EventContext, tr!
...
use crate::shared::long_op::{CapturedWork, TrackedOp, event_id, parse_payload};
...
impl SaveAsViewModel {
    ...
    /// project being saved) and the CURRENT Work (F1 — a [`crate::shared::long_op::CapturedWork`])
