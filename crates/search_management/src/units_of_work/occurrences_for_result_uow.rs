...
use crate::use_cases::occurrences_for_result_uc::{
    OccurrencesForResultUnitOfWorkFactoryTrait, OccurrencesForResultUnitOfWorkTrait,
};
...
use common::event::SearchManagementEvent::OccurrencesForResult;
...
// Unit of work for OccurrencesForResult

pub struct OccurrencesForResultUnitOfWork {
    context: DbContext,
...
impl OccurrencesForResultUnitOfWork {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        OccurrencesForResultUnitOfWork {
...
impl CommandUnitOfWork for OccurrencesForResultUnitOfWork {
...
//
// Exactly the same macros must be set in the use case uow trait file in ../use_cases/occurrences_for_result_uc.rs
//
...
impl OccurrencesForResultUnitOfWorkTrait for OccurrencesForResultUnitOfWork {
    fn publish_occurrences_for_result_event(&self, ids: Vec<EntityId>, data: Option<String>) {
        self.event_hub.send_event(Event {
            origin: Origin::SearchManagement(OccurrencesForResult),
            ids,
            data,
        });
    }
...
pub struct OccurrencesForResultUnitOfWorkFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl OccurrencesForResultUnitOfWorkFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        OccurrencesForResultUnitOfWorkFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl OccurrencesForResultUnitOfWorkFactoryTrait for OccurrencesForResultUnitOfWorkFactory {
    fn create(&self) -> Box<dyn OccurrencesForResultUnitOfWorkTrait> {
        Box::new(OccurrencesForResultUnitOfWork::new(
            &self.context,
            &self.event_hub,
        ))
    }
