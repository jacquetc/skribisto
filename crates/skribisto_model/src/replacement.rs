...
//! ask and *how* to apply the answer lives in the UI's own
//! `text_replacement::session`; this module only answers "does this text end in
//! a fired trigger?".
//!
//! ## How a trigger fires
//!
...
//! character of the window handed to
//! [`TextReplacementEngine::check`](crate::replacement::TextReplacementEngine::check); the
...
//! writer typed — Word's three-state rule. See `apply_case` below.

use crate::casing;

/// One rule of the writer's custom lexicon.
///
/// Declared here rather than beside the reactive list model that loads it: this
/// is the shape the matcher takes, and the model's job is to produce it. The UI
/// re-exports it as `models::TextReplacementRuleRow`, which is what every
/// reader there still calls it.
...
pub struct TextReplacementEngine {
    ...
    /// and mis-cases the replacement. See [`crate::casing`]. Empty is
...
mod tests {
    ...
    /// `crate::casing` and is tested exhaustively there; this is the
