...
pub fn build_bundle(
    ...
    for root in &tree.roots {
        b.emit_node(root, 0, MANUSCRIPT_INDEX, manuscript_name, &mut manuscript_items);
    }
    ...
    let mut binders = vec![make_binder(manuscript_bid, manuscript_name, &b.now, manuscript_items)];
    let mut binder_order = vec![manuscript_bid];
    if !story_items.is_empty() {
        binders.push(make_binder(story_bid, story_bible_name, &b.now, story_items));
...
impl<'a> Builder<'a> {
    fn new(source: &'a PlumeSource) -> Self {
        Self {
            ids: IdGen::new(),
            source,
    ...
    fn emit_node(
        ...
        match node.kind {
            PlumeKind::Book => {
                self.emit_container(node, indent, SubRole::Book, ContentRole::BookTitle, true, bindex, bname, out)
            }
            PlumeKind::Act => {
                self.emit_container(node, indent, SubRole::Part, ContentRole::PartTitle, false, bindex, bname, out)
            }
            PlumeKind::Chapter => {
                if node.children.iter().any(|c| c.kind == PlumeKind::Scene) {
                    self.emit_container(node, indent, SubRole::Chapter, ContentRole::ChapterTitle, false, bindex, bname, out)
                } else {
    ...
    fn emit_container(
        ...
        let (_id, bi) =
            self.make_item(bindex, bname, Role::Folder, sub_role, &node.name, indent, true, contents, refs, &node.badge);
        out.push(bi);
        ...
        if !own_text.is_empty() {
            let (_i, cbi) = self.make_item(
                bindex, bname, Role::Item, SubRole::Scene, &content_title(&node.name), indent + 1, true,
                vec![(ContentRole::SceneText, own_text), (ContentRole::SynopsisText, String::new())],
                Vec::new(), "",
            );
            out.push(cbi);
        }
        ...
        if !own_note.is_empty() {
            let (_i, nbi) = self.make_item(
                bindex, bname, Role::Item, SubRole::Note, &notes_title(&node.name), indent + 1, true,
                vec![(ContentRole::NoteText, own_note), (ContentRole::SynopsisText, String::new())],
                Vec::new(), "",
            );
            out.push(nbi);
        }
        ...
        if with_book_end {
            let (_i, ebi) = self.make_item(
                bindex, bname, Role::Item, SubRole::BookEnd, "", indent + 1, true, Vec::new(), Vec::new(), "",
            );
            out.push(ebi);
        }
    }
    ...
    fn emit_chapter_scene(&mut self, node: &PlumeNode, indent: i64, bindex: usize, bname: &str, out: &mut Vec<BundledItem>) {
        let scene = self.text_djot(node.number);
        let synopsis = self.synopsis_djot(node.number);
        let refs = self.resolve_attend(&node.attend);
        let (_id, bi) = self.make_item(
            bindex, bname, Role::Item, SubRole::ChapterScene, &node.name, indent, true,
            vec![
    ...
    fn emit_scene(&mut self, node: &PlumeNode, indent: i64, bindex: usize, bname: &str, out: &mut Vec<BundledItem>) {
        let scene = self.text_djot(node.number);
        let synopsis = self.synopsis_djot(node.number);
        let refs = self.resolve_attend(&node.attend);
        let (_id, bi) = self.make_item(
            bindex, bname, Role::Item, SubRole::Scene, &node.name, indent, true,
            vec![(ContentRole::SceneText, scene), (ContentRole::SynopsisText, synopsis)],
            refs, &node.badge,
        );
        out.push(bi);
        self.emit_sibling_note(node, indent, bindex, bname, out);
    ...
    fn emit_separator(&mut self, node: &PlumeNode, indent: i64, bindex: usize, bname: &str, out: &mut Vec<BundledItem>) {
        ...
        let (_id, bi) = self.make_item(
            bindex, bname, Role::Item, SubRole::Text, &node.name, indent, true, Vec::new(), Vec::new(), "",
        );
        out.push(bi);
    }
    ...
    fn emit_sibling_note(&mut self, node: &PlumeNode, indent: i64, bindex: usize, bname: &str, out: &mut Vec<BundledItem>) {
        let note = self.note_djot(node.number);
        if note.is_empty() {
            return;
        }
        let (_i, nbi) = self.make_item(
            bindex, bname, Role::Item, SubRole::Note, &notes_title(&node.name), indent, true,
            vec![(ContentRole::NoteText, note), (ContentRole::SynopsisText, String::new())],
            Vec::new(), "",
        );
        out.push(nbi);
    }
    ...
    fn synopsis_djot(&mut self, number: Option<u32>) -> String {
        let raw = number.and_then(|n| self.source.synopsis(n)).map(str::to_string);
        self.convert(raw.as_deref())
    }
    ...
    fn attend_doc_djot(&mut self, number: Option<u32>) -> String {
        let raw = number.and_then(|n| self.source.attend_doc(n)).map(str::to_string);
        self.convert(raw.as_deref())
    }
    ...
    fn convert(&mut self, html: Option<&str>) -> String {
        match html {
            ...
            Some(h) => match html_to_djot(h) {
                ...
                Ok(d) => d,
                Err(e) => {
                    self.warnings.push(format!("could not convert some rich text: {e}"));
                    String::new()
                }
            },
        }
...
fn build_obj_synopsis(obj: &PlumeObj, spinbox_label: &str) -> String {
    ...
    if !obj.spinbox.is_empty() {
        meta.push(format!("{} {}", spinbox_label, obj.spinbox).trim().to_string());
    }
...
fn count_meaningful(node: &PlumeNode) -> u64 {
    let self_count = if node.kind == PlumeKind::Separator { 0 } else { 1 };
