...
//    whole comment — kind, uid, uid_tag, author, initials, body, resolved/orphan state and
//    the full quote anchor — beside an `Empty` unit variant, so the size gap is real and
//    inherent to the shape the generator produces from an `enum_values` list. Boxing it
//    would change a generated type's public shape and be undone by the next regeneration;
//    the values are built once per imported comment and moved once, so the gap costs
//    nothing measurable.
