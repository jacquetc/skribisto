...
//
// 3. `#[allow(clippy::large_enum_variant)]` on `ImportComment` and on `ApplyImportRow`.
//    `ImportComment::Found` is a whole comment — kind, uid, uid_tag, author, initials,
//    body, resolved/orphan state and the full quote anchor — beside an `Empty` unit
//    variant, and `ApplyImportRow`'s two populated variants each carry a vector of those.
//    The size gap is real and inherent to the shape the generator produces from an
