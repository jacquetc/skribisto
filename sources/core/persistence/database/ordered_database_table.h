...
template <class T> Result<T> OrderedDatabaseTable<T>::add(T &&entity)
{
    return QtConcurrent::task([this](T entity) {
               QSqlDatabase database = QSqlDatabase::database(DatabaseTable<T>::databaseContext()->databaseName());
               ...
               else
               {
                   ...
                   insertOrderingStr = QString("INSERT INTO %1 (previous, next) VALUES (:lastOrderingId, NULL)")
               ...
               {
                   QString updateEntityStr =
                       "UPDATE " + entityTableName + " SET ordering_id = :ordering_id WHERE id = :id";
        ...
        .onThreadPool(DatabaseTable<T>::databaseContext()->threadPool())
        .spawn()
        .result();
}
...
{
    return QtConcurrent::task([this](T entity, int position) {
               QSqlDatabase database = QSqlDatabase::database(DatabaseTable<T>::databaseContext()->databaseName());
               const QString &entityTableName = this->tableName();
               const QString &entityOrderingTableName = this->orderingTableName();
        ...
        .withArguments(std::move(entity), position)
        .onThreadPool(DatabaseTable<T>::databaseContext()->threadPool())
        .spawn()
        .result();
}
...
{
    return QtConcurrent::task([this](const QHash<QString, QVariant> &filter) {
               const QString &entityTableName = this->tableName();
               const QString &entityOrderingTableName = this->orderingTableName();
               QSqlDatabase database = QSqlDatabase::database(DatabaseTable<T>::databaseContext()->databaseName());
               QSqlQuery query(database);
...
{
    return QtConcurrent::task([this](T entity, int position, const QHash<QString, QVariant> &filter) {
               const QString &entityTableName = this->tableName();
               const QString &entityOrderingTableName = this->orderingTableName();
               QSqlDatabase database = QSqlDatabase::database(DatabaseTable<T>::databaseContext()->databaseName());
               QSqlQuery query(database);
               ...
               updateNewRowStr = QString("UPDATE %1 SET previous = %2, next = %3 WHERE ordering_id = %4")
                                     .arg(entityOrderingTableName,
                                          previousOrderingId == -1 ? "NULL" : QString::number(previousOrderingId),
                                          nextOrderingId == -1 ? "NULL" : QString::number(nextOrderingId),
                                          QString::number(newOrderingId));
        ...
        .withArguments(std::move(entity), position, filter)
        .onThreadPool(DatabaseTable<T>::databaseContext()->threadPool())
        .spawn()
        .result();
}
...
{
    return QtConcurrent::task([this, filter]() {
               const QString &entityTableName = this->tableName();
               QSqlDatabase database = QSqlDatabase::database(DatabaseTable<T>::databaseContext()->databaseName());
               const QString &entityOrderingTableName = this->orderingTableName();
               QSqlQuery query(database);
...
template <class T> Result<T> OrderedDatabaseTable<T>::remove(T &&entity)
{
    return QtConcurrent::task([this](T entity) {
               ...
               {
                   QString updatePreviousRowQueryStr =
                       QString("UPDATE %1 SET next = %2 WHERE ordering_id = %3")
                           .arg(entityOrderingTableName)
               ...
               if (nextOrderedId != -1)
               {
                   QString updateNextRowQueryStr =
                       QString("UPDATE %1 SET previous = %2 WHERE ordering_id = %3")
                           .arg(entityOrderingTableName)
...
{
    return QtConcurrent::task([this](const QList<int> &idList) {
               QMap<QString, QList<QVariantHash>> resultMap;

               // Save the main table rows
               resultMap = DatabaseTable<T>::save(idList).value();

               QSqlDatabase database = QSqlDatabase::database(this->databaseContext()->databaseName());
               ...
               {
                   queryStr =
                       "SELECT ordering_id, previous, next FROM " + orderingTableName + " WHERE ordering_id IN (%1)";
               ...
               if (!idList.isEmpty())
               {
                   ...
                   if (!query.prepare(queryStr))
                   {
                       return Result<QMap<QString, QList<QVariantHash>>>(Error(
                           "OrderedDatabaseTable", Error::Critical, "sql_error", query.lastError().text(), queryStr));
                   }
        ...
        .withArguments(idList)
        .onThreadPool(this->databaseContext()->threadPool())
        .spawn()
        .result();
}
...
{
    return QtConcurrent::task([this](int position) {
               QSqlDatabase database = QSqlDatabase::database(DatabaseTable<T>::databaseContext()->databaseName());
               const QString &entityTableName = Tools<T>::getEntityTableName();
               const QString &orderingTableName = this->orderingTableName();
               ...
               else if (rowCount > 0 && rowCount <= position)
               {
                   ...
                   QString selectNextStr =
                       QString("SELECT ordering_id FROM %1 WHERE next IS NULL").arg(orderingTableName);
               ...
               else if (position == 0)
               {
                   ...
                   QString selectPreviousStr =
                       QString("SELECT ordering_id FROM %1 WHERE previous IS NULL").arg(orderingTableName);
               ...
               {
                   QString selectPreviousStr =
                       QString("WITH RECURSIVE ordered_entities(ordering_id, next, row_number) AS ("
...
{
    return QtConcurrent::task([this](int entityId) {
               QSqlDatabase database = QSqlDatabase::database(DatabaseTable<T>::databaseContext()->databaseName());
               const QString &entityTableName = Tools<T>::getEntityTableName();
               const QString &orderingTableName = this->orderingTableName();
               ...
               QString selectPreviousAndNextStr = QString("SELECT previous, next FROM %1 WHERE ordering_id = %2")
                                                      .arg(orderingTableName)
