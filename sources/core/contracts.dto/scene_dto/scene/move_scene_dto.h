...
public:
    ...
    MoveSceneDTO() : m_origiChapterId(0), m_targetedSceneId(0), m_destinationChapterId(0), m_scenePositionAtDestination(
            0)
    {}
    ...
    MoveSceneDTO(int origiChapterId,   int targetedSceneId,   int destinationChapterId,
                 int scenePositionAtDestination)
        : m_origiChapterId(origiChapterId), m_targetedSceneId(targetedSceneId), m_destinationChapterId(
            destinationChapterId), m_scenePositionAtDestination(scenePositionAtDestination)
    {}

    MoveSceneDTO(const MoveSceneDTO& other) : m_origiChapterId(other.m_origiChapterId), m_targetedSceneId(
            other.m_targetedSceneId), m_destinationChapterId(other.m_destinationChapterId),
    ...
    friend bool operator==(const MoveSceneDTO& lhs,
                           const MoveSceneDTO& rhs);


    friend uint qHash(const MoveSceneDTO& dto,
                      uint                seed) noexcept;
...
{
    return
        lhs.m_origiChapterId == rhs.m_origiChapterId  && lhs.m_targetedSceneId == rhs.m_targetedSceneId  &&
