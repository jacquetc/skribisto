...
public:
    ...
    MoveSceneReplyDTO() : m_originChapterId(0), m_targetedSceneId(0), m_scenePositionAtOrigin(0),
    ...
    MoveSceneReplyDTO(int originChapterId,
                      int targetedSceneId,
                      int scenePositionAtOrigin,
                      int destinationChapterId,
                      int scenePositionAtDestination)
        : m_originChapterId(originChapterId), m_targetedSceneId(targetedSceneId), m_scenePositionAtOrigin(
            scenePositionAtOrigin), m_destinationChapterId(destinationChapterId), m_scenePositionAtDestination(
            scenePositionAtDestination)
    {}

    MoveSceneReplyDTO(const MoveSceneReplyDTO& other) : m_originChapterId(other.m_originChapterId), m_targetedSceneId(
            other.m_targetedSceneId), m_scenePositionAtOrigin(other.m_scenePositionAtOrigin), m_destinationChapterId(
    ...
    friend bool operator==(const MoveSceneReplyDTO& lhs,
                           const MoveSceneReplyDTO& rhs);


    friend uint qHash(const MoveSceneReplyDTO& dto,
                      uint                     seed) noexcept;
...
{
    return
        lhs.m_originChapterId == rhs.m_originChapterId  && lhs.m_targetedSceneId == rhs.m_targetedSceneId  &&
