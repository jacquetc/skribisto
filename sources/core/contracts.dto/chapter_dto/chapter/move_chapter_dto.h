...
public:
    ...
    MoveChapterDTO(int originBookId,   int targetedSceneId,   int destinationBookId,   int scenePositionAtDestination)
        : m_originBookId(originBookId), m_targetedSceneId(targetedSceneId), m_destinationBookId(destinationBookId),
        m_scenePositionAtDestination(scenePositionAtDestination)
    {}

    MoveChapterDTO(const MoveChapterDTO& other) : m_originBookId(other.m_originBookId), m_targetedSceneId(
            other.m_targetedSceneId), m_destinationBookId(other.m_destinationBookId), m_scenePositionAtDestination(
            other.m_scenePositionAtDestination)
    {}
    ...
    friend bool operator==(const MoveChapterDTO& lhs,
                           const MoveChapterDTO& rhs);


    friend uint qHash(const MoveChapterDTO& dto,
                      uint                  seed) noexcept;
...
{
    return
        lhs.m_originBookId == rhs.m_originBookId  && lhs.m_targetedSceneId == rhs.m_targetedSceneId  &&
        lhs.m_destinationBookId == rhs.m_destinationBookId  &&
