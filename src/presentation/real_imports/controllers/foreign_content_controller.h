// Generated by Qleany v1.6.1 from foreign_entity_controller_h.tera
#pragma once
#include "content/content_controller.h"
#include <QCoro/QCoroQml>
#include <QCoro/QCoroQmlTask>
#include <QQmlEngine>

struct ForeignContentController : public QObject
{
    Q_OBJECT
    QML_NAMED_ELEMENT(ContentController)
    Q_PROPERTY(int undoRedoStackId READ undoRedoStackId WRITE setUndoRedoStackId NOTIFY undoRedoStackIdChanged)

  public:
    explicit ForeignContentController(QObject *parent = nullptr)
        : QObject(parent), m_controller(new Skribisto::DirectAccess::Content::ContentController(this))

    {
    }
    Q_INVOKABLE QCoro::QmlTask get(const QList<int> &ids) const
    {
        return m_controller->get(ids);
    }

    Q_INVOKABLE static Skribisto::DirectAccess::Content::CreateContentDto getCreateDto()
    {
        return Skribisto::DirectAccess::Content::ContentController::getCreateDto();
    }

    Q_INVOKABLE QCoro::QmlTask createOrphans(
        const QList<Skribisto::DirectAccess::Content::CreateContentDto> &dtos) const
    {
        return m_controller->createOrphans(dtos);
    }

    Q_INVOKABLE QCoro::QmlTask create(const QList<Skribisto::DirectAccess::Content::CreateContentDto> &dtos,
                                      int ownerId, int index) const
    {
        return m_controller->create(dtos, ownerId, index);
    }

    Q_INVOKABLE static Skribisto::DirectAccess::Content::UpdateContentDto getUpdateDto()
    {
        return Skribisto::DirectAccess::Content::ContentController::getUpdateDto();
    }

    Q_INVOKABLE static Skribisto::DirectAccess::Content::UpdateContentDto toUpdateDto(
        const Skribisto::DirectAccess::Content::ContentDto &dto)
    {
        return Skribisto::DirectAccess::Content::UpdateContentDto(dto);
    }

    Q_INVOKABLE QCoro::QmlTask update(const QList<Skribisto::DirectAccess::Content::UpdateContentDto> &dtos) const
    {
        return m_controller->update(dtos);
    }

    Q_INVOKABLE QCoro::QmlTask updateWithRelationships(
        const QList<Skribisto::DirectAccess::Content::ContentDto> &dtos) const
    {
        return m_controller->updateWithRelationships(dtos);
    }

    Q_INVOKABLE QCoro::QmlTask remove(const QList<int> &ids) const
    {
        return m_controller->remove(ids);
    }

  Q_SIGNALS:
    void undoRedoStackIdChanged();

  private:
    void setUndoRedoStackId(int undoRedoStackId)
    {
        if (undoRedoStackId == m_controller->undoRedoStackId())
        {
            return;
        }
        m_controller->setUndoRedoStackId(undoRedoStackId);
        Q_EMIT undoRedoStackIdChanged();
    }
    int undoRedoStackId() const
    {
        return m_controller->undoRedoStackId();
    }
    Skribisto::DirectAccess::Content::ContentController *m_controller;
};