...
{
    SKRResult result("SKRSqlTools::upgradeSQLite"_L1);
    ...
    if (dbVersion == 1.0)
    {
        ...
        QString queryStr = "CREATE TABLE tbl_project_dict (l_project_dict_id INTEGER PRIMARY KEY "
        ...
        {
            result = SKRResult(SKRResult::Critical, "Upgrader::upgradeSQLite"_L1, "sql_error"_L1);
        ...
        IFKO(result)
        {
            result = SKRResult(SKRResult::Critical, "Upgrader::upgradeSQLite"_L1, "cant_upgrade"_L1);
            result.addData("dbVersion"_L1, newDbVersion);
            result.addData("SQLError"_L1, query.lastError().text());
        }

        IFOKDO(result, result = Upgrader::setDbVersion(sqlDbConnectionName, newDbVersion));
    ...
    if (dbVersion == 1.4)
    {
        ...
        IFOKDO(result, Upgrader::movePaperToTree_1_5(sqlDb, "tbl_sheet"_L1));

        // -- from tbl_note to tbl_tree
        IFOKDO(result, Upgrader::movePaperToTree_1_5(sqlDb, "tbl_note"_L1));
    ...
    if (dbVersion == 1.5)
    {
        ...
        QString queryStr = "UPDATE tbl_tree SET l_indent = l_indent + 1;"_L1;
    ...
    if (dbVersion == 1.6)
    {
        ...
        QString queryStr = "SELECT l_tree_id FROM tbl_tree WHERE t_type = 'TEXT';"_L1;
        ...
        queryStr = "SELECT l_tree_code FROM tbl_tree_property WHERE "
                   "t_name='can_add_child_paper'"_L1;
        ...
        for (int treeItemId : otherTextIdList)
        {
            queryStr = "INSERT INTO tbl_tree_property(l_tree_code, t_name, "
                       ...
                       "'can_add_child_paper', 'INT', 'false');"_L1;

            query.prepare(queryStr);
            query.bindValue(":treeItemId"_L1, treeItemId);
            query.exec();
        ...
        for (int treeItemId : qAsConst(textIdList))
        {
            ...
            queryStr = "SELECT m_primary_content FROM tbl_tree WHERE l_tree_id=:treeItemId"_L1;
            query.prepare(queryStr);
            query.bindValue(":treeItemId"_L1, treeItemId);
            query.exec();
            ...
            queryStr = "UPDATE tbl_tree SET m_primary_content=:html WHERE "
                       "l_tree_id=:treeItemId"_L1;
            query.prepare(queryStr);
            query.bindValue(":treeItemId"_L1, treeItemId);
            query.bindValue(":html"_L1, html);
            query.exec();
    ...
    if (dbVersion == 1.9)
    {
        ...
        queryStr = "SELECT l_tree_id FROM tbl_tree WHERE t_type = 'TEXT';"_L1;
        ...
        for (int treeItemId : qAsConst(textIdList))
        {
            ...
            query.prepare(queryStr);
            query.bindValue(":treeItemId"_L1, treeItemId);
            query.bindValue(":markdown"_L1, markdown);
            query.exec();
        ...
        queryStr =
            R""""(PRAGMA foreign_keys = 0;
                ...
                INSERT INTO tbl_tree (l_sort_order, l_indent, t_type, t_title, t_internal_title) VALUES (1, 1, 'FOLDER', "Trash"_L1, "trash_folder"_L1);
...
SKRResult Upgrader::setDbVersion(const QString &sqlDbConnectionName, double newVersion)
{
    SKRResult result("SKRSqlTools::setDbVersion"_L1);

    QSqlDatabase sqlDb = QSqlDatabase::database(sqlDbConnectionName);
    sqlDb.transaction();
    QSqlQuery query(sqlDb);
    QString queryStr = "UPDATE tbl_project SET dbl_database_version = :newVersion;"_L1;

    query.prepare(queryStr);
    query.bindValue(":newVersion"_L1, newVersion);
    query.exec();

    if (query.lastError().isValid())
    {
        result = SKRResult(SKRResult::Critical, "Upgrader::setDbVersion"_L1, "sql_error"_L1);
        result.addData("SQLError"_L1, query.lastError().text());
        result.addData("SQL string"_L1, queryStr);
    }
    ...
    IFOK(result)
    {
        queryStr = "UPDATE tbl_project SET dt_updated = CURRENT_TIMESTAMP;"_L1;
        query.prepare(queryStr);
        query.exec();
...
SKRResult Upgrader::movePaperToTree_1_5(QSqlDatabase sqlDb, const QString &tableName)
{
    SKRResult result("movePaperToTree_1_5"_L1);
    QString tableId;
    QString propertyTable;
    QString propertyTableCode;
    QString treeRelationshipCode;

    if (tableName == "tbl_sheet"_L1)
    {
        tableId = "l_sheet_id"_L1;
        propertyTable = "tbl_sheet_property"_L1;
        propertyTableCode = "l_sheet_code"_L1;
        treeRelationshipCode = "l_tree_receiver_code"_L1;
    }

    if (tableName == "tbl_note"_L1)
    {
        tableId = "l_note_id"_L1;
        propertyTable = "tbl_note_property"_L1;
        propertyTableCode = "l_note_code"_L1;
        treeRelationshipCode = "l_tree_source_code"_L1;
    }
    ...
    QSqlQuery query(sqlDb);
    QString queryStr = "SELECT l_sort_order FROM tbl_tree ORDER BY l_sort_order"_L1;
    ...
    if (query.lastError().isValid())
    {
        result = SKRResult(SKRResult::Critical, "Upgrader::movePaperToTree_1_5"_L1, "sql_error"_L1);
        result.addData("SQLError"_L1, query.lastError().text());
        result.addData("SQL string"_L1, queryStr);
    ...
    queryStr = "SELECT "_L1 + tableId + " FROM "_L1 + tableName + " ORDER BY l_sort_order"_L1;
    ...
    {
        queryStr = "INSERT INTO tbl_tree (t_title, l_sort_order, l_indent, t_type, "
                   "m_primary_content, dt_created, dt_updated, dt_trashed, b_trashed)"
                   "VALUES ("
                   "                       (SELECT t_title FROM "_L1 +
                   tableName + " WHERE "_L1 + tableId +
                   " = :paperId),"
                   "                       (SELECT l_sort_order FROM "_L1 +
                   tableName + " WHERE "_L1 + tableId +
                   " = :paperId) + :startingSortOrder,"
                   "                       (SELECT l_indent FROM "_L1 +
                   tableName + " WHERE "_L1 + tableId +
                   " = :paperId),"
                   "                       'TEXT',"
                   "                       (SELECT m_content FROM "_L1 +
                   tableName + " WHERE "_L1 + tableId +
                   " = :paperId),"
                   "                       (SELECT dt_created FROM "_L1 +
                   tableName + " WHERE "_L1 + tableId +
                   " = :paperId),"
                   "                       (SELECT dt_updated FROM "_L1 +
                   tableName + " WHERE "_L1 + tableId +
                   " = :paperId),"
                   "                       (SELECT dt_trashed FROM "_L1 +
                   tableName + " WHERE "_L1 + tableId +
                   " = :paperId),"
                   "                       (SELECT b_trashed FROM "_L1 +
                   tableName + " WHERE "_L1 + tableId +
                   " = :paperId)"
                   "_L1)"_L1;
        ...
        query.bindValue(":paperId"_L1, originalId);
        query.bindValue(":startingSortOrder"_L1, startingSortOrder);
        ...
        QSqlQuery query(sqlDb);
        QString queryStr =
            "SELECT l_property_id FROM "_L1 + propertyTable + " WHERE "_L1 + propertyTableCode + " = :paperId"_L1;
        ...
        for (int originalPropertyId : originalPropertyIdList)
        {
            ...
            queryStr = "INSERT INTO tbl_tree_property (l_tree_code , t_name, "
                       ...
                       "VALUES ("
                       "                       :newTreeId,"
                       "                       (SELECT t_name FROM "_L1 +
                       propertyTable +
                       " WHERE l_property_id = :propertyId),"
                       "                       'STRING',"
                       "                       (SELECT t_value FROM "_L1 +
                       propertyTable +
                       " WHERE l_property_id = :propertyId),"
                       "                       (SELECT dt_created FROM "_L1 +
                       propertyTable +
            ...
            query.bindValue(":newTreeId"_L1, newTreeId);
            query.bindValue(":propertyId"_L1, originalPropertyId);
        ...
        queryStr =
            "UPDATE tbl_tag_relationship SET l_tree_code = :newTreeId WHERE "_L1 + propertyTableCode + " = :paperId"_L1;
        ...
        queryStr = "UPDATE tbl_sheet_note SET "_L1 + treeRelationshipCode + " = :newTreeId WHERE "_L1 +
                   propertyTableCode + " = :paperId"_L1;
...
{
    SKRResult result("SKRSqlTools::transformParentsToFolder_1_5"_L1);
    ...
    if (query.lastError().isValid())
    {
        result = SKRResult(SKRResult::Critical, "Upgrader::transformParentsToFolder_1_5"_L1, "sql_error"_L1);
        result.addData("SQLError"_L1, query.lastError().text());
        result.addData("SQL string"_L1, queryStr);
    ...
    QString incrementIndentQueryStr = "UPDATE tbl_tree SET l_indent = :newIndent WHERE l_tree_id = :treeId"_L1;

    QString isItSynopsisFolderQueryStr = "SELECT l_tree_property_id FROM tbl_tree_property WHERE t_name = "
                                         "'is_synopsis_folder' AND m_value = 'true' AND l_tree_code = :treeId"_L1;
    ...
    for (int i = treeIdList.count() - 1; i >= 0; i--)
    {
        ...
        if (previousIdIndent > currentIndent)
        {
            ...
            query.bindValue(":treeId"_L1, currentTreeId);
            query.bindValue(":newSortOrder"_L1, currentSortOrder - 1);
            ...
            if (query.lastError().isValid())
            {
                ...
                result.addData("SQLError"_L1, query.lastError().text());
                result.addData("SQL string"_L1, queryStr);
                result.addData("treeId"_L1, currentTreeId);
                result.addData("newSortOrder"_L1, currentSortOrder - 1);
                sqlDb.rollback();
            ...
            query.bindValue(":treeId"_L1, currentTreeId);
            query.bindValue(":newIndent"_L1, currentIndent + 1);
            ...
            if (query.lastError().isValid())
            {
                ...
                result.addData("SQL string"_L1, queryStr);
                result.addData("treeId"_L1, currentTreeId);
                result.addData("sortOrder"_L1, currentSortOrder);
                result.addData("newIndent"_L1, currentIndent + 1);
                sqlDb.rollback();
            ...
            {
                SKRSqlTools::addStringTreeProperty(sqlDb, newFolderTreeId, "is_synopsis_folder"_L1, "true"_L1);
            }
        }
    }
    IFOK(result)
...
{
    SKRResult result("Upgrader::dropDeprecatedTables_1_5"_L1);
...
{
    SKRResult result("Upgrader::moveSynopsisToSecondaryContent_1_6"_L1);
    ...
    QSqlQuery query(sqlDb);
    QString queryStr = "SELECT l_tree_relationship_id, l_tree_source_code, l_tree_receiver_code "
                       "FROM tbl_tree_relationship WHERE b_synopsis = 1"_L1;
    ...
    if (query.lastError().isValid())
    {
        result = SKRResult(SKRResult::Critical, "Upgrader::moveSynopsisToSecondaryContent_1_6"_L1, "sql_error"_L1);
        result.addData("SQLError"_L1, query.lastError().text());
        result.addData("SQL string"_L1, queryStr);
    ...
    for (int i = 0; i < synopsisRelationshipIdList.count(); i++)
    {
        queryStr = "UPDATE tbl_tree SET m_secondary_content = (SELECT "
                   ...
                   ":synopsisTreeId) WHERE l_tree_id = :textId"_L1;

        query.prepare(queryStr);
        query.bindValue(":textId"_L1, textIdList.at(i));
        query.bindValue(":synopsisTreeId"_L1, synopsisTreeIdList.at(i));
        ...
        queryStr = "DELETE FROM tbl_tree_relationship WHERE l_tree_relationship_id "
                   "= :synopsisRelationshipId"_L1;
        query.prepare(queryStr);
        query.bindValue(":synopsisRelationshipId"_L1, synopsisRelationshipId);
        ...
        queryStr = "DELETE FROM tbl_tree WHERE l_tree_id = :synopsisTreeId"_L1;
        query.prepare(queryStr);
        query.bindValue(":synopsisTreeId"_L1, synopsisTreeIdList.at(i));
    ...
    queryStr = "SELECT l_tree_code FROM tbl_tree_property WHERE t_name = "
               "'is_synopsis_folder' AND m_value = 'true'"_L1;
    ...
    {
        queryStr = "DELETE FROM tbl_tree WHERE l_tree_id = :synopsisFolderId"_L1;
        query.prepare(queryStr);

        query.bindValue(":synopsisFolderId"_L1, synopsisFolderId);
...
{
    SKRResult result("Upgrader::moveTrashedItemsToTrashFolder_2_0"_L1);

    QSqlQuery query(sqlDb);
    QString queryStr = "SELECT l_tree_id, l_sort_order, l_indent "
                       "FROM tbl_tree WHERE b_trashed = 1 ORDER BY l_sort_order"_L1;
    ...
    if (query.lastError().isValid())
    {
        result = SKRResult(SKRResult::Critical, "Upgrader::moveTrashedItemsToTrashFolder_2_0"_L1, "sql_error"_L1);
        result.addData("SQLError"_L1, query.lastError().text());
        result.addData("SQL string"_L1, queryStr);
    ...
    queryStr = "SELECT l_sort_order "
               "FROM tbl_tree WHERE t_internal_title = :internal_title"_L1;

    query.prepare(queryStr);
    query.bindValue(":internal_title"_L1, "trash_folder"_L1);
    query.exec();
    ...
    while (!groupedIds.isEmpty())
    {
        ...
        while (!idGroup.isEmpty())
        {
            ...
            queryStr =
                "UPDATE tbl_tree SET l_sort_order = :sort_order, l_indent = :indent WHERE l_tree_id = :treeId"_L1;

            query.prepare(queryStr);
            query.bindValue(":treeId"_L1, treeId);
            query.bindValue(":sort_order"_L1, newSortOrder);
            query.bindValue(":indent"_L1, indent - groupIndentDelta);
            ...
            {
                result =
                    SKRResult(SKRResult::Critical, "Upgrader::moveTrashedItemsToTrashFolder_2_0"_L1, "sql_error"_L1);
                result.addData("SQLError"_L1, query.lastError().text());
                result.addData("SQL string"_L1, queryStr);
                sqlDb.rollback();
