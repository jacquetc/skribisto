 ...
 */
#define IFKO(RESULT) if (Q_UNLIKELY(!RESULT))

/**
 * To facilitate the result management
 */
#define IFOK(RESULT) if (Q_LIKELY(RESULT))
...
struct SKRResult
{
  ...
  public:
    ...
    SKRResult(SKRResult::Status status, const QObject *object, const QString &errorCodeEnd);
    SKRResult(SKRResult::Status status, const QString &className, const QString &errorCodeEnd);
