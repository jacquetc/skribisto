 ...
 * @param commit
 */
PLMDbTree::PLMDbTree(const QSqlDatabase& sqlDb,
...
//            + " and"
    ...
    while (query.next()) {
        ...
        PLMDbPaper paper(m_sqlDb, m_tableName, m_idName, query.value(m_idName).toInt(),
                         false);
...
//
// If i_after_note is 0, then move the notes to the top. -1 = move notes to
// bottom
//
// The notes are moved in list order such at the first in the list ends up as
// the first after i_after_note.
//
// To move, we renumber notes from i_dest + 1. The max notes we can move is
// RENUM_INT -1.
//
{
    if (m_commit) m_sqlDb.transaction();
    ...
    if (idList.count() >= m_renumInterval) {
        m_error.setStatus(PLMDbError::InternalError, 0,
                          QString("Can't move more than %1 notes").arg(QString::number(
                                                                           m_renumInterval
                                                                           -
                                                                           1)));
    ...
    if (paperId == 0) dest = 0;
    else if (paperId == -1) dest = INT_MAX - 1000;
    else {
        PLMDbPaper paper(m_sqlDb, m_tableName, m_idName, paperId, false);
        dest = paper.getSortOrder();

        if (dest == PLMDbError::Error) {
            m_error.setStatus(PLMDbError::InvalidParameter, 2,
                              QString("After Item %1 not found").arg(QString::number(
                                                                         paperId)));
            return PLMDbError::Error;
        }
    }
    ...
    for(int id : idList) {
        ...
        if (!paper.exists()) {
            m_sqlDb.rollback();
            m_error.setStatus(PLMDbError::InvalidParameter, 1,
                              QString("Sheet %1 not found").arg("%1",
                                                                QString::number(id)));
            return PLMDbError::Error;
        }
...
//
// Copy the list of sheets. Each sheet is copied directly after its original
// sheet.
//
{
    if (m_commit) m_sqlDb.transaction();
...
int PLMDbTree::getPaperAbove(int paperId)
{
    ...
    if (!paper.exists()) {
        m_sqlDb.rollback();
        m_error.setStatus(PLMDbError::InvalidParameter,
                          1,
                          QString("Sheet (%1) not found").arg(QString::number(paperId)));
        return PLMDbError::Error;
    }
...
int PLMDbTree::getPaperBelow(int paperId)
{
    ...
    if (paper.getSortOrder() == listAllIds().last()) return -1;
