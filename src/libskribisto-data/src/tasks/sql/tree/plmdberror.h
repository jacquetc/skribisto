...
public:
    ...
    enum ReturnError {
        Error = 0, // Error return - check get_status(). MUST BE ZERO e.g.
                   // get_sheet_order
        OK    = 1  // Success return (some functions return another non zero
                   // value e.g. a sheet_id)
    };
    ...
    QString getStatus() const
    {
        ...
        if (status ==
            InvalidType) s_msg = QString("Invalid parameter type (parameter %1)").arg(QString::number(
                                                                                          parameter));
