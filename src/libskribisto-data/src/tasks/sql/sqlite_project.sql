...
-- Table: tbl_project
CREATE TABLE tbl_project (l_project_id INTEGER PRIMARY KEY AUTOINCREMENT UNIQUE, t_project_name TEXT, l_skribisto_maj_version INTEGER, l_skribisto_min_version INTEGER, l_db_maj_version INTEGER, l_db_min_version INTEGER, dt_created DATETIME, dt_updated DATETIME, t_author TEXT);
...
CREATE TABLE tbl_sheet_property (l_property_id INTEGER PRIMARY KEY AUTOINCREMENT UNIQUE, l_sheet_code INTEGER REFERENCES tbl_sheet (l_sheet_id) ON DELETE CASCADE, t_name TEXT, t_value TEXT, dt_created DATETIME NOT NULL DEFAULT (CURRENT_TIMESTAMP), dt_updated DATETIME DEFAULT (CURRENT_TIMESTAMP) NOT NULL, b_system DEFAULT (0) NOT NULL);
