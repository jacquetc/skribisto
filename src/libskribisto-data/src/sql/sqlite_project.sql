--
-- File generated with SQLiteStudio v3.3.3 on mar. oct. 18 11:31:41 2022
--
-- Text encoding used: UTF-8
--
-- skribisto_db_version:2.0
...
-- Table: tbl_tag
CREATE TABLE tbl_tag (l_tag_id INTEGER PRIMARY KEY ON CONFLICT ROLLBACK AUTOINCREMENT UNIQUE ON CONFLICT ROLLBACK NOT NULL ON CONFLICT ROLLBACK, t_name TEXT UNIQUE ON CONFLICT ROLLBACK, t_color TEXT, t_text_color TEXT, dt_created DATETIME NOT NULL ON CONFLICT ROLLBACK DEFAULT (CURRENT_TIMESTAMP), dt_updated DATETIME NOT NULL ON CONFLICT ROLLBACK DEFAULT (CURRENT_TIMESTAMP));

-- Table: tbl_tag_relationship
CREATE TABLE tbl_tag_relationship (l_tag_relationship_id INTEGER PRIMARY KEY ON CONFLICT ROLLBACK AUTOINCREMENT NOT NULL ON CONFLICT ROLLBACK, l_tree_code INTEGER REFERENCES tbl_tree (l_tree_id) ON DELETE CASCADE ON UPDATE CASCADE, l_tag_code INTEGER REFERENCES tbl_tag (l_tag_id) ON DELETE CASCADE ON UPDATE CASCADE, dt_created DATETIME NOT NULL ON CONFLICT ROLLBACK DEFAULT (CURRENT_TIMESTAMP));
