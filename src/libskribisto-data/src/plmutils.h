...
#define ENUM_TO_STRING(c, e, v)                                                             \
    (c::staticMetaObject.enumerator(c::staticMetaObject.indexOfEnumerator(# e)).valueToKey( \
         v))

// c = class name; e = enum name; s = string value
#define STRING_TO_ENUM(c, e, s)                                                        \
    (c::e)(c::staticMetaObject.enumerator(c::staticMetaObject.indexOfEnumerator(# e)). \
           keyToValue(s.toAscii()))

// c = class name; f = flag name, v = flag value
#define FLAG_TO_STRING(c, f, v)                                                  \
    (c::staticMetaObject.enumerator(c::staticMetaObject.indexOfEnumerator(# f)). \
     valueToKeys(v))

// c = class name; f = flag name; s = string value
#define STRING_TO_FLAG(c, f, s)                                                        \
    (c::f)(c::staticMetaObject.enumerator(c::staticMetaObject.indexOfEnumerator(# f)). \
           keysToValue(s.toAscii()))
...
//    static void applyAttributeRecursively(QDomElement element, QString
...
public:
    ...
    static QModelIndexList allChildIndexes(QModelIndex index);  // maybe useless
    static QModelIndexList allParentIndexes(QModelIndex index); // maybe useless
};
...
public:
    ...
    static bool                   modifyProjectModifiedDateInSettingsArray(
        int     arrayNumber,
        QString date);
    ...
    static QHash<QString, QString>listAllProjectsInSettingsArray(); // QHash<
                                                                    // path,
                                                                    // name>
};
