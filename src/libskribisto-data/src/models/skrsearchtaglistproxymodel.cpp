...
SKRSearchTagListProxyModel::SKRSearchTagListProxyModel(QObject *parent) : QSortFilterProxyModel(parent),
    m_textFilter(""), m_projectIdFilter(-2), m_sheetIdFilter(-2), m_noteIdFilter(-2)
{
    this->setSourceModel(plmmodels->tagListModel());


    connect(plmdata->projectHub(), &PLMProjectHub::projectLoaded, this, &SKRSearchTagListProxyModel::loadProjectSettings);
    connect(plmdata->projectHub(), &PLMProjectHub::projectToBeClosed, this, &SKRSearchTagListProxyModel::saveProjectSettings, Qt::DirectConnection);
...
bool SKRSearchTagListProxyModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    QModelIndex sourceIndex = this->mapToSource(index);
...
int SKRSearchTagListProxyModel::findVisualIndex(int projectId, int paperId)
{
    ...
    for(int i = 0; i < rowCount ; ++i){
        ...
        if(this->data(modelIndex, SKRTagItem::Roles::ProjectIdRole).toInt() == projectId
                && this->data(modelIndex, SKRTagItem::Roles::TagIdRole).toInt() == paperId){
            result = i;
            break;
        }
...
bool SKRSearchTagListProxyModel::filterAcceptsRow(int sourceRow, const QModelIndex &sourceParent) const
{
    bool result = true;
    ...
    // project filtering :
    if(result && item->data(SKRTagItem::Roles::ProjectIdRole).toInt() == m_projectIdFilter){
        result = true;
    }
    else  if(result){
        result = false;
    ...
    if(m_sheetIdFilter != -2 ^ m_noteIdFilter != -2){
        if(result && m_relationshipList.contains(item->data(SKRTagItem::Roles::TagIdRole).toInt())){
            result = true;
        }
        else  if(result){
            result = false;
    ...
    if(result && item->data(SKRTagItem::Roles::NameRole).toString().contains(m_textFilter, Qt::CaseInsensitive)){
        result = true;
    }
    else  if(result){
        result = false;
...
void SKRSearchTagListProxyModel::populateRelationshipList()
{
    ...
    if(m_sheetIdFilter != -2){
        m_relationshipList.append(plmdata->tagHub()->getTagsFromItemId(m_projectIdFilter, SKRTagHub::ItemType::Sheet, m_sheetIdFilter));
    }
    if(m_noteIdFilter != -2){
        m_relationshipList.append(plmdata->tagHub()->getTagsFromItemId(m_projectIdFilter, SKRTagHub::ItemType::Note, m_noteIdFilter));
    }
}
