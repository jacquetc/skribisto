...
void SpellChecker::addWordToDict(const QString &word, const QString &affix)
{
    ...
    else
    {
        ...
        if (m_encodingFix == "latin1")
            m_hunspell->add_with_affix(word.toLatin1().toStdString(), affix.toLatin1().toStdString());

        if (m_encodingFix == "utf8")
            m_hunspell->add_with_affix(word.toUtf8().toStdString(), affix.toUtf8().toStdString());
    }
}
...
QMap<QString, QString> SpellChecker::dictAndPathMap()
{
    ...
    for (const QString &path : SpellChecker::dictsPaths())
    {
        ...
        for (QString dict : dir.entryList(filters, QDir::Files))
        {
            ...
            if (QFile(possiblePath).exists())
                map.insert(dict, path + "/" + dict);
        }
    }
