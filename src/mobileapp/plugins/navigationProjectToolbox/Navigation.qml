...
NavigationForm {
    ...
    Component {
        ...
        TrashedListView {
            ...
            Component.onCompleted: {
                ...
                trashedListView.openDocumentInAnotherView.connect(
                            root.openDocumentInAnotherView)
                trashedListView.openDocumentInNewWindow.connect(
                            root.openDocumentInNewWindow)
                trashedListView.restoreDocument.connect(
                            root.defineWhichPossibleDocumentsToRestore)
    ...
    function defineWhichPossibleDocumentsToRestore(projectId, treeItemId) {
        ...
        // if get children :
        var trashedChildrenList = skrData.treeHub().getAllChildren(projectId,
                                                                   treeItemId)
        var trashedAncestorsList = skrData.treeHub().getAllAncestors(projectId,
                                                                     treeItemId)
        // if no children :
        if (trashedChildrenList.length === 0 && trashedAncestorsList === 0) {
            ...
            pushRestoreListView(projectId, treeItemId,
                                trashedChildrenList.concat(
                                    trashedAncestorsList))
        }
    }
    ...
    Component {
        ...
        RestoreListView {
            ...
            Component.onCompleted: {
                ...
                restoreListView.openDocumentInAnotherView.connect(
                            root.openDocumentInAnotherView)
                restoreListView.openDocumentInNewWindow.connect(
                            root.openDocumentInNewWindow)
    ...
    function pushRestoreListView(projectId, parentTreeItemIdToBeRestored, trashedChildrenList) {
        ...
        var treeIndentOffset = skrData.treeHub().getIndent(
                    projectId, parentTreeItemIdToBeRestored)

        stackView.push(restoreListViewComponent, {
