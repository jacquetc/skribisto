...
TagPadForm {
    ...
    tagFlow.onActiveFocusChanged: {
        if (tagFlow.children.length > 1) {
            // means there is no children
    ...
    Binding on tagFlickable.contentY {
        value: tagFlow.height < tagFlickable.height ? -(tagFlickable.height / 2
                                                        - tagFlow.height / 2) : 0
        delayed: true
    ...
    Connections {
        ...
        function onCallAddTagRelationship(projectId, treeItemId, tagName, colorCode, textColorCode) {
            ...
            if (colorCode !== skrData.tagHub().getTagColor(projectId, tagId)) {
                ...
                skrData.tagHub().setTagTextColor(projectId, tagId,
                                                 textColorCode)
            }
        ...
        Rectangle {
                ...
                SkrMenuItem {
                    ...
                    onTriggered: {
                        if (treeItemId === -2) {
                            ...
                            console.log(model.projectId, root.treeItemId,
                                        "tag", model.tagId)
                            callRemoveTagRelationship(model.projectId,
                                                      root.treeItemId,
                                                      model.tagId)
                        }
                    }
            ...
            SimpleDialog {
                ...
                text: qsTr("Do you want to delete the tag \"%1\" ?\n%2").arg(
                          tagName).arg(relatedItemNames)
                standardButtons: Dialog.Yes | Dialog.Cancel

                onOpened: {
                    var list = skrData.tagHub().getItemIdsFromTag(projectId,
                                                                  tagId)
                    ...
                    for (i = 0; i < list.length; i++) {
                        ...
                        //                            continue
                        //                        }
                        relatedItemNames += "- " + skrData.treeHub().getTitle(
                                    projectId, list[i]) + "\n"
            ...
            Keys.onPressed: function (event) {
                if (event.key === Qt.Key_Delete) {
                    ...
                    if (treeItemId === -2) {
                        ...
                        // remove relationship
                        callRemoveTagRelationship(projectId,
                                                  root.treeItemId, tagId)
                    }
                }
                ...
                if (event.key === Qt.Key_Left || event.key === Qt.Key_Up) {
                    ...
                    if (model.index === 0) {
                        tagRepeater.itemAt(
                                    tagRepeater.count - 1).forceActiveFocus()
            ...
            RowLayout {
                ...
                state: hoverHandler.hovered
                       && !showRemoveRelationshipButtonTimer.running ? "visible_removeRelationshipButton" : ""
                ...
                states: [
                    State {
                        ...
                        PropertyChanges {
                            target: removeRelationshipButton
                            width: tagTitle.height
                        }
                        PropertyChanges {
                            target: removeRelationshipButton
                            opacity: 1.0
            ...
            RowLayout {
                ...
                ColumnLayout {
                    ...
                    SkrTextField {
                        ...
                        onAccepted: {
                            if (treeItemId === -2) {
                                callAddTag(projectId,
                                           inner_titleTextField.text,
                                           inner_colorChooser.colorCode,
                                           inner_colorChooser.textColorCode)
                            } else {
                                callAddTagRelationship(
                        ...
                        Keys.onPressed: function (event) {
                            if (event.key === Qt.Key_Down) {
                                if (inner_searchResultList.count > 0) {
                                    inner_searchResultList.itemAtIndex(
                                                0).forceActiveFocus()
                                }
                            }
                        }
            ...
            ColumnLayout {
                ...
                SkrTextField {
                    ...
                    onAccepted: {
                        ...
                        //create basic note
                        var result = skrData.tagHub().setTagName(
                                    renamePopup.projectId, renamePopup.tagId,
                                    inner_titleTextField.text)
                        if (!result.success) {
                            //TODO: add notification
                            return
