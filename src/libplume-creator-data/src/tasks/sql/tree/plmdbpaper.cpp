...
PLMDbPaper::PLMDbPaper(const QSqlDatabase &sqlDb, const QString &tableName, const QString &idName, int paperId, bool commit)
{
    m_sqlDb = sqlDb;
    m_paperId = paperId;
    m_commit = commit;
...
void PLMDbPaper::set(const QString &valueName, const QVariant &value, bool updatedDateStampSet)
{
    if(m_commit)
        m_sqlDb.transaction();

    QString updatedDateStamp = " dt_updated = CURRENT_TIMESTAMP";
    if(!updatedDateStampSet)
        updatedDateStamp = "";
...
//      qDebug() << " : " << getLastExecutedQuery(query);
    ...
    QList<int> finalList;
    while (query.next()) {
        if( query.value("l_indent").toInt() <= parentIndent)
            break;
