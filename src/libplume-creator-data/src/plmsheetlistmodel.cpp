    ...
    : QAbstractListModel(parent), m_headerData(QVariant())
{
    ...
    connect(plmdata->projectHub(), &PLMProjectHub::projectLoaded, this, &PLMSheetListModel::populate);
    connect(plmdata->projectHub(), &PLMProjectHub::projectClosed, this, &PLMSheetListModel::populate);
}

QVariant PLMSheetListModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    return m_headerData;
}

bool PLMSheetListModel::setHeaderData(int section, Qt::Orientation orientation, const QVariant &value, int role)
{
...
int PLMSheetListModel::rowCount(const QModelIndex &parent) const
{
    ...
    return plmdata->sheetHub()->getOverallSize();
