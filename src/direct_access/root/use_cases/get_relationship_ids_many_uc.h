...
{
  public:
    explicit GetRelationshipIdsManyUseCase(std::unique_ptr<IRootUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~GetRelationshipIdsManyUseCase() = default;

    QHash<int, QList<int>> execute(const QList<int> &rootIds, RootRelationshipField relationship);
