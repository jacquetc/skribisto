...
{
  public:
    explicit GetRelationshipIdsCountUseCase(std::unique_ptr<IRootUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~GetRelationshipIdsCountUseCase() = default;

    int execute(int rootId, RootRelationshipField relationship);
