...
void SetRelationshipIdsUseCase::execute(int rootId, RootRelationshipField relationship, const QList<int> &relatedIds)
{
    if (m_hasExecuted)
    {
    ...
    m_originalRelatedIds = m_uow->getRootRelationship(rootId, DtoMapper::toCommonRelationshipField(relationship));

    // Set the new relationship IDs
    m_uow->setRootRelationship(rootId, DtoMapper::toCommonRelationshipField(relationship), relatedIds);

    // update date

    auto root = m_uow->getRoot({rootId}).at(0); // we are sure that it exists
    root.updatedAt = QDateTime::currentDateTimeUtc();
    m_uow->updateRoot({root});
...
SCU::Result<void> SetRelationshipIdsUseCase::undo()
{
    ...
    try
    {
        ...
        m_uow->setRootRelationship(m_rootId, DtoMapper::toCommonRelationshipField(m_relationship),
...
SCU::Result<void> SetRelationshipIdsUseCase::redo()
{
    ...
    try
    {
        ...
        m_uow->setRootRelationship(m_rootId, DtoMapper::toCommonRelationshipField(m_relationship), m_newRelatedIds);
