...
{
  public:
    explicit GetRelationshipIdsInRangeUseCase(std::unique_ptr<IRootUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~GetRelationshipIdsInRangeUseCase() = default;

    QList<int> execute(int rootId, RootRelationshipField relationship, int offset, int limit);
