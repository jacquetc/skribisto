...
class RemoveBinderTagUseCase
{
  public:
    explicit RemoveBinderTagUseCase(std::unique_ptr<IBinderTagUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~RemoveBinderTagUseCase() = default;

    QList<int> execute(const QList<int> &binderTagIds);
    SCU::Result<void> undo();
    SCU::Result<void> redo();

  private:
    std::unique_ptr<IBinderTagUnitOfWork> m_uow;
    QList<int> m_originalBinderTagIds;
    QList<BinderTagDto> m_removedBinderTags;
