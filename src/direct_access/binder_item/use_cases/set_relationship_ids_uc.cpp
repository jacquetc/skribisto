...
void SetRelationshipIdsUseCase::execute(int binderItemId, BinderItemRelationshipField relationship,
                                        const QList<int> &relatedIds)
{
    ...
    m_originalRelatedIds =
        m_uow->getBinderItemRelationship(binderItemId, DtoMapper::toCommonRelationshipField(relationship));

    // Set the new relationship IDs
    m_uow->setBinderItemRelationship(binderItemId, DtoMapper::toCommonRelationshipField(relationship), relatedIds);

    // update date

    auto binderItem = m_uow->getBinderItem({binderItemId}).at(0); // we are sure that it exists
    binderItem.updatedAt = QDateTime::currentDateTimeUtc();
    m_uow->updateBinderItem({binderItem});
...
SCU::Result<void> SetRelationshipIdsUseCase::undo()
{
    ...
    try
    {
        ...
        m_uow->setBinderItemRelationship(m_binderItemId, DtoMapper::toCommonRelationshipField(m_relationship),
                                         m_originalRelatedIds);
