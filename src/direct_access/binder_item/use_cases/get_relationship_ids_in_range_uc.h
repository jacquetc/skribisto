...
{
  public:
    explicit GetRelationshipIdsInRangeUseCase(std::unique_ptr<IBinderItemUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~GetRelationshipIdsInRangeUseCase() = default;

    QList<int> execute(int binderItemId, BinderItemRelationshipField relationship, int offset, int limit);
