...
{
  public:
    explicit GetRelationshipIdsCountUseCase(std::unique_ptr<IBinderUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~GetRelationshipIdsCountUseCase() = default;

    int execute(int binderId, BinderRelationshipField relationship);
