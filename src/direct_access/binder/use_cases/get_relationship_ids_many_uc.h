...
{
  public:
    explicit GetRelationshipIdsManyUseCase(std::unique_ptr<IBinderUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~GetRelationshipIdsManyUseCase() = default;

    QHash<int, QList<int>> execute(const QList<int> &binderIds, BinderRelationshipField relationship);
