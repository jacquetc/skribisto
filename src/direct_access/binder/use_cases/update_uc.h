...
class UpdateBinderUseCase
{
  public:
    explicit UpdateBinderUseCase(std::unique_ptr<IBinderUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~UpdateBinderUseCase() = default;

    QList<BinderDto> execute(const QList<BinderDto> &binders);
    SCU::Result<void> undo();
    SCU::Result<void> redo();

  private:
    std::unique_ptr<IBinderUnitOfWork> m_uow;
    QList<BinderDto> m_originalBinders;
    QList<BinderDto> m_updatedBinders;
