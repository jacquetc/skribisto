...
void SetRelationshipIdsUseCase::execute(int binderId, BinderRelationshipField relationship,
                                        const QList<int> &relatedIds)
{
    ...
    m_originalRelatedIds = m_uow->getBinderRelationship(binderId, DtoMapper::toCommonRelationshipField(relationship));

    // Set the new relationship IDs
    m_uow->setBinderRelationship(binderId, DtoMapper::toCommonRelationshipField(relationship), relatedIds);

    // update date

    auto binder = m_uow->getBinder({binderId}).at(0); // we are sure that it exists
    binder.updatedAt = QDateTime::currentDateTimeUtc();
...
SCU::Result<void> SetRelationshipIdsUseCase::undo()
{
    ...
    try
    {
        ...
        m_uow->setBinderRelationship(m_binderId, DtoMapper::toCommonRelationshipField(m_relationship),
...
SCU::Result<void> SetRelationshipIdsUseCase::redo()
{
    ...
    try
    {
        ...
        m_uow->setBinderRelationship(m_binderId, DtoMapper::toCommonRelationshipField(m_relationship), m_newRelatedIds);
