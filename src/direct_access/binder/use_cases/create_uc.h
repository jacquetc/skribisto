...
{
  public:
    explicit CreateBinderUseCase(std::unique_ptr<IBinderUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~CreateBinderUseCase() = default;

    QList<BinderDto> execute(const QList<CreateBinderDto> &binders);
    SCU::Result<void> undo();
    SCU::Result<void> redo();

  private:
    std::unique_ptr<IBinderUnitOfWork> m_uow;
    QList<CreateBinderDto> m_originalBinders;
    QList<BinderDto> m_createdBinders;
