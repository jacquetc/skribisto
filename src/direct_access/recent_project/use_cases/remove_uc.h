...
class RemoveRecentProjectUseCase
{
  public:
    explicit RemoveRecentProjectUseCase(std::unique_ptr<IRecentProjectUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~RemoveRecentProjectUseCase() = default;

    QList<int> execute(const QList<int> &recentProjectIds);
    SCU::Result<void> undo();
    SCU::Result<void> redo();

  private:
    std::unique_ptr<IRecentProjectUnitOfWork> m_uow;
    QList<int> m_originalRecentProjectIds;
    QList<RecentProjectDto> m_removedRecentProjects;
