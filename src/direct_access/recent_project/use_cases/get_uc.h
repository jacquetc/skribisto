...
{
  public:
    explicit GetRecentProjectUseCase(std::unique_ptr<IRecentProjectUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~GetRecentProjectUseCase() = default;

    QList<RecentProjectDto> execute(const QList<int> &recentProjectIds);
