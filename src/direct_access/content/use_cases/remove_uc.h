...
{
  public:
    explicit RemoveContentUseCase(std::unique_ptr<IContentUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~RemoveContentUseCase() = default;

    QList<int> execute(const QList<int> &contentIds);
    SCU::Result<void> undo();
    SCU::Result<void> redo();

  private:
    std::unique_ptr<IContentUnitOfWork> m_uow;
    QList<int> m_originalContentIds;
    QList<ContentDto> m_removedContents;
