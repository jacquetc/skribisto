...
{
  public:
    explicit UpdateContentUseCase(std::unique_ptr<IContentUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~UpdateContentUseCase() = default;

    QList<ContentDto> execute(const QList<ContentDto> &contents);
    SCU::Result<void> undo();
    SCU::Result<void> redo();

  private:
    std::unique_ptr<IContentUnitOfWork> m_uow;
    QList<ContentDto> m_originalContents;
    QList<ContentDto> m_updatedContents;
