...
class DtoMapper final
{
  public:
    ...
    static SCE::Work toEntity(const UpdateWorkDto &dto)
    {
        return SCE::Work{
            .id = dto.id,
            .createdAt = dto.createdAt,
    ...
    static QList<SCE::Work> toEntityList(const QList<UpdateWorkDto> &dtos)
    {
        QList<SCE::Work> entities;
        entities.reserve(dtos.size());
        std::ranges::transform(dtos, std::back_inserter(entities),
                               [](const UpdateWorkDto &dto) { return toEntity(dto); });
        return entities;
    }
