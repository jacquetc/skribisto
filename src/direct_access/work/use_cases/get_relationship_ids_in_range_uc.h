...
{
  public:
    explicit GetRelationshipIdsInRangeUseCase(std::unique_ptr<IWorkUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~GetRelationshipIdsInRangeUseCase() = default;

    QList<int> execute(int workId, WorkRelationshipField relationship, int offset, int limit);
