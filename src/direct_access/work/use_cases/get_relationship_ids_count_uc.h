...
{
  public:
    explicit GetRelationshipIdsCountUseCase(std::unique_ptr<IWorkUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~GetRelationshipIdsCountUseCase() = default;

    int execute(int workId, WorkRelationshipField relationship);
