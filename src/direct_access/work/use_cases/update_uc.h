...
{
  public:
    explicit UpdateWorkUseCase(std::unique_ptr<IWorkUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~UpdateWorkUseCase() = default;

    QList<WorkDto> execute(const QList<WorkDto> &works);
    SCU::Result<void> undo();
    SCU::Result<void> redo();

  private:
    std::unique_ptr<IWorkUnitOfWork> m_uow;
    QList<WorkDto> m_originalWorks;
    QList<WorkDto> m_updatedWorks;
