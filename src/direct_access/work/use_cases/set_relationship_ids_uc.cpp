...
void SetRelationshipIdsUseCase::execute(int workId, WorkRelationshipField relationship, const QList<int> &relatedIds)
{
    if (m_hasExecuted)
    {
    ...
    m_originalRelatedIds = m_uow->getWorkRelationship(workId, DtoMapper::toCommonRelationshipField(relationship));

    // Set the new relationship IDs
    m_uow->setWorkRelationship(workId, DtoMapper::toCommonRelationshipField(relationship), relatedIds);

    // update date

    auto work = m_uow->getWork({workId}).at(0); // we are sure that it exists
    work.updatedAt = QDateTime::currentDateTimeUtc();
    m_uow->updateWork({work});
...
SCU::Result<void> SetRelationshipIdsUseCase::undo()
{
    ...
    try
    {
        ...
        m_uow->setWorkRelationship(m_workId, DtoMapper::toCommonRelationshipField(m_relationship),
...
SCU::Result<void> SetRelationshipIdsUseCase::redo()
{
    ...
    try
    {
        ...
        m_uow->setWorkRelationship(m_workId, DtoMapper::toCommonRelationshipField(m_relationship), m_newRelatedIds);
