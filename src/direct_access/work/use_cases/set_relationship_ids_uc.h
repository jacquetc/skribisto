...
{
  public:
    explicit SetRelationshipIdsUseCase(std::unique_ptr<IWorkUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~SetRelationshipIdsUseCase() = default;

    void execute(int workId, WorkRelationshipField relationship, const QList<int> &relatedIds);
  ...
  private:
    ...
    int m_workId = 0;
    WorkRelationshipField m_relationship;
