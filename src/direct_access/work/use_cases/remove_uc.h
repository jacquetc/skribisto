...
{
  public:
    explicit RemoveWorkUseCase(std::unique_ptr<IWorkUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~RemoveWorkUseCase() = default;

    QList<int> execute(const QList<int> &workIds);
    SCU::Result<void> undo();
    SCU::Result<void> redo();

  private:
    std::unique_ptr<IWorkUnitOfWork> m_uow;
    QList<int> m_originalWorkIds;
    QList<WorkDto> m_removedWorks;
