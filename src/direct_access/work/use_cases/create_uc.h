...
{
  public:
    explicit CreateWorkUseCase(std::unique_ptr<IWorkUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~CreateWorkUseCase() = default;

    QList<WorkDto> execute(const QList<CreateWorkDto> &works);
    SCU::Result<void> undo();
    SCU::Result<void> redo();

  private:
    std::unique_ptr<IWorkUnitOfWork> m_uow;
    QList<CreateWorkDto> m_originalWorks;
    QList<WorkDto> m_createdWorks;
