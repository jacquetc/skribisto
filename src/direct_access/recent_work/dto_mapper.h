...
class DtoMapper final
{
  public:
    ...
    static SCE::RecentWork toEntity(const UpdateRecentWorkDto &dto)
    {
        return SCE::RecentWork{
            .id = dto.id,
            .createdAt = dto.createdAt,
    ...
    static QList<SCE::RecentWork> toEntityList(const QList<UpdateRecentWorkDto> &dtos)
    {
        QList<SCE::RecentWork> entities;
        entities.reserve(dtos.size());
        std::ranges::transform(dtos, std::back_inserter(entities),
                               [](const UpdateRecentWorkDto &dto) { return toEntity(dto); });
        return entities;
    }
