...
class RemoveRecentWorkUseCase
{
  public:
    explicit RemoveRecentWorkUseCase(std::unique_ptr<IRecentWorkUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~RemoveRecentWorkUseCase() = default;

    QList<int> execute(const QList<int> &recentWorkIds);
    SCU::Result<void> undo();
    SCU::Result<void> redo();

  private:
    std::unique_ptr<IRecentWorkUnitOfWork> m_uow;
    QList<int> m_originalRecentWorkIds;
    QList<RecentWorkDto> m_removedRecentWorks;
