...
{
  public:
    explicit UpdateRecentWorkUseCase(std::unique_ptr<IRecentWorkUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~UpdateRecentWorkUseCase() = default;

    QList<RecentWorkDto> execute(const QList<RecentWorkDto> &recentWorks);
    SCU::Result<void> undo();
    SCU::Result<void> redo();

  private:
    std::unique_ptr<IRecentWorkUnitOfWork> m_uow;
    QList<RecentWorkDto> m_originalRecentWorks;
    QList<RecentWorkDto> m_updatedRecentWorks;
