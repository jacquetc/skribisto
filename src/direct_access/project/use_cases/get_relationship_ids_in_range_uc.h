...
{
  public:
    explicit GetRelationshipIdsInRangeUseCase(std::unique_ptr<IProjectUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~GetRelationshipIdsInRangeUseCase() = default;

    QList<int> execute(int projectId, ProjectRelationshipField relationship, int offset, int limit);
