...
{
  public:
    explicit GetRelationshipIdsCountUseCase(std::unique_ptr<IProjectUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~GetRelationshipIdsCountUseCase() = default;

    int execute(int projectId, ProjectRelationshipField relationship);
