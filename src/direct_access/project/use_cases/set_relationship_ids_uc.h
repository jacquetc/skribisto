...
{
  public:
    explicit SetRelationshipIdsUseCase(std::unique_ptr<IProjectUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~SetRelationshipIdsUseCase() = default;

    void execute(int projectId, ProjectRelationshipField relationship, const QList<int> &relatedIds);
  ...
  private:
    ...
    int m_projectId = 0;
    ProjectRelationshipField m_relationship;
