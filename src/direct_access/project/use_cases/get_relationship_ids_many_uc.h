...
{
  public:
    explicit GetRelationshipIdsManyUseCase(std::unique_ptr<IProjectUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~GetRelationshipIdsManyUseCase() = default;

    QHash<int, QList<int>> execute(const QList<int> &projectIds, ProjectRelationshipField relationship);
