...
{
  public:
    explicit RemoveProjectUseCase(std::unique_ptr<IProjectUnitOfWork> uow) : m_uow(std::move(uow))
    {
    }
    ~RemoveProjectUseCase() = default;

    QList<int> execute(const QList<int> &projectIds);
    SCU::Result<void> undo();
    SCU::Result<void> redo();

  private:
    std::unique_ptr<IProjectUnitOfWork> m_uow;
    QList<int> m_originalProjectIds;
    QList<ProjectDto> m_removedProjects;
