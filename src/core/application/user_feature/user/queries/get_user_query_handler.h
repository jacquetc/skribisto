...
{
class SKRIBISTO_APPLICATION_USER_EXPORT GetUserQueryHandler : public QObject
{
    Q_OBJECT
  public:
    GetUserQueryHandler(InterfaceUserRepository *repository);
    Result<UserDTO> handle(QPromise<Result<void>> &progressPromise, const GetUserQuery &query);

  private:
    InterfaceUserRepository *m_repository;
    Result<UserDTO> handleImpl(QPromise<Result<void>> &progressPromise, const GetUserQuery &query);
