...
{
class SKRIBISTO_APPLICATION_ATELIER_EXPORT GetAtelierWithDetailsQueryHandler : public QObject
{
    Q_OBJECT
  public:
    GetAtelierWithDetailsQueryHandler(InterfaceAtelierRepository *repository);
    Result<AtelierWithDetailsDTO> handle(QPromise<Result<void>> &progressPromise, const GetAtelierQuery &query);

  private:
    InterfaceAtelierRepository *m_repository;
    Result<AtelierWithDetailsDTO> handleImpl(QPromise<Result<void>> &progressPromise, const GetAtelierQuery &query);
