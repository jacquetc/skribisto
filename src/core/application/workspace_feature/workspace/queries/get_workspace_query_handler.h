...
{
class SKRIBISTO_APPLICATION_WORKSPACE_EXPORT GetWorkspaceQueryHandler : public QObject
{
    Q_OBJECT
  public:
    GetWorkspaceQueryHandler(InterfaceWorkspaceRepository *repository);
    Result<WorkspaceDTO> handle(QPromise<Result<void>> &progressPromise, const GetWorkspaceQuery &query);

  private:
    InterfaceWorkspaceRepository *m_repository;
    Result<WorkspaceDTO> handleImpl(QPromise<Result<void>> &progressPromise, const GetWorkspaceQuery &query);
