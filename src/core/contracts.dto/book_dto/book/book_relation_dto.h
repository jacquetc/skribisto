...
class BookRelationDTO
{
  ...
  public:
    ...
    BookRelationDTO() : m_id(0), m_relationField(RelationField::Undefined), m_relatedIds({}), m_position(0)
    {
    }
    ...
    BookRelationDTO(int id, const RelationField &relationField, const QList<int> &relatedIds, int position)
        : m_id(id), m_relationField(relationField), m_relatedIds(relatedIds), m_position(position)
    {
    }

    BookRelationDTO(const BookRelationDTO &other)
    ...
    BookRelationDTO &operator=(const BookRelationDTO &other)
    {
        if (this != &other)
        {
    ...
    BookRelationDTO &mergeWith(const BookRelationDTO &other)
    {
        if (this != &other)
        {
    ...
    friend bool operator==(const BookRelationDTO &lhs, const BookRelationDTO &rhs);

    friend uint qHash(const BookRelationDTO &dto, uint seed) noexcept;
...
inline bool operator==(const BookRelationDTO &lhs, const BookRelationDTO &rhs)
{
...
inline uint qHash(const BookRelationDTO &dto, uint seed = 0) noexcept
...
Q_DECLARE_METATYPE(Skribisto::Contracts::DTO::Book::BookRelationDTO)