...
class BookDTO
{
  ...
  public:
    ...
    BookDTO() : m_id(0), m_uuid(QUuid()), m_creationDate(QDateTime()), m_updateDate(QDateTime()), m_title(QString())
    {
    }
    ...
    BookDTO(int id, const QUuid &uuid, const QDateTime &creationDate, const QDateTime &updateDate, const QString &title)
        : m_id(id), m_uuid(uuid), m_creationDate(creationDate), m_updateDate(updateDate), m_title(title)
    {
    }

    BookDTO(const BookDTO &other)
    ...
    BookDTO &operator=(const BookDTO &other)
    {
        if (this != &other)
        {
    ...
    BookDTO &mergeWith(const BookDTO &other)
    {
        if (this != &other)
        {
    ...
    friend bool operator==(const BookDTO &lhs, const BookDTO &rhs);

    friend uint qHash(const BookDTO &dto, uint seed) noexcept;
...
inline bool operator==(const BookDTO &lhs, const BookDTO &rhs)
{
...
inline uint qHash(const BookDTO &dto, uint seed = 0) noexcept
...
Q_DECLARE_METATYPE(Skribisto::Contracts::DTO::Book::BookDTO)