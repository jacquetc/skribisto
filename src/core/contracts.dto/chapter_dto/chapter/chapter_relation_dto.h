...
class ChapterRelationDTO
{
  ...
  public:
    ...
    ChapterRelationDTO() : m_id(0), m_relationField(RelationField::Undefined), m_relatedIds({}), m_position(0)
    {
    }
    ...
    ChapterRelationDTO(int id, const RelationField &relationField, const QList<int> &relatedIds, int position)
        : m_id(id), m_relationField(relationField), m_relatedIds(relatedIds), m_position(position)
    {
    }

    ChapterRelationDTO(const ChapterRelationDTO &other)
    ...
    ChapterRelationDTO &operator=(const ChapterRelationDTO &other)
    {
        if (this != &other)
        {
    ...
    ChapterRelationDTO &mergeWith(const ChapterRelationDTO &other)
    {
        if (this != &other)
        {
    ...
    friend bool operator==(const ChapterRelationDTO &lhs, const ChapterRelationDTO &rhs);

    friend uint qHash(const ChapterRelationDTO &dto, uint seed) noexcept;
...
inline bool operator==(const ChapterRelationDTO &lhs, const ChapterRelationDTO &rhs)
{
...
inline uint qHash(const ChapterRelationDTO &dto, uint seed = 0) noexcept
...
Q_DECLARE_METATYPE(Skribisto::Contracts::DTO::Chapter::ChapterRelationDTO)