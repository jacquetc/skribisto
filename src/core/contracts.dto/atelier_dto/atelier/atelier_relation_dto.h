...
class AtelierRelationDTO
{
  ...
  public:
    ...
    AtelierRelationDTO() : m_id(0), m_relationField(RelationField::Undefined), m_relatedIds({}), m_position(0)
    {
    }
    ...
    AtelierRelationDTO(int id, const RelationField &relationField, const QList<int> &relatedIds, int position)
        : m_id(id), m_relationField(relationField), m_relatedIds(relatedIds), m_position(position)
    {
    }

    AtelierRelationDTO(const AtelierRelationDTO &other)
    ...
    AtelierRelationDTO &operator=(const AtelierRelationDTO &other)
    {
        if (this != &other)
        {
    ...
    AtelierRelationDTO &mergeWith(const AtelierRelationDTO &other)
    {
        if (this != &other)
        {
    ...
    friend bool operator==(const AtelierRelationDTO &lhs, const AtelierRelationDTO &rhs);

    friend uint qHash(const AtelierRelationDTO &dto, uint seed) noexcept;
...
inline bool operator==(const AtelierRelationDTO &lhs, const AtelierRelationDTO &rhs)
{
...
inline uint qHash(const AtelierRelationDTO &dto, uint seed = 0) noexcept
...
Q_DECLARE_METATYPE(Skribisto::Contracts::DTO::Atelier::AtelierRelationDTO)