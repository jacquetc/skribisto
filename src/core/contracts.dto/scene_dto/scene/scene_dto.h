...
class SceneDTO
{
  ...
  public:
    ...
    SceneDTO(int id, const QUuid &uuid, const QDateTime &creationDate, const QDateTime &updateDate,
             const QString &title, const QString &label)
        : m_id(id), m_uuid(uuid), m_creationDate(creationDate), m_updateDate(updateDate), m_title(title), m_label(label)
    {
    }

    SceneDTO(const SceneDTO &other)
    ...
    SceneDTO &mergeWith(const SceneDTO &other)
    {
        if (this != &other)
        {
...
Q_DECLARE_METATYPE(Skribisto::Contracts::DTO::Scene::SceneDTO)