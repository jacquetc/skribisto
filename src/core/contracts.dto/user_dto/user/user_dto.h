...
class UserDTO
{
  ...
  public:
    ...
    UserDTO(int id, const QUuid &uuid, const QDateTime &creationDate, const QDateTime &updateDate, const QString &name,
            const QString &email)
    ...
    UserDTO &mergeWith(const UserDTO &other)
    {
        if (this != &other)
        {
    ...
    friend bool operator==(const UserDTO &lhs, const UserDTO &rhs);

    friend uint qHash(const UserDTO &dto, uint seed) noexcept;
...
inline uint qHash(const UserDTO &dto, uint seed = 0) noexcept
...
Q_DECLARE_METATYPE(Skribisto::Contracts::DTO::User::UserDTO)