...
UserRepository::UserRepository(InterfaceDatabaseTableGroup<Skribisto::Domain::User> *userDatabase)
    : Qleany::Repository::GenericRepository<Skribisto::Domain::User>(userDatabase)
{
    m_signalHolder.reset(new SignalHolder(nullptr));
}

SignalHolder *UserRepository::signalHolder()
{
    QReadLocker locker(&m_lock);
    return m_signalHolder.data();
}

Result<QHash<int, QList<int>>> UserRepository::removeInCascade(QList<int> ids)
{
    ...
    returnedHashOfEntityWithRemovedIds.insert(Skribisto::Domain::Entities::User, removedIdsResult.value());
...
Result<QHash<int, QList<int>>> UserRepository::changeActiveStatusInCascade(QList<int> ids, bool active)
{
    QWriteLocker locker(&m_lock);
    ...
    returnedHashOfEntityWithActiveChangedIds.insert(Skribisto::Domain::Entities::User, changedIdsResult.value());
