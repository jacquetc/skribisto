...
public:
    ...
    template<typename T>
    std::shared_ptr<Query<T>> createQuery(const QString &description);
...
Query<T>::Query(const QString &description, QObject *parent)
    : QueryBase(description, parent)
    , m_watcher(new QFutureWatcher<T>(this))
{
    connect(m_watcher, &QFutureWatcher<T>::finished,
            this, [this]() { this->onQueryFinished(); });
}

template<typename T>
void Query<T>::setQueryFunction(const std::function<T()> &function)
{
    m_queryFunction = function;
}

template<typename T>
void Query<T>::asyncExecute()
{
    if (m_queryFunction) {
...
template<typename T>
std::shared_ptr<Query<T>> QueryHandler::createQuery(const QString &description)
{
