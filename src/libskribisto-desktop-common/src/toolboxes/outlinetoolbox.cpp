...
OutlineToolbox::OutlineToolbox(QWidget *parent) : Toolbox(parent), ui(new Ui::OutlineToolbox), m_wasModified(false)
{
    ui->setupUi(this);
...
OutlineToolbox::~OutlineToolbox()
{
    ...
    QString uniqueDocumentReference = QString("%1_%2_%3")
                                          .arg(QString::number(this->treeItemAddress().projectId),
                                               QString::number(this->treeItemAddress().itemId), "secondary");
    textBridge->unsubscribeTextDocument(uniqueDocumentReference, ui->textEdit->uuid(),
...
void OutlineToolbox::initialize()
{
    ...
    QString uniqueDocumentReference = QString("%1_%2_%3")
                                          .arg(QString::number(this->treeItemAddress().projectId),
                                               QString::number(this->treeItemAddress().itemId), "secondary");
    textBridge->subscribeTextDocument(uniqueDocumentReference, ui->textEdit->uuid(), document);
    ...
    int textFontPointSize = settings.value("outlineToolbox/textFontPointSize", qApp->font().pointSize()).toInt() - 2;
    QString textFontFamily = settings.value("outlineToolbox/textFontFamily", qApp->font().family()).toString();
    ...
    int cursorPosition = SKRUserSettings::getFromProjectSettingHash(this->projectId(), "outlineTextCursorPosition",
...
{
    SKRUserSettings::insertInProjectSettingHash(this->projectId(), "outlineTextCursorPosition",
