...
class SKRWindowManager : public QObject
{
    Q_OBJECT
public:
    explicit SKRWindowManager(QObject *parent, QQmlApplicationEngine *engine, const QUrl& mainUrl);
    ~SKRWindowManager();
    ...
    void addEmptyWindow();
    void addWindow(int projectId = -1, int treeItemId = -1, const QString &pageType = "");
    void restoreWindows();
    Q_INVOKABLE void addWindowForItemId(int projectId, int treeItemId);
    Q_INVOKABLE void addWindowForProjectIndependantPageType(const QString &pageType);
    Q_INVOKABLE void insertAdditionalProperty(const QString &key, const QVariant &value);
    Q_INVOKABLE void insertAdditionalPropertyForViewManager(const QString &key, const QVariant &value);
