...
WritePageForm {
    ...
    writingZone.maximumTextAreaWidth: base.width - leftDock.width
    ...
    writingZone.onStretchChanged: writingZone.stretch ? resizeHandle.visible = false : resizeHandle.visible = true

    Connections {
        target: Globals
        onCompactSizeChanged: Globals.compactSize ? writingZone.stretch = true : writingZone.stretch = false
    }
    Connections {
        target: Globals
        onCompactSizeChanged: Globals.compactSize ? leftDock.contentWidth = undefined : leftDock.contentWidth = writingZone.textAreaLeftPos
    }
    Connections {
        target: Globals
        onCompactSizeChanged: Globals.compactSize ? leftDock.folded = true : leftDock.folded = false
    }
    Binding on leftDock.contentWidth {
        when: !Globals.compactSize
        value: writingZone.textAreaLeftPos
    }
//    Connections {
//        target: Globals
//        onCompactSizeChanged: Globals.compactSize ? leftDock.anchors.rightMargin = -base.width : leftDock.anchors.rightMargin =  -writingZone.textAreaLeftPos
//    }
    Binding on leftDock.anchors.rightMargin {
        when: !Globals.compactSize
        value: -writingZone.textAreaLeftPos
    }
    Binding on leftDock.anchors.rightMargin {
        when: Globals.compactSize && !leftDock.folded
        value: -base.width
    }
    Binding on leftDock.anchors.rightMargin {
        when: Globals.compactSize && leftDock.folded
    ...
    Binding on leftDock.anchors.bottom {
        when: !leftDock.folded
        value: leftDock.parent.bottom
    }
    Binding on leftDock.anchors.bottom {
        when: leftDock.folded
        value: 0
    }
    resizeHandleMouseArea.onContainsMouseChanged: {
        resizeHandleMouseArea.containsMouse ? resizeHandleMouseArea.cursorShape = Qt.SizeHorCursor : resizeHandleMouseArea.cursorShape = Qt.ArrowCursor
        resizeHandleMouseArea.containsMouse ? resizeHandle.color = "#ffa369" : resizeHandle.color = "transparent"
    }
    resizeHandleMouseArea.drag.target: resizeHandle
    resizeHandleMouseArea.drag.axis: Drag.XAxis
    resizeHandleMouseArea.drag.minimumX: 200
    resizeHandleMouseArea.drag.maximumX: base.width / 2 - 200
    ...
    resizeHandleMouseArea.onReleased:  {
        ...
        resizeHandle.color = "transparent"
    }
