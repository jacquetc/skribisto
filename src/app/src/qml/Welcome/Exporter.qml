...
ExporterForm {
    ...
    Connections {
        ...
        function onActiveProjectChanged(projectId){
            ...
            sheetProxyModel.projectIdFilter = projectId
            sheetProxyModel.checkAllButNonPrintable()
            noteProxyModel.projectIdFilter = projectId
            noteProxyModel.checkAllButNonPrintable()
        }
    }
    ...
    Component.onCompleted: {
        ...
        sheetProxyModel.projectIdFilter = activeProjectId
        sheetProxyModel.checkAllButNonPrintable()
        noteProxyModel.projectIdFilter = activeProjectId
        noteProxyModel.checkAllButNonPrintable()
    ...
    sheetTree.model: sheetProxyModel

    Action {
        id: selectAllSheetsAction
        text: selectAllSheetsAction.checked ? qsTr("Select none") : qsTr("Select all")
        //enabled: navigationMenu.opened
        //shortcut: "Ctrl+Shift+Del"
        icon.source: selectAllSheetsAction.checked ? "qrc:///icons/backup/edit-select-none.svg" : "qrc:///icons/backup/edit-select-all.svg"
        checkable: true
        onTriggered: {

            if(selectAllSheetsAction.checked){
                sheetProxyModel.checkAll()
            }
            else {
                sheetProxyModel.checkNone()
    ...
    selectAllSheetsToolButton.action: selectAllSheetsAction
    ...
    noteTree.model: noteProxyModel

    Action {
        id: selectAllNotesAction
        text: selectAllNotesAction.checked ? qsTr("Select none") : qsTr("Select all")
        //enabled: navigationMenu.opened
        //shortcut: "Ctrl+Shift+Del"
        icon.source: selectAllNotesAction.checked ? "qrc:///icons/backup/edit-select-none.svg" : "qrc:///icons/backup/edit-select-all.svg"
        checkable: true
        onTriggered: {

            if(selectAllNotesAction.checked){
                noteProxyModel.checkAll()
            }
            else {
                noteProxyModel.checkNone()
    ...
    selectAllNotesToolButton.action: selectAllNotesAction
    ...
    previewButton.onClicked: {
        ...
        exporter.sheetIdList = sheetProxyModel.getCheckedIdsList()
        exporter.noteIdList = noteProxyModel.getCheckedIdsList()
        exporter.run()
    ...
    onActiveFocusChanged: {
        if (activeFocus) {
            selectAllSheetsToolButton.forceActiveFocus()
        }
    }
