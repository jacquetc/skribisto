...
class IEmptyTrashUnitOfWork : public virtual Common::UnitOfWork::ITransactional
{
  public:
    ~IEmptyTrashUnitOfWork() override = default;

    DECLARE_UOW_ENTITY_GET(System);
    DECLARE_UOW_ENTITY_RELATIONSHIPS(System, SCDSystem::SystemRelationshipField);
    DECLARE_UOW_ENTITY_GET(TrashInfo);
    DECLARE_UOW_ENTITY_REMOVE(TrashInfo);
    DECLARE_UOW_ENTITY_GET(Binder);
    DECLARE_UOW_ENTITY_REMOVE(Binder);
    ...
    virtual void publishEmptyTrashSignal() = 0;
};
} // namespace Skribisto::TrashManagement
