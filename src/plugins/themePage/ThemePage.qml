...
ThemePageForm {
    ...
    themeListView.delegate: SkrListItemPane {
        ...
        contentItem: RowLayout {
            ...
            Rectangle {
                ...
                visible: themeListView.currentIndex === model.index
                         && themeListView.activeFocus
            }
            Rectangle {
            ...
            SkrRoundButton {
                id: lightButton
                display: AbstractButton.IconOnly
                text: SkrTheme.currentLightThemeName === model.text ? qsTr("Light") : qsTr(
                                                                          "Set as light theme")
                icon {
                    source: SkrTheme.currentLightThemeName
                            === model.text ? "qrc:///icons/backup/color-picker-white.svg" : ""
                    color: "transparent"
                }
                onClicked: {
            ...
            SkrRoundButton {
                id: darkButton
                display: AbstractButton.IconOnly
                text: SkrTheme.currentDarkThemeName === model.text ? qsTr("Dark") : qsTr(
                                                                         "Set as dark theme")
                icon {
                    source: SkrTheme.currentDarkThemeName
                            === model.text ? "qrc:///icons/backup/color-picker-black.svg" : ""
                    color: "transparent"
                }
            ...
            SkrRoundButton {
                ...
                display: AbstractButton.IconOnly
                text: SkrTheme.currentDistractionFreeThemeName
                      === model.text ? qsTr("Distraction free") : qsTr(
                                           "Set as distraction free theme")
                icon.source: SkrTheme.currentDistractionFreeThemeName
                             === model.text ? "qrc:///icons/backup/view-fullscreen.svg" : ""
    ...
    SimpleDialog {
        ...
        text: qsTr("Do you really want to remove the theme \"%1\"?").arg(
                  removeDialog.name)
    ...
    function populateColorPropertiesListModel() {
        ...
        for (var i in list) {
            ...
            if (SkrTheme.changedColorsMap[exactName]) {
                colorPropertiesModel.append({
                                                "text": propertyHumanNamesMap[exactName],
                                                "propertyExactName": exactName,
                                                "color": SkrTheme.changedColorsMap[exactName],
                                                "isEditable": isEditable
                                            })
            } else {
                colorPropertiesModel.append({
                                                "text": propertyHumanNamesMap[exactName],
                                                "propertyExactName": exactName,
                                                "color": propertyValuesMap[exactName],
                                                "isEditable": isEditable
                                            })
            }
        }
    ...
    primaryTextAreaSample.textArea.text: qsTr("Primary text")
