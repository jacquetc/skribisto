...
class DummyEntityWithDetailsDTO
{
  ...
  public:
    ...
    DummyEntityWithDetailsDTO(int id, const QUuid &uuid, const QDateTime &creationDate, const QString &name,
                              const QList<DummyBasicEntityDTO> &details)
        : m_id(id), m_uuid(uuid), m_creationDate(creationDate), m_name(name), m_details(details)
    {
    }
    ...
    friend bool operator==(const DummyEntityWithDetailsDTO &lhs, const DummyEntityWithDetailsDTO &rhs);

    friend uint qHash(const DummyEntityWithDetailsDTO &dto, uint seed) noexcept;
